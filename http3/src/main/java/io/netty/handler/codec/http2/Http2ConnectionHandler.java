...
public class Http2ConnectionHandler extends ByteToMessageDecoder implements Http2LifecycleManager,
    ...
    protected Http2ConnectionHandler(final Http2ConnectionDecoder decoder, final Http2ConnectionEncoder encoder,
                                     final Http2Settings initialSettings) {
        this(decoder, encoder, initialSettings, false);
    }

    protected Http2ConnectionHandler(final Http2ConnectionDecoder decoder, final Http2ConnectionEncoder encoder,
                                     final Http2Settings initialSettings, final boolean decoupleCloseAndGoAway) {
    ...
    private final class PrefaceDecoder extends BaseDecoder {
        ...
        private boolean readClientPrefaceString(final ByteBuf in) throws Http2Exception {
            ...
            if (bytesRead == 0 || !ByteBufUtil.equals(in, in.readerIndex(),
                ...
                final String receivedBytes = hexDump(in, in.readerIndex(),
    ...
    public void close(final ChannelHandlerContext ctx, ChannelPromise promise) throws Exception {
        ...
        final ChannelFuture f = connection().goAwaySent() ? ctx.write(EMPTY_BUFFER) : goAway(ctx, null, ctx.newPromise());
        ctx.flush();
    ...
    public void onError(final ChannelHandlerContext ctx, final boolean outbound, final Throwable cause) {
        final Http2Exception embedded = getEmbeddedHttp2Exception(cause);
        if (isStreamError(embedded)) {
            ...
            final CompositeStreamException compositException = (CompositeStreamException) embedded;
            for (final StreamException streamException : compositException) {
     ...
     */
    protected void onConnectionError(final ChannelHandlerContext ctx, final boolean outbound,
                                     final Throwable cause, Http2Exception http2Ex) {
        if (http2Ex == null) {
     ...
     */
    protected void onStreamError(final ChannelHandlerContext ctx, final boolean outbound,
                                 @SuppressWarnings("unused") final Throwable cause, final StreamException http2Ex) {
     ...
     */
    private ChannelFuture resetUnknownStream(final ChannelHandlerContext ctx, final int streamId, final long errorCode,
    ...
    @Override
    public ChannelFuture resetStream(final ChannelHandlerContext ctx, final int streamId, final long errorCode,
                                     final ChannelPromise promise) {
    ...
    private ChannelFuture resetStream(final ChannelHandlerContext ctx, final Http2Stream stream,
                                      final long errorCode, ChannelPromise promise) {
        promise = promise.unvoid();
    ...
    public ChannelFuture goAway(final ChannelHandlerContext ctx, final int lastStreamId, final long errorCode,
        ...
        debugData.retain();
        final ChannelFuture future = frameWriter().writeGoAway(ctx, lastStreamId, errorCode, debugData, promise);
    ...
    private ChannelFuture goAway(final ChannelHandlerContext ctx, final Http2Exception cause, final ChannelPromise promise) {
        final long errorCode = cause != null ? cause.error().code() : NO_ERROR.code();
    ...
    private static void processGoAwayWriteResult(final ChannelHandlerContext ctx, final int lastStreamId,
                                                 final long errorCode, final ByteBuf debugData, final ChannelFuture future) {
        try {
            if (future.isSuccess()) {
