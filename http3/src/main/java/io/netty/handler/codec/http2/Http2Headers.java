...
public interface Http2Headers extends Headers<CharSequence, CharSequence, Http2Headers> {
  ...
  enum PseudoHeaderName {
    ...
    private static final CharSequenceMap<PseudoHeaderName> PSEUDO_HEADERS =
        new CharSequenceMap<PseudoHeaderName>();

    static {
      for (final PseudoHeaderName pseudoHeader : PseudoHeaderName.values()) {
        PSEUDO_HEADERS.add(pseudoHeader.value(), pseudoHeader);
      }
    }

    PseudoHeaderName(final String value, final boolean requestOnly) {
    ...
    /**
     * Indicates whether the specified header follows the pseudo-header format (begins with ':'
     * character)
     *
    ...
    public static boolean hasPseudoHeaderFormat(final CharSequence headerName) {
      if (headerName instanceof AsciiString) {
        ...
        return asciiHeaderName.length() > 0
            && asciiHeaderName.byteAt(0) == PSEUDO_HEADER_PREFIX_BYTE;
      } else {
  ...
  /**
   * Returns an iterator over all HTTP/2 headers. The iteration order is as follows: 1. All pseudo
   * headers (order not specified). 2. All non-pseudo headers (in insertion order).
   */
  @Override
