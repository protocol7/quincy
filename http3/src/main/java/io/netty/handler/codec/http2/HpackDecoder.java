...
final class HpackDecoder {
    ...
    public void decode(final int streamId, final ByteBuf in, final Http2Headers headers, final boolean validateHeaders) throws Http2Exception {
        final Http2HeadersSink sink = new Http2HeadersSink(streamId, headers, maxHeaderListSize, validateHeaders);
        decode(in, sink);
    ...
    private static HeaderType validate(final int streamId, final CharSequence name,
                                       final HeaderType previousHeaderType) throws Http2Exception {
    ...
    private CharSequence readName(final int index) throws Http2Exception {
        ...
        if (index - HpackStaticTable.length <= hpackDynamicTable.length()) {
            final HpackHeaderField hpackHeaderField = hpackDynamicTable.getEntry(index - HpackStaticTable.length);
            return hpackHeaderField.name;
        }
    ...
    private void insertHeader(final Sink sink, final CharSequence name, final CharSequence value, final IndexType indexType) {
