...
public class DefaultHttp2ConnectionEncoder
  ...
  private final Queue<Http2Settings> outstandingLocalSettingsQueue =
      new ArrayDeque<Http2Settings>(4);
  private Queue<Http2Settings> outstandingRemoteSettingsQueue;

  public DefaultHttp2ConnectionEncoder(
      final Http2Connection connection, final Http2FrameWriter frameWriter) {
    ...
    if (pushEnabled != null) {
      if (!connection.isServer() && pushEnabled) {
        throw connectionError(
            PROTOCOL_ERROR, "Client received a value of ENABLE_PUSH specified to other than 0");
      }
  ...
  public ChannelFuture writeData(
      ...
      switch (stream.state()) {
        ...
        default:
          throw new IllegalStateException(
              "Stream " + stream.id() + " in unexpected state " + stream.state());
      }
    ...
    flowController()
        .addFlowControlled(
            stream, new FlowControlledData(stream, data, padding, endOfStream, promise));
    return promise;
  }
  ...
  public ChannelFuture writeHeaders(
      ...
      final ChannelPromise promise) {
    return writeHeaders(
        ctx, streamId, headers, 0, DEFAULT_PRIORITY_WEIGHT, false, padding, endStream, promise);
  }

  private static boolean validateHeadersSentState(
      final Http2Stream stream,
      final Http2Headers headers,
      final boolean isServer,
      final boolean endOfStream) {
    final boolean isInformational =
        isServer && HttpStatusClass.valueOf(headers.status()) == INFORMATIONAL;
    if ((isInformational || !endOfStream) && stream.isHeadersSent() || stream.isTrailersSent()) {
      throw new IllegalStateException(
          "Stream " + stream.id() + " sent too many headers EOS: " + endOfStream);
    }
  ...
  public ChannelFuture writeHeaders(
    ...
    try {
      ...
      // Trailing headers must go through flow control if there are other frames queued in flow
      // control
      // for this stream.
      ...
      if (!endOfStream || !flowController.hasFlowControlled(stream)) {
        ...
        if (endOfStream) {
          // Must handle calling onError before calling closeStreamLocal, otherwise the error
          // handler will
          // incorrectly think the stream no longer exists and so may not send RST_STREAM or perform
          // similar
          // appropriate action.
        ...
        // Pass headers to the flow-controller so it can maintain their sequence relative to DATA
        // frames.
        flowController.addFlowControlled(
            stream,
            new FlowControlledHeaders(
                stream, headers, streamDependency, weight, exclusive, padding, true, promise));
        return promise;
      }
  ...
  public ChannelFuture writeSettingsAck(
    ...
    if (settings == null) {
      return promise.setFailure(
          new Http2Exception(
              INTERNAL_ERROR, "attempted to write a SETTINGS ACK with no " + " pending SETTINGS"));
    }
    final SimpleChannelPromiseAggregator aggregator =
        new SimpleChannelPromiseAggregator(promise, ctx.channel(), ctx.executor());
    // Acknowledge receipt of the settings. We should do this before we process the settings to
    // ensure our
    // remote peer applies these settings before any subsequent frames that we may send which depend
    // upon
    // these new settings. See https://github.com/netty/netty/issues/6520.
    frameWriter.writeSettingsAck(ctx, aggregator.newPromise());

    // We create a "new promise" to make sure that status from both the write and the application
    // are taken into
  ...
  public ChannelFuture writeWindowUpdate(
    ...
    return promise.setFailure(
        new UnsupportedOperationException(
            "Use the Http2[Inbound|Outbound]FlowController" + " objects to control window sizes"));
  }
  ...
  /**
   * Wrap a DATA frame so it can be written subject to flow-control. Note that this implementation
   * assumes it only writes padding once for the entire payload as opposed to writing it once
   * per-frame. This makes the {@link #size} calculation deterministic thereby greatly simplifying
   * the implementation.
   *
   * <p>If frame-splitting is required to fit within max-frame-size and flow-control constraints we
   * ensure that the passed promise is not completed until last frame write.
   */
  private final class FlowControlledData extends FlowControlledBase {
    ...
    public void error(final ChannelHandlerContext ctx, final Throwable cause) {
      ...
      // Don't update dataSize because we need to ensure the size() method returns a consistent size
      // even after
      // error so we don't invalidate flow control when returning bytes to flow control.
      //
      // That said we will set dataSize and padding to 0 in the write(...) method if we cleared the
      // queue
      // because of an error.
    ...
    public void write(final ChannelHandlerContext ctx, final int allowedBytes) {
      ...
      if (!endOfStream) {
        if (queuedData == 0) {
          if (queue.isEmpty()) {
            ...
            // (as otherwise we will have at least 1 entry in there), which will happen either when
            // called
            // explicit or when the write itself fails. In this case just set dataSize and padding
            // to 0
            ...
            // There's no need to write any data frames because there are only empty data frames in
            // the
            // queue and it is not end of stream yet. Just complete their promises by getting the
            // buffer
            // corresponding to 0 bytes and writing it to the channel (to preserve notification
            // order).
      ...
      frameWriter()
          .writeData(
              ctx, stream.id(), toWrite, writablePadding, endOfStream && size() == 0, writePromise);
    }

    @Override
    public boolean merge(
        final ChannelHandlerContext ctx, final Http2RemoteFlowController.FlowControlled next) {
      ...
      dataSize = queue.readableBytes();
      // Given that we're merging data into a frame it doesn't really make sense to accumulate
      // padding.
  ...
  private void notifyLifecycleManagerOnError(
      final ChannelFuture future, final ChannelHandlerContext ctx) {
    future.addListener(
  ...
  /**
   * Wrap headers so they can be written subject to flow-control. While headers do not have cost
   * against the flow-control window their order with respect to other frames must be maintained,
   * hence if a DATA frame is blocked on flow-control a HEADER frame must wait until this frame has
   * been written.
   */
  private final class FlowControlledHeaders extends FlowControlledBase {
    ...
    public void write(final ChannelHandlerContext ctx, final int allowedBytes) {
      final boolean isInformational =
          validateHeadersSentState(stream, headers, connection.isServer(), endOfStream);
      // The code is currently requiring adding this listener before writing, in order to call
      // onError() before
      // closeStreamLocal().
