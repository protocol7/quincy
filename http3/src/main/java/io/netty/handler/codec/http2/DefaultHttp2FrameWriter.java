...
public class DefaultHttp2FrameWriter implements Http2FrameWriter, Http2FrameSizePolicy, Configuration {
    ...
    public DefaultHttp2FrameWriter(final SensitivityDetector headersSensitivityDetector, final boolean ignoreMaxHeaderListSize) {
    ...
    @Override
    public ChannelFuture writeData(final ChannelHandlerContext ctx, final int streamId, ByteBuf data,
                                   int padding, final boolean endStream, final ChannelPromise promise) {
    ...
    public ChannelFuture writeSettings(final ChannelHandlerContext ctx, final Http2Settings settings,
        ...
        try {
            ...
            final ByteBuf buf = ctx.alloc().buffer(FRAME_HEADER_LENGTH + settings.size() * SETTING_ENTRY_LENGTH);
    ...
    public ChannelFuture writePushPromise(final ChannelHandlerContext ctx, final int streamId,
        ...
        try {
            ...
            final ByteBuf fragment = headerBlock.readRetainedSlice(min(headerBlock.readableBytes(), maxFragmentLength));
    ...
    @Override
    public ChannelFuture writeWindowUpdate(final ChannelHandlerContext ctx, final int streamId,
                                           final int windowSizeIncrement, final ChannelPromise promise) {
        try {
    ...
    private ChannelFuture writeHeadersInternal(final ChannelHandlerContext ctx,
                                               final int streamId, final Http2Headers headers, final int padding, final boolean endStream,
                                               final boolean hasPriority, final int streamDependency, final short weight, final boolean exclusive, final ChannelPromise promise) {
        ByteBuf headerBlock = null;
        final SimpleChannelPromiseAggregator promiseAggregator =
                new SimpleChannelPromiseAggregator(promise, ctx.channel(), ctx.executor());
        try {
     ...
     */
    private ChannelFuture writeContinuationFrames(final ChannelHandlerContext ctx, final int streamId,
                                                  final ByteBuf headerBlock, final int padding, final SimpleChannelPromiseAggregator promiseAggregator) {
