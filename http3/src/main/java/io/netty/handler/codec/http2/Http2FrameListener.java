...
public interface Http2FrameListener {
   ...
   *     for this stream.
   * @return the number of bytes that have been processed by the application. The returned bytes are
   *     used by the inbound flow controller to determine the appropriate time to expand the inbound
   *     flow control window (i.e. send {@code WINDOW_UPDATE}). Returning a value equal to the
   *     length of {@code data} + {@code padding} will effectively opt-out of application-level flow
   *     control for this frame. Returning a value less than the length of {@code data} + {@code
   *     padding} will defer the returning of the processed bytes, which the application must later
   *     return via {@link Http2LocalFlowController#consumeBytes(Http2Stream, int)}. The returned
   *     value must be >= {@code 0} and <= {@code data.readableBytes()} + {@code padding}.
   */
  int onDataRead(
   ...
   *
   * <p>Only one of the following methods will be called for each {@code HEADERS} frame sequence.
   * One will be called when the {@code END_HEADERS} flag has been received.
   *
   * <ul>
   *   <li>{@link #onHeadersRead(ChannelHandlerContext, int, Http2Headers, int, boolean)}
   *   <li>{@link #onHeadersRead(ChannelHandlerContext, int, Http2Headers, int, short, boolean, int,
   *       boolean)}
   *   <li>{@link #onPushPromiseRead(ChannelHandlerContext, int, int, Http2Headers, int)}
   * </ul>
   *
   * <p>To say it another way; the {@link Http2Headers} will contain all of the headers for the
   * current message exchange step (additional queuing is not necessary).
   *
   * @param ctx the context from the handler where the frame was read.
   * @param streamId the subject stream for the frame.
  ...
  /**
   * Handles an inbound {@code HEADERS} frame with priority information specified. Only called if
   * {@code END_HEADERS} encountered.
   *
   * <p>Only one of the following methods will be called for each {@code HEADERS} frame sequence.
   * One will be called when the {@code END_HEADERS} flag has been received.
   *
   ...
   *
   * <p>Note that is it possible to have this method called and no stream object exist for either
   * {@code streamId}, {@code streamDependency}, or both. This is because the {@code PRIORITY} frame
   * can be sent/received when streams are in the {@code CLOSED} state.
   *
   * @param ctx the context from the handler where the frame was read.
   * @param streamId the subject stream for the frame.
   * @param streamDependency the stream on which this stream depends, or 0 if dependent on the
   ...
   *
   * <p>Promised requests MUST be authoritative, cacheable, and safe. See <a
   * href="https://tools.ietf.org/html/draft-ietf-httpbis-http2-17#section-8.2">[RFC http2], Section
   * 8.2</a>.
   *
   * <p>Only one of the following methods will be called for each {@code HEADERS} frame sequence.
   * One will be called when the {@code END_HEADERS} flag has been received.
   ...
   * @param ctx the context from the handler where the frame was read.
   * @param streamId the stream the frame was sent on.
   * @param windowSizeIncrement the increased number of bytes of the remote endpoint's flow control
   *     window.
   */
