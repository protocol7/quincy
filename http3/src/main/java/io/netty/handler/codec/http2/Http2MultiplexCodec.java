...
public class Http2MultiplexCodec extends Http2FrameCodec {
    ...
    Http2MultiplexCodec(final Http2ConnectionEncoder encoder,
                        final Http2ConnectionDecoder decoder,
                        final Http2Settings initialSettings,
                        final ChannelHandler inboundStreamHandler,
                        final ChannelHandler upgradeStreamHandler, final boolean decoupleCloseAndGoAway) {
    ...
    private void onHttp2UpgradeStreamInitialized(final ChannelHandlerContext ctx, final Http2MultiplexCodecStream stream) {
    ...
    @Override
    final void onHttp2StreamStateChanged(final ChannelHandlerContext ctx, final Http2FrameStream stream) {
        ...
        switch (stream.state()) {
            ...
            case OPEN:
                ...
                // fall-trough
                final ChannelFuture future = ctx.channel().eventLoop().register(new DefaultHttp2StreamChannel(s, false));
                if (future.isDone()) {
                    registerDone(future);
                } else {
                    future.addListener(CHILD_CHANNEL_REGISTRATION_LISTENER);
    ...
    @Override
    final void onHttp2StreamWritabilityChanged(final ChannelHandlerContext ctx, final Http2FrameStream stream, final boolean writable) {
    ...
    private void tryRemoveChildChannelFromReadPendingQueue(final DefaultHttp2StreamChannel childChannel) {
        if (isChildChannelInReadPendingQueue(childChannel)) {
            removeChildChannelFromReadPendingQueue(childChannel);
        }
    }

    private void removeChildChannelFromReadPendingQueue(final DefaultHttp2StreamChannel childChannel) {
        ...
        private final class Http2ChannelUnsafe implements Unsafe {
            ...
            public void close(final ChannelPromise promise) {
                ...
                if (parent().isActive() && !readEOS && connection().streamMayHaveExisted(stream().id())) {
                    final Http2StreamFrame resetFrame = new DefaultHttp2ResetFrame(Http2Error.CANCEL).stream(stream());
            ...
            public void write(final Object msg, final ChannelPromise promise) {
                ...
                try {
                    if (msg instanceof Http2StreamFrame) {
                        final Http2StreamFrame frame = validateStreamFrame((Http2StreamFrame) msg).stream(stream());
