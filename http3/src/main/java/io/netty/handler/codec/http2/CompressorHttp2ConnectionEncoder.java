...
/**
 * A decorating HTTP2 encoder that will compress data frames according to the {@code
 * content-encoding} header for each stream. The compression provided by this class will be applied
 * to the data for the entire stream.
 */
@UnstableApi
public class CompressorHttp2ConnectionEncoder extends DecoratingHttp2ConnectionEncoder {
  ...
  public CompressorHttp2ConnectionEncoder(
  ...
  public ChannelFuture writeData(
    ...
    final EmbeddedChannel channel =
        stream == null ? null : (EmbeddedChannel) stream.getProperty(propertyKey);
    if (channel == null) {
      // The compressor may be null if no compatible encoding type was found in this stream's
      // headers
      ...
      if (buf == null) {
        if (endOfStream) {
          ...
          return super.writeData(
              ctx, streamId, buf == null ? Unpooled.EMPTY_BUFFER : buf, padding, true, promise);
        }
  ...
  public ChannelFuture writeHeaders(
    ...
    try {
      ...
      final EmbeddedChannel compressor = newCompressor(ctx, headers, endStream);
  ...
  public ChannelFuture writeHeaders(
    ...
    try {
      // Determine if compression is required and sanitize the headers.
      final EmbeddedChannel compressor = newCompressor(ctx, headers, endOfStream);
  ...
  /**
   * Returns a new {@link EmbeddedChannel} that encodes the HTTP2 message content encoded in the
   ...
   */
  protected EmbeddedChannel newContentCompressor(
  ...
  /**
   * Returns the expected content encoding of the decoded content. Returning {@code contentEncoding}
   * is the default behavior, which is the case for most compressors.
   *
   * @param contentEncoding the value of the {@code content-encoding} header
   ...
   *     exception
   */
  protected CharSequence getTargetContentEncoding(final CharSequence contentEncoding)
   ...
   */
  private EmbeddedChannel newCompressionChannel(
      final ChannelHandlerContext ctx, final ZlibWrapper wrapper) {
  ...
  /**
   * Checks if a new compressor object is needed for the stream identified by {@code streamId}. This
   * method will modify the {@code content-encoding} header contained in {@code headers}.
   *
   * @param ctx the context.
   ...
   */
  private EmbeddedChannel newCompressor(
      final ChannelHandlerContext ctx, final Http2Headers headers, final boolean endOfStream)
      throws Http2Exception {
    if (endOfStream) {
      return null;
    }
    ...
    if (compressor != null) {
      final CharSequence targetContentEncoding = getTargetContentEncoding(encoding);
  ...
  /**
   * Release remaining content from {@link EmbeddedChannel} and remove the compressor from the
   * {@link Http2Stream}.
   *
  ...
  /**
   * Read the next compressed {@link ByteBuf} from the {@link EmbeddedChannel} or {@code null} if
   * one does not exist.
   *
