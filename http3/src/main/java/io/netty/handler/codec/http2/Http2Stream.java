...
public interface Http2Stream {
  ...
  enum State {
    ...
    State(final boolean localSideOpen, final boolean remoteSideOpen) {
      this.localSideOpen = localSideOpen;
      this.remoteSideOpen = remoteSideOpen;
    }

    /**
     * Indicates whether the local side of this stream is open (i.e. the state is either {@link
     * State#OPEN} or {@link State#HALF_CLOSED_REMOTE}).
     */
    ...
    /**
     * Indicates whether the remote side of this stream is open (i.e. the state is either {@link
     * State#OPEN} or {@link State#HALF_CLOSED_LOCAL}).
     */
  ...
  /**
   * Opens this stream, making it available via {@link
   * Http2Connection#forEachActiveStream(Http2StreamVisitor)} and transition state to:
   *
   * <ul>
   *   <li>{@link State#OPEN} if {@link #state()} is {@link State#IDLE} and {@code halfClosed} is
   *       {@code false}.
   *   <li>{@link State#HALF_CLOSED_LOCAL} if {@link #state()} is {@link State#IDLE} and {@code
   *       halfClosed} is {@code true} and the stream is local. In this state, {@link
   *       #isHeadersSent()} is {@code true}
   *   <li>{@link State#HALF_CLOSED_REMOTE} if {@link #state()} is {@link State#IDLE} and {@code
   *       halfClosed} is {@code true} and the stream is remote. In this state, {@link
   *       #isHeadersReceived()} is {@code true}
   *   <li>{@link State#RESERVED_LOCAL} if {@link #state()} is {@link State#HALF_CLOSED_REMOTE}.
   *   <li>{@link State#RESERVED_REMOTE} if {@link #state()} is {@link State#HALF_CLOSED_LOCAL}.
   * </ul>
   */
  ...
  /**
   * Indicates whether a {@code RST_STREAM} frame has been sent from the local endpoint for this
   * stream.
   */
  boolean isResetSent();
   ...
   *
   * @return The value that was previously associated with {@code key}, or {@code null} if there was
   *     none.
   */
  ...
  /**
   * Indicates that headers have been sent to the remote endpoint on this stream. The first call to
   * this method would be for the initial headers (see {@link #isHeadersSent()}} and the second call
   * would indicate the trailers (see {@link #isTrailersReceived()}).
   *
   * @param isInformational {@code true} if the headers contain an informational status code (for
   *     responses only).
   */
  ...
  /**
   * Indicates that headers have been received. The first call to this method would be for the
   * initial headers (see {@link #isHeadersReceived()}} and the second call would indicate the
   * trailers (see {@link #isTrailersReceived()}).
   *
   * @param isInformational {@code true} if the headers contain an informational status code (for
   *     responses only).
   */
