...
import static com.protocol7.quincy.http3.Http2Error.COMPRESSION_ERROR;
...
final class HpackHuffmanDecoder {
  ...
  private static final Http2Exception EOS_DECODED =
      ThrowableUtil.unknownStackTrace(
          connectionError(COMPRESSION_ERROR, "HPACK - EOS Decoded"),
          HpackHuffmanDecoder.class,
          "decode(..)");
  private static final Http2Exception INVALID_PADDING =
      ThrowableUtil.unknownStackTrace(
          connectionError(COMPRESSION_ERROR, "HPACK - Invalid Padding"),
          HpackHuffmanDecoder.class,
          "decode(..)");

  private static final Node ROOT =
      buildTree(HpackUtil.HUFFMAN_CODES, HpackUtil.HUFFMAN_CODE_LENGTHS);
  ...
  private static final class DecoderProcessor implements ByteProcessor {
    ...
    private void append(final int i) {
      if (bytes.length == index) {
        ...
        // 1024 was choosen as a good guess and we may be able to investigate more if there are
        // better choices.
        // See also https://github.com/netty/netty/issues/6846
        final int newLength =
            bytes.length >= 1024 ? bytes.length + initialCapacity : bytes.length << 1;
