...
import static com.protocol7.quincy.http3.Http2Exception.headerListSizeError;
...
public final class Http2CodecUtil {
  ...
  /**
   * The maximum number of padding bytes. That is the 255 padding bytes appended to the end of a
   * frame and the 1 byte pad length field.
   */
  ...
  public static final int DATA_FRAME_HEADER_LENGTH =
      FRAME_HEADER_LENGTH + MAX_PADDING_LENGTH_LENGTH;
  ...
  /**
   * <a href="https://tools.ietf.org/html/rfc7540#section-6.5.2">The initial value of this setting
   * is unlimited</a>. However in practice we don't want to allow our peers to use unlimited memory
   * by default. So we take advantage of the <q>For any given request, a lower limit than what is
   * advertised MAY be enforced.</q> loophole.
   */
  ...
  /**
   * The assumed minimum value for {@code SETTINGS_MAX_CONCURRENT_STREAMS} as recommended by the <a
   * herf="https://tools.ietf.org/html/rfc7540#section-6.5.2">HTTP/2 spec</a>.
   */
  ...
  public static boolean isOutboundStream(final boolean server, final int streamId) {
    final boolean even = (streamId & 1) == 0;
  ...
  /**
   * Creates a buffer containing the error message from the given exception. If the cause is {@code
   * null} returns an empty buffer.
   */
  ...
  /**
   * Results in a RST_STREAM being sent for {@code streamId} due to violating <a
   * href="https://tools.ietf.org/html/rfc7540#section-6.5.2">SETTINGS_MAX_HEADER_LIST_SIZE</a>.
   *
   * @param streamId The stream ID that was being processed when the exceptional condition occurred.
   * @param maxHeaderListSize The max allowed size for a list of headers in bytes which was
   *     exceeded.
   * @param onDecode {@code true} if the exception was encountered during decoder. {@code false} for
   *     encode.
   * @throws Http2Exception a stream error.
   */
  public static void headerListSizeExceeded(
      final int streamId, final long maxHeaderListSize, final boolean onDecode)
      throws Http2Exception {
    throw headerListSizeError(
        streamId,
        Http2Error.PROTOCOL_ERROR,
        onDecode,
        "Header size exceeded max " + "allowed size (%d)",
        maxHeaderListSize);
  }

  /**
   * Results in a GO_AWAY being sent due to violating <a
   * href="https://tools.ietf.org/html/rfc7540#section-6.5.2">SETTINGS_MAX_HEADER_LIST_SIZE</a> in
  ...
  static void writeFrameHeaderInternal(
      final ByteBuf out,
      final int payloadLength,
      final byte type,
      final Http2Flags flags,
  ...
  /**
   * Provides the ability to associate the outcome of multiple {@link ChannelPromise} objects into a
   * single {@link ChannelPromise} object.
   */
  static final class SimpleChannelPromiseAggregator extends DefaultChannelPromise {
    ...
    SimpleChannelPromiseAggregator(
        final ChannelPromise promise, final Channel c, final EventExecutor e) {
      super(c, e);
    ...
    /**
     * Allocate a new promise which will be used to aggregate the overall success of this promise
     * aggregator.
     *
     * @return A new promise which will be aggregated. {@code null} if {@link
     *     #doneAllocatingPromises()} was previously called.
     */
     ...
     *
     * @return The promise that is the aggregation of all promises allocated with {@link
     *     #newPromise()}.
     */
     ...
     *
     * <p>This method will NOT throw an {@link IllegalStateException} if called multiple times
