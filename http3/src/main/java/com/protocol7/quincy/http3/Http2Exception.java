...
public class Http2Exception extends Exception {
  ...
  public Http2Exception(
      final Http2Error error, final String message, final ShutdownHint shutdownHint) {
    super(message);
    this.error = checkNotNull(error, "error");
    this.shutdownHint = checkNotNull(shutdownHint, "shutdownHint");
  }
  ...
  public Http2Exception(
      ...
      final ShutdownHint shutdownHint) {
    super(message, cause);
    this.error = checkNotNull(error, "error");
    this.shutdownHint = checkNotNull(shutdownHint, "shutdownHint");
  }
   ...
   */
  public static Http2Exception connectionError(
      final Http2Error error, final String fmt, final Object... args) {
   ...
   */
  public static Http2Exception connectionError(
      final Http2Error error, final Throwable cause, final String fmt, final Object... args) {
   ...
   * @return An exception which can be translated into a HTTP/2 error.
   */
  public static Http2Exception closedStreamError(
      final Http2Error error, final String fmt, final Object... args) {
   ...
   * {@link Http2CodecUtil#CONNECTION_STREAM_ID} then a {@link Http2Exception.StreamException} will
   * be returned. Otherwise the error is considered a connection error and a {@link Http2Exception}
   * is returned.
   *
   ...
   * @param fmt String with the content and format for the additional debug data.
   * @param args Objects which fit into the format defined by {@code fmt}.
   * @return If the {@code id} is not {@link Http2CodecUtil#CONNECTION_STREAM_ID} then a {@link
   *     Http2Exception.StreamException} will be returned. Otherwise the error is considered a
   *     connection error and a {@link Http2Exception} is returned.
   */
  public static Http2Exception streamError(
      final int id, final Http2Error error, final String fmt, final Object... args) {
  ...
  /**
   * A specific stream error resulting from failing to decode headers that exceeds the max header
   * size list. If the {@code id} is not {@link Http2CodecUtil#CONNECTION_STREAM_ID} then a {@link
   * Http2Exception.StreamException} will be returned. Otherwise the error is considered a
   * connection error and a {@link Http2Exception} is returned.
   *
   * @param id The stream id for which the error is isolated to.
   ...
   * @param args Objects which fit into the format defined by {@code fmt}.
   * @return If the {@code id} is not {@link Http2CodecUtil#CONNECTION_STREAM_ID} then a {@link
   *     HeaderListSizeException} will be returned. Otherwise the error is considered a connection
   *     error and a {@link Http2Exception} is returned.
   */
  public static Http2Exception headerListSizeError(
      final int id,
      final Http2Error error,
      final boolean onDecode,
      final String fmt,
      final Object... args) {
    return Http2CodecUtil.CONNECTION_STREAM_ID == id
        ? Http2Exception.connectionError(error, fmt, args)
  ...
  public static int streamId(final Http2Exception e) {
    return isStreamError(e)
        ? ((StreamException) e).streamId()
  ...
  public enum ShutdownHint {
    ...
    NO_SHUTDOWN,
    /**
     * Attempt to execute a "graceful" shutdown. The definition of "graceful" is left to the
     * implementation. An example of "graceful" would be wait for some amount of time until all
  ...
  public static final class ClosedStreamCreationException extends Http2Exception {
    ...
    public ClosedStreamCreationException(
        final Http2Error error, final String message, final Throwable cause) {
      super(error, message, cause);
    }
  }

  /**
   * Represents an exception that can be isolated to a single stream (as opposed to the entire
   * connection).
   */
  public static class StreamException extends Http2Exception {
    ...
    StreamException(
        final int streamId, final Http2Error error, final String message, final Throwable cause) {
  ...
  public static final class HeaderListSizeException extends StreamException {
    ...
    HeaderListSizeException(
        final int streamId, final Http2Error error, final String message, final boolean decode) {
