 ...
 * <p>
 *
 * <p>If a {@code GOAWAY} frame is received from the remote endpoint, all buffered writes for
 * streams with an ID less than the specified {@code lastStreamId} will immediately fail with a
 * {@link Http2GoAwayException}.
 *
 * <p>
 *
 * <p>If the channel/encoder gets closed, all new and buffered writes will immediately fail with a
...
public class StreamBufferingEncoder extends DecoratingHttp2ConnectionEncoder {
  ...
  /**
   * Thrown by {@link StreamBufferingEncoder} if buffered streams are terminated due to receipt of a
   * {@code GOAWAY}.
   */
  public static final class Http2GoAwayException extends Http2Exception {
    ...
    public Http2GoAwayException(
        final int lastStreamId, final long errorCode, final byte[] debugData) {
  ...
  public StreamBufferingEncoder(
      final Http2ConnectionEncoder delegate, final int initialMaxConcurrentStreams) {
    super(delegate);
    ...
    connection()
        .addListener(
            new Http2ConnectionAdapter() {
              ...
              @Override
              public void onGoAwayReceived(
                  final int lastStreamId, final long errorCode, final ByteBuf debugData) {
  ...
  public ChannelFuture writeHeaders(
    ...
    pendingStream.frames.add(
        new HeadersFrame(
            headers, streamDependency, weight, exclusive, padding, endOfStream, promise));
    return promise;
  }
  ...
  public ChannelFuture writeRstStream(
    ...
    final PendingStream stream = pendingStreams.remove(streamId);
    if (stream != null) {
      ...
      promise.setFailure(
          connectionError(Http2Error.PROTOCOL_ERROR, "Stream does not exist %d", streamId));
    }
    return promise;
  }
  ...
  public ChannelFuture writeData(
    ...
    final PendingStream pendingStream = pendingStreams.get(streamId);
    if (pendingStream != null) {
    ...
    try {
      if (!closed) {
        ...
        while (!pendingStreams.isEmpty()) {
          final PendingStream stream = pendingStreams.pollFirstEntry().getValue();
          stream.close(e);
        }
      }
    } finally {
  ...
  private void tryCreatePendingStreams() {
    while (!pendingStreams.isEmpty() && canCreateStream()) {
      final Map.Entry<Integer, PendingStream> entry = pendingStreams.pollFirstEntry();
  ...
  private void cancelGoAwayStreams(
      final int lastStreamId, final long errorCode, final ByteBuf debugData) {
    final Iterator<PendingStream> iter = pendingStreams.values().iterator();
    final Exception e =
        new Http2GoAwayException(lastStreamId, errorCode, ByteBufUtil.getBytes(debugData));
    while (iter.hasNext()) {
