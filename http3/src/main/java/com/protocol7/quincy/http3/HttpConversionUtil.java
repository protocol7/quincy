...
public final class HttpConversionUtil {
  /**
   * The set of headers that should not be directly copied when converting headers from HTTP to
   * HTTP/2.
   */
  ...
  /**
   * This will be the method used for {@link HttpRequest} objects generated out of the HTTP message
   * flow defined in <a
   * href="http://tools.ietf.org/html/draft-ietf-httpbis-http2-16#section-8.1.">HTTP/2 Spec Message
   * Flow</a>
   */
  public static final HttpMethod OUT_OF_MESSAGE_SEQUENCE_METHOD = HttpMethod.OPTIONS;

  /**
   * This will be the path used for {@link HttpRequest} objects generated out of the HTTP message
   * flow defined in <a
   * href="http://tools.ietf.org/html/draft-ietf-httpbis-http2-16#section-8.1.">HTTP/2 Spec Message
   * Flow</a>
   */
  ...
  /**
   * This will be the status code used for {@link HttpResponse} objects generated out of the HTTP
   * message flow defined in <a
   * href="http://tools.ietf.org/html/draft-ietf-httpbis-http2-16#section-8.1.">HTTP/2 Spec Message
   * Flow</a>
   */
  ...
  /**
   * <a href="https://tools.ietf.org/html/rfc7540#section-8.1.2.3">rfc7540, 8.1.2.3</a> states the
   * path must not be empty, and instead should be {@code /}.
   */
  ...
  public enum ExtensionHeaderNames {
    /**
     * HTTP extension header which will identify the stream id from the HTTP/2 event(s) responsible
     * for generating a {@code HttpObject}
     *
     * <p>{@code "x-http2-stream-id"}
     */
    STREAM_ID("x-http2-stream-id"),
    /**
     * HTTP extension header which will identify the scheme pseudo header from the HTTP/2 event(s)
     * responsible for generating a {@code HttpObject}
     *
     * <p>{@code "x-http2-scheme"}
     */
     ...
     *
     * <p>{@code "x-http2-stream-promise-id"}
     */
     ...
     *
     * <p>{@code "x-http2-stream-dependency-id"}
     */
    STREAM_DEPENDENCY_ID("x-http2-stream-dependency-id"),
    /**
     * HTTP extension header which will identify the weight (if non-default and the priority is not
     * on the default stream) of the associated HTTP/2 stream responsible responsible for generating
     * a {@code HttpObject}
     *
     * <p>{@code "x-http2-stream-weight"}
     */
    STREAM_WEIGHT("x-http2-stream-weight");

    private final AsciiString text;

    ExtensionHeaderNames(final String text) {
  ...
  public static HttpResponseStatus parseStatus(final CharSequence status) throws Http2Exception {
    final HttpResponseStatus result;
    try {
   ...
   * @throws Http2Exception see {@link #addHttp2ToHttpHeaders(int, Http2Headers, FullHttpMessage,
   *     boolean)}
   */
  public static FullHttpResponse toFullHttpResponse(
      final int streamId,
    ...
    final FullHttpResponse msg =
        new DefaultFullHttpResponse(
            HttpVersion.HTTP_1_1, status, alloc.buffer(), validateHttpHeaders);
    try {
   ...
   *     boolean)}
   */
  public static FullHttpRequest toFullHttpRequest(
      final int streamId,
      final Http2Headers http2Headers,
      final ByteBufAllocator alloc,
      final boolean validateHttpHeaders)
      throws Http2Exception {
    // HTTP/2 does not define a way to carry the version identifier that is included in the HTTP/1.1
    // request line.
    final CharSequence method =
        checkNotNull(
            http2Headers.method(), "method header cannot be null in conversion to HTTP/1.x");
    final CharSequence path =
        checkNotNull(http2Headers.path(), "path header cannot be null in conversion to HTTP/1.x");
  ...
  public static HttpRequest toHttpRequest(
      final int streamId, final Http2Headers http2Headers, final boolean validateHttpHeaders)
      throws Http2Exception {
    // HTTP/2 does not define a way to carry the version identifier that is included in the HTTP/1.1
    // request line.
    final CharSequence method =
    ...
    try {
      addHttp2ToHttpHeaders(
          streamId, http2Headers, msg.headers(), msg.protocolVersion(), false, true);
    } catch (final Http2Exception e) {
      throw e;
    } catch (final Throwable t) {
      throw streamError(streamId, PROTOCOL_ERROR, t, "HTTP/2 to HTTP/1.x headers conversion error");
   ...
   * @throws Http2Exception see {@link #addHttp2ToHttpHeaders(int, Http2Headers, HttpHeaders,
  ...
  public static HttpResponse toHttpResponse(
    ...
    final HttpResponse msg =
        new DefaultHttpResponse(HttpVersion.HTTP_1_1, status, validateHttpHeaders);
    try {
      addHttp2ToHttpHeaders(
          streamId, http2Headers, msg.headers(), msg.protocolVersion(), false, true);
    } catch (final Http2Exception e) {
  ...
  public static void addHttp2ToHttpHeaders(
      final int streamId,
      final Http2Headers sourceHeaders,
      final FullHttpMessage destinationMessage,
      final boolean addToTrailer)
      throws Http2Exception {
    addHttp2ToHttpHeaders(
        streamId,
        sourceHeaders,
   ...
   * @param httpVersion What HTTP/1.x version {@code outputHeaders} should be treated as when doing
   *     the conversion.
  ...
  public static void addHttp2ToHttpHeaders(
      final int streamId,
      final Http2Headers inputHeaders,
      final HttpHeaders outputHeaders,
      final HttpVersion httpVersion,
      final boolean isTrailer,
      final boolean isRequest)
      throws Http2Exception {
    final Http2ToHttpHeaderTranslator translator =
        new Http2ToHttpHeaderTranslator(streamId, outputHeaders, isRequest);
    try {
      for (final Entry<CharSequence, CharSequence> entry : inputHeaders) {
  ...
  /**
   * Converts the given HTTP/1.x headers into HTTP/2 headers. The following headers are only used if
   * they can not be found in from the {@code HOST} header or the {@code Request-Line} as defined by
   * <a href="https://tools.ietf.org/html/rfc7230">rfc7230</a>
   *
   * <ul>
   *   <li>{@link ExtensionHeaderNames#SCHEME}
   * </ul>
   *
   * {@link ExtensionHeaderNames#PATH} is ignored and instead extracted from the {@code
   * Request-Line}.
   */
  ...
  public static Http2Headers toHttp2Headers(
      final HttpHeaders inHeaders, final boolean validateHeaders) {
  ...
  private static CharSequenceMap<AsciiString> toLowercaseMap(
      final Iterator<? extends CharSequence> valuesIter, final int arraySizeHint) {
    final UnsupportedValueConverter<AsciiString> valueConverter =
        UnsupportedValueConverter.<AsciiString>instance();
    final CharSequenceMap<AsciiString> result =
        new CharSequenceMap<AsciiString>(true, valueConverter, arraySizeHint);
        ...
        if (index != -1) {
          ...
          do {
            ...
            start = index + 1;
              ...
              && (index = lowerCased.forEachByte(start, lowerCased.length() - start, FIND_COMMA))
                  != -1);
          result.add(
              lowerCased.subSequence(start, lowerCased.length(), false).trim(), EMPTY_STRING);
        } else {
  ...
  /**
   * Filter the {@link HttpHeaderNames#TE} header according to the <a
   * href="https://tools.ietf.org/html/rfc7540#section-8.1.2.2">special rules in the HTTP/2 RFC</a>.
   *
   * @param entry An entry whose name is {@link HttpHeaderNames#TE}.
   * @param out the resulting HTTP/2 headers.
   */
  private static void toHttp2HeadersFilterTE(
      final Entry<CharSequence, CharSequence> entry, final Http2Headers out) {
    if (indexOf(entry.getValue(), ',', 0) == -1) {
      if (contentEqualsIgnoreCase(trim(entry.getValue()), TRAILERS)) {
        out.add(TE, TRAILERS);
      }
    } else {
      final List<CharSequence> teValues = unescapeCsvFields(entry.getValue());
  ...
  public static void toHttp2Headers(final HttpHeaders inHeaders, final Http2Headers out) {
    ...
    // Choose 8 as a default size because it is unlikely we will see more than 4 Connection headers
    // values, but
    // still allowing for "enough" space in the map to reduce the chance of hash code collision.
    final CharSequenceMap<AsciiString> connectionBlacklist =
            ...
            if (index != -1) {
              ...
              if (start >= value.length()) {
                throw new IllegalArgumentException(
                    "cookie value is of unexpected format: " + value);
              }
  ...
  /**
   * Generate a HTTP/2 {code :path} from a URI in accordance with <a
   * href="https://tools.ietf.org/html/rfc7230#section-5.3">rfc7230, 5.3</a>.
   */
  private static AsciiString toHttp2Path(final URI uri) {
    final StringBuilder pathBuilder =
        new StringBuilder(
            length(uri.getRawPath())
                + length(uri.getRawQuery())
                + length(uri.getRawFragment())
                + 2);
  ...
  private static final class Http2ToHttpHeaderTranslator {
    ...
    static {
      RESPONSE_HEADER_TRANSLATIONS.add(
          Http2Headers.PseudoHeaderName.AUTHORITY.value(), HttpHeaderNames.HOST);
      RESPONSE_HEADER_TRANSLATIONS.add(
          Http2Headers.PseudoHeaderName.SCHEME.value(), ExtensionHeaderNames.SCHEME.text());
      REQUEST_HEADER_TRANSLATIONS.add(RESPONSE_HEADER_TRANSLATIONS);
      RESPONSE_HEADER_TRANSLATIONS.add(
          Http2Headers.PseudoHeaderName.PATH.value(), ExtensionHeaderNames.PATH.text());
    }
    ...
    Http2ToHttpHeaderTranslator(
        final int streamId, final HttpHeaders output, final boolean request) {
      this.streamId = streamId;
      this.output = output;
      translations = request ? REQUEST_HEADER_TRANSLATIONS : RESPONSE_HEADER_TRANSLATIONS;
    }

    public void translate(final Entry<CharSequence, CharSequence> entry) throws Http2Exception {
      ...
      if (translatedName != null) {
        ...
        if (name.length() == 0 || name.charAt(0) == ':') {
          throw streamError(
              streamId,
              PROTOCOL_ERROR,
              "Invalid HTTP/2 header '%s' encountered in translation to HTTP/1.x",
              name);
        }
        if (COOKIE.equals(name)) {
          // combine the cookie values into 1 header entry.
          // https://tools.ietf.org/html/rfc7540#section-8.1.2.5
          final String existingCookie = output.get(COOKIE);
          output.set(COOKIE, (existingCookie != null) ? (existingCookie + "; " + value) : value);
        } else {
