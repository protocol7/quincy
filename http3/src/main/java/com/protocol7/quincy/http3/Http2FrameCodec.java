...
import static com.protocol7.quincy.http3.Http2CodecUtil.HTTP_UPGRADE_STREAM_ID;
...
/**
 * <em>This API is very immature.</em> The Http2Connection-based API is currently preferred over
 * this API. This API is targeted to eventually replace or reduce the need for the {@link
 * Http2ConnectionHandler} API.
 *
 * <p>A HTTP/2 handler that maps HTTP/2 frames to {@link Http2Frame} objects and vice versa. For
 * every incoming HTTP/2 frame, a {@link Http2Frame} object is created and propagated via {@link
 * #channelRead}. Outbound {@link Http2Frame} objects received via {@link #write} are converted to
 * the HTTP/2 wire format. HTTP/2 frames specific to a stream implement the {@link Http2StreamFrame}
 * interface. The {@link Http2FrameCodec} is instantiated using the {@link Http2FrameCodecBuilder}.
 * It's recommended for channel handlers to inherit from the {@link Http2ChannelDuplexHandler}, as
 * it provides additional functionality like iterating over all active streams or creating outbound
 * streams.
 *
 * <h3>Stream Lifecycle</h3>
 *
 * The frame codec delivers and writes frames for active streams. An active stream is closed when
 * either side sends a {@code RST_STREAM} frame or both sides send a frame with the {@code
 * END_STREAM} flag set. Each {@link Http2StreamFrame} has a {@link Http2FrameStream} object
 * attached that uniquely identifies a particular stream.
 *
 * <p>{@link Http2StreamFrame}s read from the channel always a {@link Http2FrameStream} object set,
 * while when writing a {@link Http2StreamFrame} the application code needs to set a {@link
 * Http2FrameStream} object using {@link Http2StreamFrame#stream(Http2FrameStream)}.
 *
 * <h3>Flow control</h3>
 *
 * The frame codec automatically increments stream and connection flow control windows.
 *
 * <p>Incoming flow controlled frames need to be consumed by writing a {@link
 * Http2WindowUpdateFrame} with the consumed number of bytes and the corresponding stream identifier
 * set to the frame codec.
 *
 * <p>The local stream-level flow control window can be changed by writing a {@link
 * Http2SettingsFrame} with the {@link Http2Settings#initialWindowSize()} set to the targeted value.
 *
 * <p>The connection-level flow control window can be changed by writing a {@link
 * Http2WindowUpdateFrame} with the desired window size <em>increment</em> in bytes and the stream
 * identifier set to {@code 0}. By default the initial connection-level flow control window is the
 * same as initial stream-level flow control window.
 *
 * <h3>New inbound Streams</h3>
 *
 * The first frame of a HTTP/2 stream must be a {@link Http2HeadersFrame}, which will have a {@link
 * Http2FrameStream} object attached.
 *
 * <h3>New outbound Streams</h3>
 *
 * A outbound HTTP/2 stream can be created by first instantiating a new {@link Http2FrameStream}
 * object via {@link Http2ChannelDuplexHandler#newStream()}, and then writing a {@link
 * Http2HeadersFrame} object with the stream attached.
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>If a new stream cannot be created due to stream id exhaustion of the endpoint, the {@link
 * ChannelPromise} of the HEADERS frame will fail with a {@link Http2NoMoreStreamIdsException}.
 *
 * <p>The HTTP/2 standard allows for an endpoint to limit the maximum number of concurrently active
 * streams via the {@code SETTINGS_MAX_CONCURRENT_STREAMS} setting. When this limit is reached, no
 * new streams can be created. However, the {@link Http2FrameCodec} can be build with {@link
 * Http2FrameCodecBuilder#encoderEnforceMaxConcurrentStreams(boolean)} enabled, in which case a new
 * stream and its associated frames will be buffered until either the limit is increased or an
 * active stream is closed. It's, however, possible that a buffered stream will never become active.
 * That is, the channel might get closed or a GO_AWAY frame might be received. In the first case,
 * all writes of buffered streams will fail with a {@link
 * StreamBufferingEncoder.Http2ChannelClosedException}. In the second case, all writes of buffered
 * streams with an identifier less than the last stream identifier of the GO_AWAY frame will fail
 * with a {@link StreamBufferingEncoder.Http2GoAwayException}.
 *
 * <h3>Error Handling</h3>
 *
 * Exceptions and errors are propagated via {@link ChannelInboundHandler#exceptionCaught}.
 * Exceptions that apply to a specific HTTP/2 stream are wrapped in a {@link
 * Http2FrameStreamException} and have the corresponding {@link Http2FrameStream} object attached.
 *
 * <h3>Reference Counting</h3>
 *
 * Some {@link Http2StreamFrame}s implement the {@link ReferenceCounted} interface, as they carry
 * reference counted objects (e.g. {@link ByteBuf}s). The frame codec will call {@link
 * ReferenceCounted#retain()} before propagating a reference counted object through the pipeline,
 * and thus an application handler needs to release such an object after having consumed it. For
 * more information on reference counting take a look at
 * http://netty.io/wiki/reference-counted-objects.html
 *
 * <h3>HTTP Upgrade</h3>
 *
 * Server-side HTTP to HTTP/2 upgrade is supported in conjunction with {@link
...
public class Http2FrameCodec extends Http2ConnectionHandler {
  ...
  private static final InternalLogger LOG =
      InternalLoggerFactory.getInstance(Http2FrameCodec.class);
  ...
  private void tryExpandConnectionFlowControlWindow(final Http2Connection connection)
      throws Http2Exception {
    if (initialFlowControlWindowSize != null) {
      // The window size in the settings explicitly excludes the connection window. So we manually
      // manipulate the
      ...
      final int delta =
          initialFlowControlWindowSize - localFlowController.initialWindowSize(connectionStream);
  ...
  void handlerAdded0(@SuppressWarnings("unsed") final ChannelHandlerContext ctx) throws Exception {
  ...
  public final void userEventTriggered(final ChannelHandlerContext ctx, final Object evt)
    ...
    if (evt == Http2ConnectionPrefaceAndSettingsFrameWrittenEvent.INSTANCE) {
      ...
      final UpgradeEvent upgrade = (UpgradeEvent) evt;
      try {
        onUpgradeEvent(ctx, upgrade.retain());
        final Http2Stream stream = connection().stream(HTTP_UPGRADE_STREAM_ID);
        if (stream.getProperty(streamKey) == null) {
          // TODO: improve handler/stream lifecycle so that stream isn't active before handler
          // added.
          // The stream was already made active, but ctx may have been null so it wasn't
          // initialized.
  ...
  public void write(
    ...
    if (msg instanceof Http2DataFrame) {
      ...
      // It is legit to send a WINDOW_UPDATE frame for the connection stream. The parent channel
      // doesn't attempt
      // to set the Http2FrameStream so we assume if it is null the WINDOW_UPDATE is for the
      // connection stream.
      try {
        if (frameStream == null) {
      ...
      final Http2PingFrame frame = (Http2PingFrame) msg;
      ...
      // In the event of manual SETTINGS ACK is is assumed the encoder will apply the earliest
      // received but not
      // yet ACKed settings.
  ...
  private void increaseInitialConnectionWindow(final int deltaBytes) throws Http2Exception {
    ...
    connection()
        .local()
        .flowController()
        .incrementWindowSize(connection().connectionStream(), deltaBytes);
  }
  ...
  final boolean consumeBytes(final int streamId, final int bytes) throws Http2Exception {
    ...
    if (stream != null && streamId == Http2CodecUtil.HTTP_UPGRADE_STREAM_ID) {
      final Boolean upgraded = stream.getProperty(upgradeKey);
  ...
  private void writeGoAwayFrame(
      final ChannelHandlerContext ctx, final Http2GoAwayFrame frame, final ChannelPromise promise) {
    if (frame.lastStreamId() > -1) {
      frame.release();
      throw new IllegalArgumentException("Last stream id must not be set on GOAWAY frame");
    }

    final int lastStreamCreated = connection().remote().lastStreamCreated();
  ...
  private void writeHeadersFrame(
    ...
    if (isStreamIdValid(headersFrame.stream().id())) {
      ...
      if (streamId < 0) {
        ...
        // Simulate a GOAWAY being received due to stream exhaustion on this connection. We use the
        // maximum
        // valid stream ID for the current peer.
        onHttp2Frame(
            ctx,
            new DefaultHttp2GoAwayFrame(
                connection.isServer() ? Integer.MAX_VALUE : Integer.MAX_VALUE - 1,
      ...
      // Use a Map to store all pending streams as we may have multiple. This is needed as if we
      // would store the
      // stream in a field directly we may override the stored field before onStreamAdded(...) was
      // called
      // and so not correctly set the property for the buffered stream.
      //
      // See https://github.com/netty/netty/issues/8692
      final Object old = frameStreamToInitializeMap.put(streamId, stream);
  ...
  private static void notifyHeaderWritePromise(
      final ChannelFuture future, final ChannelPromise promise) {
  ...
  private void onStreamActive0(final Http2Stream stream) {
    ...
    final Http2FrameStream stream2 = newStream().setStreamAndProperty(streamKey, stream);
  ...
  private final class ConnectionListener extends Http2ConnectionAdapter {
    ...
    public void onStreamAdded(final Http2Stream stream) {
      final DefaultHttp2FrameStream frameStream = frameStreamToInitializeMap.remove(stream.id());
    ...
    public void onStreamClosed(final Http2Stream stream) {
      final Http2FrameStream stream2 = stream.getProperty(streamKey);
      if (stream2 != null) {
        onHttp2StreamStateChanged(ctx, stream2);
      }
    }
  ...
  protected void onConnectionError(
    ...
    if (!outbound) {
      ...
      //
      // We only forward non outbound errors as outbound errors will already be reflected by failing
      // the promise.
  ...
  /**
   * Exceptions for unknown streams, that is streams that have no {@link Http2FrameStream} object
   * attached are simply logged and replied to by sending a RST_STREAM frame.
   */
  @Override
  protected final void onStreamError(
      ...
      final boolean outbound,
      final Throwable cause,
      final Http2Exception.StreamException streamException) {
    final int streamId = streamException.streamId();
    final Http2Stream connectionStream = connection().stream(streamId);
    ...
    final Http2FrameStream stream = connectionStream.getProperty(streamKey);
    if (stream == null) {
    ...
    if (!outbound) {
      ...
      onHttp2FrameStreamException(
          ctx, new Http2FrameStreamException(stream, streamException.error(), cause));
    }
  }

  void onHttp2UnknownStreamError(
      @SuppressWarnings("unused") final ChannelHandlerContext ctx,
      final Throwable cause,
      final Http2Exception.StreamException streamException) {
    // Just log....
  ...
  private final class FrameListener implements Http2FrameListener {
    ...
    public void onUnknownFrame(
      ...
      onHttp2Frame(
          ctx,
          new DefaultHttp2UnknownFrame(frameType, flags, payload)
    ...
    public void onWindowUpdateRead(
      ...
      onHttp2Frame(
          ctx,
          new DefaultHttp2WindowUpdateFrame(windowSizeIncrement).stream(requireStream(streamId)));
    }
    ...
    public int onDataRead(
      ...
      onHttp2Frame(
          ctx,
          new DefaultHttp2DataFrame(data, endOfStream, padding)
              .stream(requireStream(streamId))
              .retain());
    ...
    private Http2FrameStream requireStream(final int streamId) {
      final Http2FrameStream stream = connection().stream(streamId).getProperty(streamKey);
      if (stream == null) {
  ...
  void onUpgradeEvent(final ChannelHandlerContext ctx, final UpgradeEvent evt) {
    ctx.fireUserEventTriggered(evt);
  }

  void onHttp2StreamWritabilityChanged(
      final ChannelHandlerContext ctx,
      final Http2FrameStream stream,
      @SuppressWarnings("unused") final boolean writable) {
  ...
  void onHttp2FrameStreamException(
      final ChannelHandlerContext ctx, final Http2FrameStreamException cause) {
    ctx.fireExceptionCaught(cause);
  }
  ...
  private final class Http2RemoteFlowControllerListener
    ...
    public void writabilityChanged(final Http2Stream stream) {
      final Http2FrameStream frameStream = stream.getProperty(streamKey);
      if (frameStream == null) {
        return;
      }
  ...
  static class DefaultHttp2FrameStream implements Http2FrameStream {
    ...
    DefaultHttp2FrameStream setStreamAndProperty(
        final Http2Connection.PropertyKey streamKey, final Http2Stream stream) {
