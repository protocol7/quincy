...
import static com.protocol7.quincy.http3.Http2CodecUtil.HTTP_UPGRADE_PROTOCOL_NAME;
...
public class Http2ClientUpgradeCodec implements HttpClientUpgradeHandler.UpgradeCodec {
  ...
  public Http2ClientUpgradeCodec(
      final Http2FrameCodec frameCodec, final ChannelHandler upgradeToHandler) {
  ...
  public Http2ClientUpgradeCodec(
      ...
      final ChannelHandler upgradeToHandler) {
  ...
  private Http2ClientUpgradeCodec(
      final String handlerName,
  ...
  @Override
  public Collection<CharSequence> setUpgradeHeaders(
      final ChannelHandlerContext ctx, final HttpRequest upgradeRequest) {
    final CharSequence settingsValue = getSettingsHeaderValue(ctx);
    upgradeRequest.headers().set(HTTP_UPGRADE_SETTINGS_HEADER, settingsValue);
    return UPGRADE_HEADERS;
  }

  @Override
  public void upgradeTo(final ChannelHandlerContext ctx, final FullHttpResponse upgradeResponse)
      throws Exception {
  ...
  /**
   * Converts the current settings for the handler to the Base64-encoded representation used in the
  ...
  private CharSequence getSettingsHeaderValue(final ChannelHandlerContext ctx) {
    ...
    try {
      ...
      final Http2Settings settings = connectionHandler.decoder().localSettings();
