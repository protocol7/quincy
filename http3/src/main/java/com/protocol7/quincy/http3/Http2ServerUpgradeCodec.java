...
import static com.protocol7.quincy.http3.Http2CodecUtil.HTTP_UPGRADE_SETTINGS_HEADER;
import static com.protocol7.quincy.http3.Http2CodecUtil.writeFrameHeader;
...
public class Http2ServerUpgradeCodec implements HttpServerUpgradeHandler.UpgradeCodec {
  ...
  private static final InternalLogger logger =
      InternalLoggerFactory.getInstance(Http2ServerUpgradeCodec.class);
   ...
   *
   * @param handlerName the name of the HTTP/2 connection handler to be used in the pipeline, or
   *     {@code null} to auto-generate the name
   * @param connectionHandler the HTTP/2 connection handler
   */
  public Http2ServerUpgradeCodec(
      final String handlerName, final Http2ConnectionHandler connectionHandler) {
   ...
   */
  public Http2ServerUpgradeCodec(
      final Http2FrameCodec http2Codec, final ChannelHandler... handlers) {
    this(null, http2Codec, handlers);
  }

  private Http2ServerUpgradeCodec(
  ...
  public boolean prepareUpgradeResponse(
      ...
      final FullHttpRequest upgradeRequest,
    ...
    try {
      ...
      final List<String> upgradeHeaders =
          upgradeRequest.headers().getAll(HTTP_UPGRADE_SETTINGS_HEADER);
  ...
  private Http2Settings decodeSettingsHeader(
      final ChannelHandlerContext ctx, final CharSequence settingsHeader) throws Http2Exception {
    final ByteBuf header =
        ByteBufUtil.encodeString(ctx.alloc(), CharBuffer.wrap(settingsHeader), CharsetUtil.UTF_8);
    try {
  ...
  private Http2Settings decodeSettings(final ChannelHandlerContext ctx, final ByteBuf frame)
  ...
  private static ByteBuf createSettingsFrame(
      final ChannelHandlerContext ctx, final ByteBuf payload) {
