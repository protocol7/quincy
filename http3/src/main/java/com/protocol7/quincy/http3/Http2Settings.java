...
import static com.protocol7.quincy.http3.Http2CodecUtil.MAX_CONCURRENT_STREAMS;
...
import static com.protocol7.quincy.http3.Http2CodecUtil.MAX_INITIAL_WINDOW_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.MIN_CONCURRENT_STREAMS;
...
import static com.protocol7.quincy.http3.Http2CodecUtil.MIN_INITIAL_WINDOW_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.NUM_STANDARD_SETTINGS;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_ENABLE_PUSH;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_HEADER_TABLE_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_INITIAL_WINDOW_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_MAX_CONCURRENT_STREAMS;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_MAX_FRAME_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_MAX_HEADER_LIST_SIZE;
...
public final class Http2Settings extends CharObjectHashMap<Long> {
  /**
   * Default capacity based on the number of standard settings from the HTTP/2 spec, adjusted so
   * that adding all of the standard settings will not cause the map capacity to change.
   */
  private static final int DEFAULT_CAPACITY =
      (int) (NUM_STANDARD_SETTINGS / DEFAULT_LOAD_FACTOR) + 1;
  ...
  /**
   * Adds the given setting key/value pair. For standard settings defined by the HTTP/2 spec,
   * performs validation on the values.
   *
  ...
  public Boolean pushEnabled() {
    final Long value = get(SETTINGS_ENABLE_PUSH);
    if (value == null) {
      return null;
    }
  ...
  /**
   * A helper method that returns {@link Long#intValue()} on the return of {@link #get(char)}, if
   * present. Note that if the range of the value exceeds {@link Integer#MAX_VALUE}, the {@link
   * #get(char)} method should be used instead to avoid truncation of the value.
   */
