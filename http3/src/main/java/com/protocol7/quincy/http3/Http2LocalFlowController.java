...
/**
 * A {@link Http2FlowController} for controlling the inbound flow of {@code DATA} frames from the
 * remote endpoint.
 */
@UnstableApi
public interface Http2LocalFlowController extends Http2FlowController {
  /**
   * Sets the writer to be use for sending {@code WINDOW_UPDATE} frames. This must be called before
   * any flow controlled data is received.
   *
   ...
   *
   * <p>If {@code stream} is {@code null} or closed, flow control should only be applied to the
   * connection window and the bytes are immediately consumed.
   *
   * @param stream the subject stream for the received frame. The connection stream object must not
   *     be used. If {@code stream} is {@code null} or closed, flow control should only be applied
   *     to the connection window and the bytes are immediately consumed.
   ...
   */
  void receiveFlowControlledFrame(
   ...
   * therefore ready to receive more data from the remote endpoint. The application must consume any
   * bytes that it receives or the flow control window will collapse. Consuming bytes enables the
   * flow controller to send {@code WINDOW_UPDATE} to restore a portion of the flow control window
   * for the stream.
   *
   * <p>If {@code stream} is {@code null} or closed (i.e. {@link Http2Stream#state()} method returns
   * {@link Http2Stream.State#CLOSED}), calling this method has no effect.
   *
   * @param stream the stream for which window space should be freed. The connection stream object
   *     must not be used. If {@code stream} is {@code null} or closed (i.e. {@link
   *     Http2Stream#state()} method returns {@link Http2Stream.State#CLOSED}), calling this method
   *     has no effect.
   ...
   * @throws Http2Exception if the number of bytes returned exceeds the {@link
   *     #unconsumedBytes(Http2Stream)} for the stream.
   */
  ...
  /**
   * Get the initial flow control window size for the given stream. This quantity is measured in
   * number of bytes. Note the unavailable window portion can be calculated by {@link
   * #initialWindowSize()} - {@link #windowSize(Http2Stream)}.
   */
