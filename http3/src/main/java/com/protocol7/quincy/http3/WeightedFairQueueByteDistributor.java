...
import static com.protocol7.quincy.http3.Http2CodecUtil.CONNECTION_STREAM_ID;
import static com.protocol7.quincy.http3.Http2CodecUtil.DEFAULT_MIN_ALLOCATION_CHUNK;
import static com.protocol7.quincy.http3.Http2CodecUtil.DEFAULT_PRIORITY_WEIGHT;
import static com.protocol7.quincy.http3.Http2CodecUtil.streamableBytes;
...
/**
 * A {@link StreamByteDistributor} that is sensitive to stream priority and uses <a
 * href="https://en.wikipedia.org/wiki/Weighted_fair_queueing">Weighted Fair Queueing</a> approach
 * for distributing bytes.
 *
 * <p>Inspiration for this distributor was taken from Linux's <a
 * href="https://www.kernel.org/doc/Documentation/scheduler/sched-design-CFS.txt">Completely Fair
 * Scheduler</a> to model the distribution of bytes to simulate an "ideal multi-tasking CPU", but in
 * this case we are simulating an "ideal multi-tasking NIC".
 *
 * <p>Each write operation will use the {@link #allocationQuantum(int)} to know how many more bytes
 * should be allocated relative to the next stream which wants to write. This is to balance fairness
 * while also considering goodput.
 */
@UnstableApi
public final class WeightedFairQueueByteDistributor implements StreamByteDistributor {
  /**
   * The initial size of the children map is chosen to be conservative on initial memory allocations
   * under the assumption that most streams will have a small number of children. This choice may be
   * sub-optimal if when children are present there are many children (i.e. a web page which has
   * many dependencies to load).
   *
   * <p>Visible only for testing!
   */
   ...
   * the state will reside.
   */
  private final IntObjectMap<State> stateOnlyMap;
  /**
   * This queue will hold streams that are not active and provides the capability to retain priority
   * for streams which have no {@link Http2Stream} object. See {@link StateOnlyComparator} for the
   * priority comparator.
   */
  ...
  /**
   * The minimum number of bytes that we will attempt to allocate to a stream. This is to help
   * improve goodput on a per-stream basis.
   */
  ...
  public WeightedFairQueueByteDistributor(
    ...
    if (maxStateOnlySize == 0) {
      ...
      // +2 because we may exceed the limit by 2 if a new dependency has no associated Http2Stream
      // object. We need
      // to create the State objects to put them into the dependency tree, which then impacts
      // priority.
      stateOnlyRemovalQueue =
          new DefaultPriorityQueue<State>(StateOnlyComparator.INSTANCE, maxStateOnlySize + 2);
    }
        ...
        new Http2ConnectionAdapter() {
          ...
          public void onStreamAdded(final Http2Stream stream) {
            ...
            if (state == null) {
              ...
              // Only the stream which was just added will change parents. So we only need an array
              // of size 1.
            ...
            switch (stream.state()) {
              ...
              case RESERVED_LOCAL:
                ...
                // wasStreamReservedOrActivated is part of the comparator for stateOnlyRemovalQueue
                // there is no
          ...
          public void onStreamRemoved(final Http2Stream stream) {
            // The stream has been removed from the connection. We can no longer rely on the
            // stream's property
            // storage to track the State. If we have room, and the precedence of the stream is
            // sufficient, we
            // should retain the State in the stateOnlyMap.
            final State state = state(stream);

            // Typically the stream is set to null when the stream is closed because it is no longer
            // needed to write
            // data. However if the stream was not activated it may not be closed (reserved streams)
            // so we ensure
            // the stream reference is set to null to avoid retaining a reference longer than
            // necessary.
            state.stream = null;
            ...
            if (stateOnlyRemovalQueue.size()
                == WeightedFairQueueByteDistributor.this.maxStateOnlySize) {
              final State stateToRemove = stateOnlyRemovalQueue.peek();
              if (StateOnlyComparator.INSTANCE.compare(stateToRemove, state) >= 0) {
                // The "lowest priority" stream is a "higher priority" than the stream being
                // removed, so we
  ...
  public void updateStreamableBytes(final StreamState state) {
    state(state.stream())
        .updateStreamableBytes(streamableBytes(state), state.hasFrame() && state.windowSize() >= 0);
  }
  ...
  public void updateDependencyTree(
    ...
    if (state == null) {
      // If there is no State object that means there is no Http2Stream object and we would have to
      // keep the
      // State object in the stateOnlyMap and stateOnlyRemovalQueue. However if maxStateOnlySize is
      // 0 this means
      // stateOnlyMap and stateOnlyRemovalQueue are empty collections and cannot be modified so we
      // drop the State.
      if (maxStateOnlySize == 0) {
        return;
      }
    ...
    if (newParent == null) {
      ...
      // Only the stream which was just added will change parents. So we only need an array of size
      // 1.
      final List<ParentChangedEvent> events = new ArrayList<ParentChangedEvent>(1);
    ...
    // if activeCountForTree == 0 then it will not be in its parent's pseudoTimeQueue and thus
    ...
    // The location in the dependency tree impacts the priority in the stateOnlyRemovalQueue map. If
    // we created new
    // State objects we must check if we exceeded the limit after we insert into the dependency tree
    // to ensure the
    // stateOnlyRemovalQueue has been updated.
    while (stateOnlyRemovalQueue.size() > maxStateOnlySize) {
      final State stateToRemove = stateOnlyRemovalQueue.poll();
      stateToRemove.parent.removeChild(stateToRemove);
      stateOnlyMap.remove(stateToRemove.streamId);
    }
  }
  ...
  public boolean distribute(
    ...
    // We still attempt to write even after the number of allocated bytes has been exhausted to
    // allow empty frames
    // to be sent. Making progress means the active streams rooted at the connection stream has
    // changed.
   ...
   *
   * @param allocationQuantum the amount of bytes that will be allocated to each stream. Must be
   *     &gt; 0.
   */
  ...
  private int distribute(final int maxBytes, final Writer writer, final State state)
      throws Http2Exception {
    if (state.isActive()) {
      final int nsent = min(maxBytes, state.streamableBytes);
      state.write(nsent, writer);
      if (nsent == 0 && maxBytes != 0) {
        // If a stream sends zero bytes, then we gave it a chance to write empty frames and it is
        // now
        // considered inactive until the next call to updateStreamableBytes. This allows descendant
        // streams to
        // be allocated bytes when the parent stream can't utilize them. This may be as a result of
        // the
  ...
  /**
   * It is a pre-condition that {@code state.poll()} returns a non-{@code null} value. This is a
   * result of the way the allocation algorithm is structured and can be explained in the following
   * cases:
   *
   * <h3>For the recursive case</h3>
   *
   * If a stream has no children (in the allocation tree) than that node must be active or it will
   * not be in the allocation tree. If a node is active then it will not delegate to children and
   * recursion ends.
   *
   * <h3>For the initial case</h3>
   *
   * We check connectionState.activeCountForTree == 0 before any allocation is done. So if the
   * connection stream has no active children we don't get into this method.
   */
  private int distributeToChildren(final int maxBytes, final Writer writer, final State state)
      throws Http2Exception {
    ...
    try {
      assert nextChildState == null
              || nextChildState.pseudoTimeToWrite >= childState.pseudoTimeToWrite
          : "nextChildState["
      ...
      // The offer operation is delayed until we unroll up the recursive stack, so we don't have to
      // remove from
  ...
  boolean isChild(final int childId, final int parentId, final short weight) {
    final State parent = state(parentId);
    final State child;
    return parent.children.containsKey(childId)
        && (child = state(childId)).parent == parent
        && child.weight == weight;
  }

  /** For testing only! */
  int numChildren(final int streamId) {
    final State state = state(streamId);
  ...
  /**
   * A comparator for {@link State} which has no associated {@link Http2Stream} object. The general
   * precedence is:
   *
   * <ul>
   *   <li>Was a stream activated or reserved (streams only used for priority are higher priority)
   *   <li>Depth in the priority tree (closer to root is higher priority>
   *   <li>Stream ID (higher stream ID is higher priority - used for tie breaker)
   * </ul>
   */
  private static final class StateOnlyComparator implements Comparator<State>, Serializable {
    ...
    public int compare(final State o1, final State o2) {
      // "priority only streams" (which have not been activated) are higher priority than streams
      // used for data.
      ...
      // I also considered tracking the number of streams which are "activated" (eligible transfer
      // data) at each
      // subtree. This would require a traversal from each node to the root on dependency tree
      // structural changes,
      // and then it would require a re-prioritization at each of these nodes (instead of just the
      // nodes where the
      // direct parent changed). The costs of this are judged to be relatively high compared to the
      // nominal
  ...
  private final class State implements PriorityQueueNode {
    ...
    /**
     * A pseudo time maintained for immediate children to base their {@link #pseudoTimeToWrite} off
     * of.
     */
    long pseudoTime;
    ...
    State(final Http2Stream stream, final int initialSize) {
    ...
    State(final int streamId, final Http2Stream stream, final int initialSize) {
      ...
      pseudoTimeQueue =
          new DefaultPriorityQueue<State>(StatePseudoTimeComparator.INSTANCE, initialSize);
    }
    ...
    void takeChild(
        final State child, final boolean exclusive, final List<ParentChangedEvent> events) {
      takeChild(null, child, exclusive, events);
    }

    /**
     * Adds a child to this priority. If exclusive is set, any children of this node are moved to
     * being dependent on the child.
     */
    void takeChild(
        final Iterator<IntObjectMap.PrimitiveEntry<State>> childItr,
        final State child,
        final boolean exclusive,
        final List<ParentChangedEvent> events) {
      ...
      if (oldParent != this) {
        ...
        // If the childItr is not null we are iterating over the oldParent.children collection and
        // should
        // use the iterator to remove from the collection to avoid concurrent modification.
        // Otherwise it is
      ...
      if (exclusive && !children.isEmpty()) {
        ...
        final Iterator<IntObjectMap.PrimitiveEntry<State>> itr =
            removeAllChildrenExcept(child).entries().iterator();
        while (itr.hasNext()) {
     ...
     * be used to support an exclusive priority dependency operation.
     *
    ...
    private IntObjectMap<State> removeAllChildrenExcept(
      ...
      // added.
      // It will either be added directly in this method, or after this method is called...but it
      // will be added.
      initChildren();
    ...
    private void setParent(final State newParent) {
      ...
      parent = newParent;
      // Use MAX_VALUE if no parent because lower depth is considered higher priority by
      // StateOnlyComparator.
    ...
    void write(final int numBytes, final Writer writer) throws Http2Exception {
      ...
      try {
        ...
        throw connectionError(Http2Error.INTERNAL_ERROR, t, "byte distribution write error");
      }
    }
      ...
      if (parent != null) {
        assert activeCountForTree != increment
            ...
            : "State["
                ...
                + increment
                + " is in a "
                + "pseudoTimeQueue, but not in parent[ "
                + parent.streamId
                + "]'s pseudoTimeQueue";
        if (activeCountForTree == 0) {
          parent.removePseudoTimeQueue(this);
        } else if (activeCountForTree == increment && !isDistributing()) {
          // If frame count was 0 but is now not, and this node is not already in a pseudoTimeQueue
          // (assumed
          // to be pState's pseudoTimeQueue) then enqueue it. If this State object is being
          // processed the
          ...
          // activeCountForTree == 0 (a node which can't stream anything and is blocked) is at/near
          // root of
          // the tree, and is popped off the pseudoTimeQueue during processing, and then put back on
          // the
          // pseudoTimeQueue because a child changes position in the priority tree (or is closed
          // because it is
    ...
    void updatePseudoTime(final State parentState, final int nsent, final long totalQueuedWeights) {
      ...
      // If the current pseudoTimeToSend is greater than parentState.pseudoTime then we previously
      // over accounted
      // and should use parentState.pseudoTime.
      pseudoTimeToWrite =
          min(pseudoTimeToWrite, parentState.pseudoTime) + nsent * totalQueuedWeights / weight;
    }

    /**
     * The concept of pseudoTime can be influenced by priority tree manipulations or if a stream
     * goes from "active" to "non-active". This method accounts for that by initializing the {@link
     * #pseudoTimeToWrite} for {@code state} to {@link #pseudoTime} of this node and then calls
     * {@link #offerPseudoTimeQueue(State)}.
     */
    ...
    State pollPseudoTimeQueue() {
      final State state = pseudoTimeQueue.poll();
    ...
    public String toString() {
      ...
      final StringBuilder sb =
          new StringBuilder(256 * (activeCountForTree > 0 ? activeCountForTree : 1));
      toString(sb);
      return sb.toString();
    }
    ...
    private void toString(final StringBuilder sb) {
      sb.append("{streamId ")
          ...
          .append(parent == null ? -1 : parent.streamId)
          .append("} [");

      if (!pseudoTimeQueue.isEmpty()) {
        for (final State s : pseudoTimeQueue) {
          s.toString(sb);
          sb.append(", ");
        }
        // Remove the last ", "
  ...
  /**
   * Allows a correlation to be made between a stream and its old parent before a parent change
   * occurs.
   */
