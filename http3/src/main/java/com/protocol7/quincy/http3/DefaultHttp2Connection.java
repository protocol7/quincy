...
import static com.protocol7.quincy.http3.Http2CodecUtil.DEFAULT_MAX_RESERVED_STREAMS;
import static com.protocol7.quincy.http3.Http2Exception.closedStreamError;
import static com.protocol7.quincy.http3.Http2Exception.connectionError;
import static com.protocol7.quincy.http3.Http2Exception.streamError;
import static com.protocol7.quincy.http3.Http2Stream.State.CLOSED;
import static com.protocol7.quincy.http3.Http2Stream.State.HALF_CLOSED_LOCAL;
import static com.protocol7.quincy.http3.Http2Stream.State.HALF_CLOSED_REMOTE;
import static com.protocol7.quincy.http3.Http2Stream.State.IDLE;
import static com.protocol7.quincy.http3.Http2Stream.State.OPEN;
import static com.protocol7.quincy.http3.Http2Stream.State.RESERVED_LOCAL;
import static com.protocol7.quincy.http3.Http2Stream.State.RESERVED_REMOTE;
...
public class DefaultHttp2Connection implements Http2Connection {
  private static final InternalLogger logger =
      InternalLoggerFactory.getInstance(DefaultHttp2Connection.class);
  ...
  /**
   * We chose a {@link List} over a {@link Set} to avoid allocating an {@link Iterator} objects when
   * iterating over the listeners.
   *
   * <p>Initial size of 4 because the default configuration currently has 3 listeners (local/remote
   * flow controller and {@link StreamByteDistributor}) and we leave room for 1 extra. We could be
   * more aggressive but the ArrayList resize will double the size if we are too small.
   */
   ...
   *
   * @param server whether or not this end-point is the server-side of the HTTP/2 connection.
   * @param maxReservedStreams The maximum amount of streams which can exist in the reserved state
   *     for each endpoint.
   */
  public DefaultHttp2Connection(final boolean server, final int maxReservedStreams) {
    activeStreams = new ActiveStreams(listeners);
    // Reserved streams are excluded from the SETTINGS_MAX_CONCURRENT_STREAMS limit according to [1]
    // and the RFC
    // doesn't define a way to communicate the limit on reserved streams. We rely upon the peer to
    // send RST_STREAM
    // in response to any locally enforced limits being exceeded [2].
    // [1] https://tools.ietf.org/html/rfc7540#section-5.1.2
    // [2] https://tools.ietf.org/html/rfc7540#section-8.2.2
    localEndpoint =
        new DefaultEndpoint<Http2LocalFlowController>(
            server, server ? MAX_VALUE : maxReservedStreams);
  ...
  /**
   * Determine if {@link #close(Promise)} has been called and no more streams are allowed to be
   * created.
   */
  ...
  public Future<Void> close(final Promise<Void> promise) {
    ...
    // Since we allow this method to be called multiple times, we must make sure that all the
    // promises are notified
    ...
    final Iterator<PrimitiveEntry<Http2Stream>> itr = streamMap.entries().iterator();
    // We must take care while iterating the streamMap as to not modify while iterating in case
    ...
    if (activeStreams.allowModifications()) {
      ...
      try {
        while (itr.hasNext()) {
          ...
          if (stream.id() != CONNECTION_STREAM_ID) {
            // If modifications of the activeStream map is allowed, then a stream close operation
            // will also
      ...
      while (itr.hasNext()) {
        final Http2Stream stream = itr.next().value();
        if (stream.id() != CONNECTION_STREAM_ID) {
          // We are not allowed to make modifications, so the close calls will be executed after
          // this
          // iteration completes.
          stream.close();
        }
  ...
  public boolean streamMayHaveExisted(final int streamId) {
    return remoteEndpoint.mayHaveCreatedStream(streamId)
        || localEndpoint.mayHaveCreatedStream(streamId);
  }
  ...
  public void goAwayReceived(
      final int lastKnownStream, final long errorCode, final ByteBuf debugData)
      throws Http2Exception {
    if (localEndpoint.lastStreamKnownByPeer() >= 0
        && localEndpoint.lastStreamKnownByPeer() < lastKnownStream) {
      throw connectionError(
          Http2Error.PROTOCOL_ERROR,
          "lastStreamId MUST NOT increase. Current value: %d new value: %d",
    ...
    if (remoteEndpoint.lastStreamKnownByPeer() >= 0) {
      ...
      if (lastKnownStream > remoteEndpoint.lastStreamKnownByPeer()) {
        throw connectionError(
            ...
            remoteEndpoint.lastStreamKnownByPeer(),
            lastKnownStream);
      }
    }
  ...
  private void closeStreamsGreaterThanLastKnownStreamId(
      final int lastKnownStream, final DefaultEndpoint<?> endpoint) throws Http2Exception {
    forEachActiveStream(
   ...
   * @param itr an iterator that may be pointing to the stream during iteration and {@link
   *     Iterator#remove()} will be used if non-{@code null}.
   */
  ...
  static State activeState(
      final int streamId, final State initialState, final boolean isLocal, final boolean halfClosed)
      throws Http2Exception {
    switch (initialState) {
      case IDLE:
      ...
      default:
        throw streamError(
            streamId,
            Http2Error.PROTOCOL_ERROR,
            "Attempting to open a stream in an invalid state: " + initialState);
    }
  }
  ...
  private class DefaultStream implements Http2Stream {
    ...
    public Http2Stream open(final boolean halfClosed) throws Http2Exception {
      ...
      if (!createdBy().canOpenStream()) {
        throw connectionError(
            Http2Error.PROTOCOL_ERROR, "Maximum active streams violated for this endpoint.");
      }
    ...
    private class PropertyMap {
      ...
      <V> V add(final DefaultPropertyKey key, final V value) {
        ...
        final V prevValue = (V) values[key.index];
  ...
  private final class DefaultEndpoint<F extends Http2FlowController> implements Endpoint<F> {
    ...
    /**
     * The minimum stream ID allowed when creating the next stream. This only applies at the time
     * the stream is created. If the ID of the stream being created is less than this value, stream
     * creation will fail. Upon successful creation of a stream, this value is incremented to the
     * next valid stream ID.
     */
    private int nextStreamIdToCreate;
    /**
     * Used for reservation of stream IDs. Stream IDs can be reserved in advance by applications
     * before the streams are actually created. For example, applications may choose to buffer
     * stream creation attempts as a way of working around {@code SETTINGS_MAX_CONCURRENT_STREAMS},
    ...
    @Override
    public DefaultStream createStream(final int streamId, final boolean halfClosed)
        throws Http2Exception {
      final State state = activeState(streamId, IDLE, isLocal(), halfClosed);
    ...
    @Override
    public DefaultStream reservePushStream(final int streamId, final Http2Stream parent)
        throws Http2Exception {
      if (parent == null) {
        throw connectionError(Http2Error.PROTOCOL_ERROR, "Parent stream missing");
      }
      if (isLocal() ? !parent.state().localSideOpen() : !parent.state().remoteSideOpen()) {
        throw connectionError(
            Http2Error.PROTOCOL_ERROR,
            "Stream %d is not open for sending push promise",
            parent.id());
      }
      if (!opposite().allowPushTo()) {
        throw connectionError(
            Http2Error.PROTOCOL_ERROR, "Server push not allowed to opposite endpoint");
      }
      final State state = isLocal() ? RESERVED_LOCAL : RESERVED_REMOTE;
      checkNewStreamAllowed(streamId, state);
    ...
    private void checkNewStreamAllowed(final int streamId, final State state)
        throws Http2Exception {
      assert state != IDLE;
      ...
      if (!isValidStreamId(streamId)) {
        ...
        throw connectionError(
            Http2Error.PROTOCOL_ERROR,
            "Request stream %d is not correct for %s connection",
            streamId,
            server ? "server" : "client");
      }
      // This check must be after all id validated checks, but before the max streams check because
      // it may be
      ...
      if (streamId < nextStreamIdToCreate) {
        throw closedStreamError(
            Http2Error.PROTOCOL_ERROR,
            "Request stream %d is behind the next expected stream %d",
            streamId,
            nextStreamIdToCreate);
      }
      if (nextStreamIdToCreate <= 0) {
        throw connectionError(
            Http2Error.REFUSED_STREAM, "Stream IDs are exhausted for this endpoint.");
      }
      final boolean isReserved = state == RESERVED_LOCAL || state == RESERVED_REMOTE;
      ...
      if (isClosed()) {
        throw connectionError(
            ...
            "Attempted to create stream id %d after connection was closed",
            streamId);
      }
    }
  ...
  /**
   * Allows events which would modify the collection of active streams to be queued while iterating
   * via {@link #forEachActiveStream(Http2StreamVisitor)}.
   */
  interface Event {
    /**
     * Trigger the original intention of this event. Expect to modify the active streams list.
     *
     * <p>If a {@link RuntimeException} object is thrown it will be logged and <strong>not
     * propagated</strong>. Throwing from this method is not supported and is considered a
     * programming error.
     */
    void process();
  }
  ...
  /**
   * Manages the list of currently active streams. Queues any {@link Event}s that would modify the
   * list of active streams in order to prevent modification while iterating.
   */
  private final class ActiveStreams {
    ...
    ActiveStreams(final List<Listener> listeners) {
  ...
  private final class PropertyKeyRegistry {
    /**
     * Initial size of 4 because the default configuration currently has 3 listeners (local/remote
     * flow controller and {@link StreamByteDistributor}) and we leave room for 1 extra. We could be
     * more aggressive but the ArrayList resize will double the size if we are too small.
     */
    ...
    DefaultPropertyKey newKey() {
      final DefaultPropertyKey key = new DefaultPropertyKey(keys.size());
      keys.add(key);
      return key;
    }
