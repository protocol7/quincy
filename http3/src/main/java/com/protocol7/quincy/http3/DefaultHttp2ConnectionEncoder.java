  ...
  public ChannelFuture writeSettings(
    ...
    try {
      ...
      if (pushEnabled != null && connection.isServer()) {
        throw Http2Exception.connectionError(
            Http2Error.PROTOCOL_ERROR, "Server sending SETTINGS frame with ENABLE_PUSH specified");
      }
    } catch (final Throwable e) {
      return promise.setFailure(e);
    }
    ...
    if (settings == null) {
      return promise.setFailure(
          new Http2Exception(
              ...
              "attempted to write a SETTINGS ACK with no " + " pending SETTINGS"));
    }
    final Http2CodecUtil.SimpleChannelPromiseAggregator aggregator =
  ...
  public ChannelFuture writePushPromise(
    ...
    try {
      if (connection.goAwayReceived()) {
        throw Http2Exception.connectionError(
            Http2Error.PROTOCOL_ERROR, "Sending PUSH_PROMISE after GO_AWAY received.");
      }
