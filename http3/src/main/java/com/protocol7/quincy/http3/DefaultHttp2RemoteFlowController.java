...
public class DefaultHttp2RemoteFlowController implements Http2RemoteFlowController {
    ...
    connection.addListener(
        new Http2ConnectionAdapter() {
          ...
          public void onStreamClosed(final Http2Stream stream) {
            ...
            state(stream).cancel(Http2Error.STREAM_CLOSED, null);
          }
  ...
  private int maxUsableChannelBytes() {
    ...
    final int channelWritableBytes =
        (int) min(Integer.MAX_VALUE, ctx.channel().bytesBeforeUnwritable());
  ...
  private final class FlowState implements StreamByteDistributor.StreamState {
    ...
    int writeAllocatedBytes(@SuppressWarnings("checkstyle:finalparameters") int allocated) {
      ...
      try {
        ...
        while (!cancelled && (frame = peek()) != null) {
          final int maxBytes = min(allocated, writableWindow());
    ...
    void enqueueFrame(final FlowControlled frame) {
      final FlowControlled last = pendingWriteQueue.peekLast();
      if (last == null) {
     ...
     */
    void cancel(final Http2Error error, final Throwable cause) {
      cancelled = true;
  ...
  private class WritabilityMonitor implements StreamByteDistributor.Writer {
    ...
    @Override
    public final void write(final Http2Stream stream, final int numBytes) {
  ...
  private final class ListenerWritabilityMonitor extends WritabilityMonitor
    ...
    private void checkConnectionThenStreamWritabilityChanged(final FlowState state)
