        ...
        public void validateName(final CharSequence name) {
          if (name == null || name.length() == 0) {
            PlatformDependent.throwException(
                connectionError(
                    Http2Error.PROTOCOL_ERROR, "empty headers are not allowed [%s]", name));
          }
          if (name instanceof AsciiString) {
            ...
            try {
              ...
              PlatformDependent.throwException(
                  connectionError(
                      ...
                      t,
                      "unexpected error. invalid header name [%s]",
                      name));
              return;
            }
            ...
            if (index != -1) {
              PlatformDependent.throwException(
                  connectionError(Http2Error.PROTOCOL_ERROR, "invalid header name [%s]", name));
            }
          } else {
