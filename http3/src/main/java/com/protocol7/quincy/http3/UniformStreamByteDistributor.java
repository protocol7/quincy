 ...
 * streams. This class uses a minimum chunk size that will be allocated to each stream. While fewer
 * streams may be written to in each call to {@link #distribute(int, Writer)}, doing this should
 * improve the goodput on each written stream.
 */
@UnstableApi
public final class UniformStreamByteDistributor implements StreamByteDistributor {
   ...
   */
  private int minAllocationChunk = Http2CodecUtil.DEFAULT_MIN_ALLOCATION_CHUNK;
  ...
  public boolean distribute(
    ...
    do {
      ...
      if (maxBytes == 0 && state.streamableBytes > 0) {
        // Stop at the first state that can't send. Add this state back to the head of the queue.
        // Note
        // that empty frames at the head of the queue will always be written, assuming the stream
        // window
        // is not negative.
      ...
      final int chunk = min(chunkSize, min(maxBytes, state.streamableBytes));
      maxBytes -= chunk;
  ...
  private final class State {
    ...
    void updateStreamableBytes(
        final int newStreamableBytes, final boolean hasFrame, final int windowSize) {
      assert hasFrame || newStreamableBytes == 0
          : "hasFrame: " + hasFrame + " newStreamableBytes: " + newStreamableBytes;

      final int delta = newStreamableBytes - streamableBytes;
      if (delta != 0) {
        streamableBytes = newStreamableBytes;
        totalStreamableBytes += delta;
      }
      // In addition to only enqueuing state when they have frames we enforce the following
      // restrictions:
      // 1. If the window has gone negative. We never want to queue a state. However we also don't
      // want to
      //    Immediately remove the item if it is already queued because removal from deque is O(n).
      // So
      //    we allow it to stay queued and rely on the distribution loop to remove this state.
      // 2. If the window is zero we only want to queue if we are not writing. If we are writing
      // that means
      //    we gave the state a chance to write zero length frames. We wait until
      // updateStreamableBytes is
    ...
    /**
     * Write any allocated bytes for the given stream and updates the streamable bytes, assuming all
     * of the bytes will be written.
     */
