...
import static com.protocol7.quincy.http3.Http2CodecUtil.DEFAULT_MAX_FRAME_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.FRAME_HEADER_LENGTH;
import static com.protocol7.quincy.http3.Http2CodecUtil.INT_FIELD_LENGTH;
import static com.protocol7.quincy.http3.Http2CodecUtil.PING_FRAME_PAYLOAD_LENGTH;
import static com.protocol7.quincy.http3.Http2CodecUtil.PRIORITY_ENTRY_LENGTH;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_INITIAL_WINDOW_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTINGS_MAX_FRAME_SIZE;
import static com.protocol7.quincy.http3.Http2CodecUtil.SETTING_ENTRY_LENGTH;
import static com.protocol7.quincy.http3.Http2CodecUtil.headerListSizeExceeded;
import static com.protocol7.quincy.http3.Http2CodecUtil.isMaxFrameSizeValid;
import static com.protocol7.quincy.http3.Http2CodecUtil.readUnsignedInt;
import static com.protocol7.quincy.http3.Http2Error.FLOW_CONTROL_ERROR;
import static com.protocol7.quincy.http3.Http2Error.FRAME_SIZE_ERROR;
...
import static com.protocol7.quincy.http3.Http2FrameTypes.CONTINUATION;
import static com.protocol7.quincy.http3.Http2FrameTypes.DATA;
import static com.protocol7.quincy.http3.Http2FrameTypes.GO_AWAY;
import static com.protocol7.quincy.http3.Http2FrameTypes.HEADERS;
import static com.protocol7.quincy.http3.Http2FrameTypes.PING;
import static com.protocol7.quincy.http3.Http2FrameTypes.PRIORITY;
import static com.protocol7.quincy.http3.Http2FrameTypes.PUSH_PROMISE;
import static com.protocol7.quincy.http3.Http2FrameTypes.RST_STREAM;
import static com.protocol7.quincy.http3.Http2FrameTypes.SETTINGS;
import static com.protocol7.quincy.http3.Http2FrameTypes.WINDOW_UPDATE;

import com.protocol7.quincy.http3.Http2FrameReader.Configuration;
...
public class DefaultHttp2FrameReader
    implements Http2FrameReader, Http2FrameSizePolicy, Configuration {
  private final Http2HeadersDecoder headersDecoder;

  /** {@code true} = reading headers, {@code false} = reading payload. */
  private boolean readingHeaders = true;
  /**
   * Once set to {@code true} the value will never change. This is set to {@code true} if an
   * unrecoverable error which renders the connection unusable.
   */
   ...
   *
   * <p>Header names will be validated.
   */
  ...
  public void maxFrameSize(final int max) throws Http2Exception {
    if (!isMaxFrameSizeValid(max)) {
      throw streamError(
          streamId, FRAME_SIZE_ERROR, "Invalid MAX_FRAME_SIZE specified in sent settings: %d", max);
    }
    maxFrameSize = max;
  }
  ...
  private void processHeaderState(final ByteBuf in) throws Http2Exception {
    ...
    if (payloadLength > maxFrameSize) {
      throw connectionError(
          FRAME_SIZE_ERROR, "Frame length: %d exceeds maximum: %d", payloadLength, maxFrameSize);
    }
  ...
  private void processPayloadState(
      final ChannelHandlerContext ctx, final ByteBuf in, final Http2FrameListener listener)
      throws Http2Exception {
    ...
    final int payloadEndIndex = in.readerIndex() + payloadLength;
  ...
  private void verifyHeadersFrame() throws Http2Exception {
    ...
    final int requiredLength = flags.getPaddingPresenceFieldLength() + flags.getNumPriorityBytes();
  ...
  private void verifyPushPromiseFrame() throws Http2Exception {
    ...
    final int minLength = flags.getPaddingPresenceFieldLength() + INT_FIELD_LENGTH;
  ...
  private void verifyPingFrame() throws Http2Exception {
    ...
    if (payloadLength != PING_FRAME_PAYLOAD_LENGTH) {
      throw connectionError(
          FRAME_SIZE_ERROR, "Frame length %d incorrect size for ping.", payloadLength);
    }
  }
  ...
  private void verifyContinuationFrame() throws Http2Exception {
    ...
    if (headersContinuation == null) {
      throw connectionError(
          PROTOCOL_ERROR, "Received %s frame but not currently processing headers.", frameType);
    }
    ...
    if (payloadLength < flags.getPaddingPresenceFieldLength()) {
      throw streamError(
          streamId, FRAME_SIZE_ERROR, "Frame length %d too small for padding.", payloadLength);
    }
  }
  ...
  private void readDataFrame(
    ...
    final int dataLength =
        lengthWithoutTrailingPadding(payloadEndIndex - payload.readerIndex(), padding);
  ...
  private void readHeadersFrame(
    ...
    // Process the initial fragment, invoking the listener's callback if end of headers.
    final int len = lengthWithoutTrailingPadding(payloadEndIndex - payload.readerIndex(), padding);
  ...
  private void readPriorityFrame(
      final ChannelHandlerContext ctx, final ByteBuf payload, final Http2FrameListener listener)
      throws Http2Exception {
    final long word1 = payload.readUnsignedInt();
    final boolean exclusive = (word1 & 0x80000000L) != 0;
    final int streamDependency = (int) (word1 & 0x7FFFFFFFL);
  ...
  private void readSettingsFrame(
    ...
    if (flags.ack()) {
      ...
      final int numSettings = payloadLength / SETTING_ENTRY_LENGTH;
  ...
  private void readPingFrame(
      final ChannelHandlerContext ctx, final long data, final Http2FrameListener listener)
      throws Http2Exception {
    if (flags.ack()) {
  ...
  private static void readGoAwayFrame(
      ...
      final Http2FrameListener listener)
      throws Http2Exception {
    final int lastStreamId = readUnsignedInt(payload);
    final long errorCode = payload.readUnsignedInt();
    final ByteBuf debugData = payload.readSlice(payloadEndIndex - payload.readerIndex());
  ...
  private void readWindowUpdateFrame(
      final ChannelHandlerContext ctx, final ByteBuf payload, final Http2FrameListener listener)
      throws Http2Exception {
    final int windowSizeIncrement = readUnsignedInt(payload);
    if (windowSizeIncrement == 0) {
      throw streamError(
          streamId, PROTOCOL_ERROR, "Received WINDOW_UPDATE with delta 0 for stream: %d", streamId);
    }
    listener.onWindowUpdateRead(ctx, streamId, windowSizeIncrement);
  }

  private void readContinuationFrame(
      final ByteBuf payload, final int payloadEndIndex, final Http2FrameListener listener)
      throws Http2Exception {
    // Process the initial fragment, invoking the listener's callback if end of headers.
  ...
  /**
   * If padding is present in the payload, reads the next byte as padding. The padding also includes
   * the one byte width of the pad length field. Otherwise, returns zero.
   */
  ...
  private void verifyPadding(final int padding) throws Http2Exception {
    final int len = lengthWithoutTrailingPadding(payloadLength, padding);
    if (len < 0) {
   ...
   * This method returns the number of readable bytes without the trailing padding.
   */
  ...
  private abstract class HeadersContinuation {
     ...
     */
    abstract void processFragment(
        boolean endOfHeaders, ByteBuf fragment, int len, Http2FrameListener listener)
        throws Http2Exception;
  ...
  /**
   * Utility class to help with construction of the headers block that may potentially span multiple
   * frames.
   */
  protected class HeadersBlockBuilder {
    ...
    final void addFragment(
      ...
      if (headerBlock.isWritable(len)) {
        ...
        final ByteBuf buf = alloc.buffer(headerBlock.readableBytes() + len);
  ...
  private void verifyNotProcessingHeaders() throws Http2Exception {
    if (headersContinuation != null) {
      throw connectionError(
          PROTOCOL_ERROR,
          "Received frame of type %s while processing headers on stream %d.",
          frameType,
          headersContinuation.getStreamId());
    }
  }
  ...
  private void verifyPayloadLength(final int payloadLength) throws Http2Exception {
    if (payloadLength > maxFrameSize) {
      throw connectionError(
          PROTOCOL_ERROR, "Total payload length %d exceeds max frame length.", payloadLength);
    }
  }
  ...
  private void verifyAssociatedWithAStream() throws Http2Exception {
    if (streamId == 0) {
      throw connectionError(
          PROTOCOL_ERROR, "Frame of type %s must be associated with a stream.", frameType);
    }
  }
