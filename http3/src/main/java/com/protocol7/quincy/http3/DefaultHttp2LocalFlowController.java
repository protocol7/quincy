...
import com.protocol7.quincy.http3.Http2Exception.CompositeStreamException;
...
public class DefaultHttp2LocalFlowController implements Http2LocalFlowController {
  /**
   * The default ratio of window size to initial window size below which a {@code WINDOW_UPDATE} is
   * sent to expand the window.
   */
  ...
  private int initialWindowSize = Http2CodecUtil.DEFAULT_WINDOW_SIZE;
   ...
   *     in the flow control algorithm as they will always refill automatically without requiring
   *     the application to consume the bytes. When enabled, the maximum bytes you must be prepared
   *     to queue is proportional to {@code maximum number of concurrent streams * the initial
   *     window size per stream} (<a
   *     href="https://tools.ietf.org/html/rfc7540#section-6.5.2">SETTINGS_MAX_CONCURRENT_STREAMS</a>
   *     <a
   *     href="https://tools.ietf.org/html/rfc7540#section-6.5.2">SETTINGS_INITIAL_WINDOW_SIZE</a>).
   */
  public DefaultHttp2LocalFlowController(
      final Http2Connection connection,
      final float windowUpdateRatio,
      final boolean autoRefillConnectionWindow) {
    this.connection = checkNotNull(connection, "connection");
    windowUpdateRatio(windowUpdateRatio);

    // Add a flow state for the connection.
    stateKey = connection.newKey();
    final FlowState connectionState =
    ...
    connection.addListener(
        new Http2ConnectionAdapter() {
          ...
          public void onStreamAdded(final Http2Stream stream) {
            // Unconditionally used the reduced flow control state because it requires no object
            // allocation
          ...
          public void onStreamClosed(final Http2Stream stream) {
            try {
              ...
              // Unconditionally reduce the amount of memory required for flow control because there
              // is no
              // object allocation costs associated with doing so and the stream will not have any
              // more
  ...
  public void incrementWindowSize(final Http2Stream stream, final int delta) throws Http2Exception {
    assert ctx != null && ctx.executor().inEventLoop();
    final FlowState state = state(stream);
    // Just add the delta to the stream-specific initial window size so that the next time the
    // window
  ...
  /**
   * The window update ratio is used to determine when a window update must be sent. If the ratio of
   * bytes processed since the last update has meet or exceeded this ratio then a window update will
   ...
   *
   * <p>Note it is the responsibly of the caller to ensure that the the initial {@code SETTINGS}
   * frame is sent before this is called. It would be considered a {@link Http2Error#PROTOCOL_ERROR}
   * if a {@code WINDOW_UPDATE} was generated by this method before the initial {@code SETTINGS}
   * frame is sent.
   *
  ...
  public void receiveFlowControlledFrame(
    ...
    final FlowState connectionState = connectionState();
  ...
  private class DefaultState implements FlowState {
     ...
     * application has indicated that the data has been fully processed. This prevents sending a
     * {@code WINDOW_UPDATE} until the number of processed bytes drops below the threshold.
     */
    private int processedWindow;

    /**
     * This is what is used to determine how many bytes need to be returned relative to {@link
     * #processedWindow}. Each stream has their own initial window size.
     */
    private int initialStreamWindowSize;

    /**
     * This is used to determine when {@link #processedWindow} is sufficiently far away from {@link
     * #initialStreamWindowSize} such that a {@code WINDOW_UPDATE} should be sent. Each stream has
     * their own window update ratio.
     */
      ...
      final int newValue =
          (int)
              Math.min(
                  ...
                  Math.max(
                      Http2CodecUtil.MIN_INITIAL_WINDOW_SIZE,
                      initialStreamWindowSize + (long) delta));
    ...
    public void incrementFlowControlWindows(final int delta) throws Http2Exception {
      if (delta > 0 && window > Http2CodecUtil.MAX_INITIAL_WINDOW_SIZE - delta) {
        throw streamError(
            stream.id(),
    ...
    public boolean writeWindowUpdateIfNeeded() throws Http2Exception {
      ...
      final int threshold = (int) (initialStreamWindowSize * streamWindowUpdateRatio);
    ...
    private void writeWindowUpdate() throws Http2Exception {
      ...
      final int deltaWindowSize = initialStreamWindowSize - processedWindow;
      try {
  ...
  /**
   * The local flow control state for a single stream that is not in a state where flow controlled
  ...
  private interface FlowState {
    ...
    /**
     * Indicates that the application has consumed {@code numBytes} from the connection or stream
    ...
    /**
     * A flow control event has occurred and we should decrement the amount of available bytes for
     * this stream.
     *
     * @param dataLength The amount of data to for which this stream is no longer eligible to use
     *     for flow control.
