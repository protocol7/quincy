...
/**
 * This adapter provides just header/data events from the HTTP message flow defined here <a
 * href="http://tools.ietf.org/html/draft-ietf-httpbis-http2-16#section-8.1.">HTTP/2 Spec Message
 * Flow</a>.
 *
 * <p>See {@link HttpToHttp2ConnectionHandler} to get translation from HTTP/1.x objects to HTTP/2
 * frames for writes.
 */
@UnstableApi
public class InboundHttp2ToHttpAdapter extends Http2EventAdapter {
  ...
  protected InboundHttp2ToHttpAdapter(
    ...
    if (maxContentLength <= 0) {
      throw new IllegalArgumentException(
          "maxContentLength: " + maxContentLength + " (expected: > 0)");
    }
   ...
   * @param release {@code true} to call release on the value if it is present. {@code false} to not
   *     call release.
   */
  protected final void removeMessage(final Http2Stream stream, final boolean release) {
    final FullHttpMessage msg = stream.removeProperty(messageKey);
    if (release && msg != null) {
      msg.release();
    }
  }
  ...
  protected final void putMessage(final Http2Stream stream, final FullHttpMessage message) {
    final FullHttpMessage previous = stream.setProperty(messageKey, message);
   ...
   */
  protected void fireChannelRead(
      final ChannelHandlerContext ctx,
      final FullHttpMessage msg,
  ...
  protected FullHttpMessage newMessage(
    ...
    return connection.isServer()
        ? HttpConversionUtil.toFullHttpRequest(stream.id(), headers, alloc, validateHttpHeaders)
   ...
   *     <ul>
   *       <li>{@code true} if a message {@code stream} already exists then the headers should be
  ...
  private void processHeadersEnd(
      ...
      final boolean endOfStream) {
    if (endOfStream) {
      // Release if the msg from the map is different from the object being forwarded up the
      // pipeline.
  ...
  public int onDataRead(
    ...
    final FullHttpMessage msg = getMessage(stream);
    if (msg == null) {
      throw connectionError(
          Http2Error.PROTOCOL_ERROR, "Data Frame received for unknown stream id %d", streamId);
    }
  ...
  public void onHeadersRead(
      ...
      throws Http2Exception {
    final Http2Stream stream = connection.stream(streamId);
    final FullHttpMessage msg = processHeadersBegin(ctx, stream, headers, endOfStream, true, true);
    if (msg != null) {
  ...
  public void onRstStreamRead(
    ...
    ctx.fireExceptionCaught(
        Http2Exception.streamError(
            streamId, Http2Error.valueOf(errorCode), "HTTP/2 to HTTP layer caught stream reset"));
  }
  ...
  public void onPushPromiseRead(
    ...
    final Http2Stream promisedStream = connection.stream(promisedStreamId);
    ...
    final FullHttpMessage msg =
        processHeadersBegin(ctx, promisedStream, headers, false, false, false);
    if (msg == null) {
      throw connectionError(
  ...
  /**
   * Allows messages to be sent up the pipeline before the next phase in the HTTP message flow is
   * detected.
   */
  private interface ImmediateSendDetector {
     ...
     *
     * <p>An example of this use case is if a request is received with a 'Expect: 100-continue'
     * header. The message will be sent immediately, and the data will be queued and sent at the end
     * of the stream.
     *
     * @param msg The message which has just been sent due to {@link
     *     #mustSendImmediately(FullHttpMessage)}
