 ...
 *
 * @see <a href="https://tools.ietf.org/html/draft-ietf-httpbis-http2-17#section-8.2">[RFC http2],
 *     Section 8.2</a>.
 */
@UnstableApi
public interface Http2PromisedRequestVerifier {
  /**
   * Determine if a {@link Http2Headers} are authoritative for a particular {@link
   * ChannelHandlerContext}.
   *
   * @param ctx The context on which the {@code headers} where received on.
   * @param headers The headers to be verified.
   * @return {@code true} if the {@code ctx} is authoritative for the {@code headers}, {@code false}
   *     otherwise.
   * @see <a href="https://tools.ietf.org/html/draft-ietf-httpbis-http2-17#section-10.1">[RFC
   *     http2], Section 10.1</a>.
   */
   ...
   *
   * @param headers The headers for a push request.
   * @return {@code true} if the request associated with {@code headers} is known to be safe, {@code
   *     false} otherwise.
  ...
  /**
   * A default implementation of {@link Http2PromisedRequestVerifier} which always returns positive
   * responses for all verification challenges.
   */
  Http2PromisedRequestVerifier ALWAYS_VERIFY =
      new Http2PromisedRequestVerifier() {
        @Override
        public boolean isAuthoritative(
            final ChannelHandlerContext ctx, final Http2Headers headers) {
          return true;
        }
