...
import static com.protocol7.quincy.http3.Http2Headers.PseudoHeaderName.getPseudoHeader;
import static com.protocol7.quincy.http3.Http2Headers.PseudoHeaderName.hasPseudoHeaderFormat;
...
final class HpackDecoder {
  private static final Http2Exception DECODE_ULE_128_DECOMPRESSION_EXCEPTION =
      unknownStackTrace(
          connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - decompression failure"),
          HpackDecoder.class,
          "decodeULE128(..)");
  private static final Http2Exception DECODE_ULE_128_TO_LONG_DECOMPRESSION_EXCEPTION =
      unknownStackTrace(
          connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - long overflow"),
          HpackDecoder.class,
          "decodeULE128(..)");
  private static final Http2Exception DECODE_ULE_128_TO_INT_DECOMPRESSION_EXCEPTION =
      unknownStackTrace(
          connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - int overflow"),
          HpackDecoder.class,
          "decodeULE128ToInt(..)");
  private static final Http2Exception DECODE_ILLEGAL_INDEX_VALUE =
      unknownStackTrace(
          connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - illegal index value"),
          HpackDecoder.class,
          "decode(..)");
  ...
  private static final Http2Exception INVALID_MAX_DYNAMIC_TABLE_SIZE =
      unknownStackTrace(
          connectionError(Http2Error.COMPRESSION_ERROR, "HPACK - invalid max dynamic table size"),
          HpackDecoder.class,
          "setDynamicTableSize(..)");
  private static final Http2Exception MAX_DYNAMIC_TABLE_SIZE_CHANGE_REQUIRED =
      unknownStackTrace(
          connectionError(
              Http2Error.COMPRESSION_ERROR, "HPACK - max dynamic table size change required"),
          HpackDecoder.class,
          "decode(..)");
   ...
   *
   * @param maxHeaderListSize This is the only setting that can be configured before notifying the
   *     peer. This is because <a
   *     href="https://tools.ietf.org/html/rfc7540#section-6.5.1">SETTINGS_MAX_HEADER_LIST_SIZE</a>
   *     allows a lower than advertised limit from being enforced, and the default limit is
   *     unlimited (which is dangerous).
  ...
  /**
   * Exposed Used for testing only! Default values used in the initial settings frame are overridden
  ...
  public void decode(
    ...
    final Http2HeadersSink sink =
        new Http2HeadersSink(streamId, headers, maxHeaderListSize, validateHeaders);
    decode(in, sink);
  ...
  private void decode(final ByteBuf in, final Sink sink) throws Http2Exception {
    ...
    while (in.isReadable()) {
      switch (state) {
          ...
          if (b < 0) {
            ...
            switch (index) {
              ...
              case 0x7F:
                ...
                break;
              default:
                final HpackHeaderField indexedHeader = getIndexedHeader(index);
        ...
        case READ_INDEXED_HEADER:
          final HpackHeaderField indexedHeader = getIndexedHeader(decodeULE128(in, index));
          sink.appendToHeaderList(indexedHeader.name, indexedHeader.value);
          state = READ_HEADER_REPRESENTATION;
          break;
        ...
        case READ_LITERAL_HEADER_VALUE:
          ...
          final CharSequence value = readStringLiteral(in, valueLength, huffmanEncoded);
    ...
    if (state != READ_HEADER_REPRESENTATION) {
      throw connectionError(Http2Error.COMPRESSION_ERROR, "Incomplete header block fragment.");
    }
  }

  /**
   * Set the maximum table size. If this is below the maximum size of the dynamic table used by the
   * encoder, the beginning of the next header block MUST signal this change.
   */
  ...
  /**
   * @deprecated use {@link #setMaxHeaderListSize(long)}; {@code maxHeaderListSizeGoAway} is ignored
   */
  @Deprecated
  public void setMaxHeaderListSize(final long maxHeaderListSize, final long maxHeaderListSizeGoAway)
      throws Http2Exception {
  ...
  private static HeaderType validate(
      final int streamId, final CharSequence name, final HeaderType previousHeaderType)
      throws Http2Exception {
    if (hasPseudoHeaderFormat(name)) {
      if (previousHeaderType == HeaderType.REGULAR_HEADER) {
        throw streamError(
            streamId,
            Http2Error.PROTOCOL_ERROR,
            "Pseudo-header field '%s' found after regular header.",
            name);
      }
      ...
      final HeaderType currentHeaderType =
          pseudoHeader.isRequestOnly()
              ? HeaderType.REQUEST_PSEUDO_HEADER
              : HeaderType.RESPONSE_PSEUDO_HEADER;
      if (previousHeaderType != null && currentHeaderType != previousHeaderType) {
        throw streamError(
            streamId, Http2Error.PROTOCOL_ERROR, "Mix of request and response pseudo-headers.");
      }
  ...
  private CharSequence readName(final int index) throws Http2Exception {
    ...
    if (index - HpackStaticTable.length <= hpackDynamicTable.length()) {
      final HpackHeaderField hpackHeaderField =
          hpackDynamicTable.getEntry(index - HpackStaticTable.length);
      return hpackHeaderField.name;
    }
  ...
  private CharSequence readStringLiteral(
      final ByteBuf in, final int length, final boolean huffmanEncoded) throws Http2Exception {
    if (huffmanEncoded) {
