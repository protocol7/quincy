...
public class DefaultHttp2ConnectionDecoder implements Http2ConnectionDecoder {
  ...
  private final class FrameReadListener implements Http2FrameListener {
    ...
    public int onDataRead(
      ...
      try {
        ...
        throw connectionError(
            Http2Error.INTERNAL_ERROR, t, "Unhandled error on data stream id %d", streamId);
      }
    ...
    public void onRstStreamRead(
      ...
      switch (stream.state()) {
        case IDLE:
          throw connectionError(
              Http2Error.PROTOCOL_ERROR, "RST_STREAM received for IDLE stream %d", streamId);
        case CLOSED:
    ...
    public void onPushPromiseRead(
      ...
      if (connection().isServer()) {
        throw connectionError(Http2Error.PROTOCOL_ERROR, "A client cannot push.");
      }
      ...
      if (parentStream == null) {
        throw connectionError(Http2Error.PROTOCOL_ERROR, "Stream %d does not exist", streamId);
      }
    ...
    public void onWindowUpdateRead(
      ...
      if (stream == null
          ...
          || streamCreatedAfterGoAwaySent(streamId)) {
        // Ignore this frame.
        verifyStreamMayHaveExisted(streamId);
        return;
      }
    ...
    private boolean streamCreatedAfterGoAwaySent(final int streamId) {
      final Http2Connection.Endpoint<?> remote = connection.remote();
  ...
  private final class PrefaceFrameListener implements Http2FrameListener {
    ...
    private void verifyPrefaceReceived() throws Http2Exception {
      if (!prefaceReceived()) {
        throw connectionError(Http2Error.PROTOCOL_ERROR, "Received non-SETTINGS as first frame.");
      }
    }
