...
/**
 * Abstract base class which defines commonly used features required to build {@link
 ...
 *
 * Simply call all the necessary setter methods, and then use {@link #build()} to build a new {@link
 * Http2ConnectionHandler}. Setting the following properties are prohibited because they are used
 * for other ways of building a {@link Http2ConnectionHandler}. conflicts with this option:
 *
 * <ul>
 *   <li>{@link #connection(Http2Connection)}
 *   <li>{@link #codec(Http2ConnectionDecoder, Http2ConnectionEncoder)}
 * </ul>
 *
 * <h4>Let the builder use the {@link Http2ConnectionHandler} you specified</h4>
 *
 * Call {@link #connection(Http2Connection)} to tell the builder that you want to build the handler
 * from the {@link Http2Connection} you specified. Setting the following properties are prohibited
 * and thus will trigger an {@link IllegalStateException} because they conflict with this option.
 *
 * <ul>
 *   <li>{@link #server(boolean)}
 *   <li>{@link #codec(Http2ConnectionDecoder, Http2ConnectionEncoder)}
 * </ul>
 *
 * <h4>Let the builder use the {@link Http2ConnectionDecoder} and {@link Http2ConnectionEncoder} you
 * specified</h4>
 *
 * Call {@link #codec(Http2ConnectionDecoder, Http2ConnectionEncoder)} to tell the builder that you
 * want to built the handler from the {@link Http2ConnectionDecoder} and {@link
 * Http2ConnectionEncoder} you specified. Setting the following properties are prohibited and thus
 * will trigger an {@link IllegalStateException} because they conflict with this option:
 *
 * <ul>
 *   <li>{@link #server(boolean)}
 *   <li>{@link #connection(Http2Connection)}
 *   <li>{@link #frameLogger(Http2FrameLogger)}
 *   <li>{@link #headerSensitivityDetector(Http2HeadersEncoder.SensitivityDetector)}
 *   <li>{@link #encoderEnforceMaxConcurrentStreams(boolean)}
 *   <li>{@link #encoderIgnoreMaxHeaderListSize(boolean)}
 *   <li>{@link #initialHuffmanDecodeCapacity(int)}
 * </ul>
 *
 * <h3>Exposing necessary methods in a subclass</h3>
 *
 * {@link #build()} method and all property access methods are {@code protected}. Choose the methods
 * to expose to the users of your builder implementation and make them {@code public}.
 *
...
public abstract class AbstractHttp2ConnectionHandlerBuilder<
    T extends Http2ConnectionHandler, B extends AbstractHttp2ConnectionHandlerBuilder<T, B>> {
  ...
  private int initialHuffmanDecodeCapacity = Http2CodecUtil.DEFAULT_INITIAL_HUFFMAN_DECODE_CAPACITY;
  private Http2PromisedRequestVerifier promisedRequestVerifier =
      Http2PromisedRequestVerifier.ALWAYS_VERIFY;
  ...
  /**
   * Sets the listener of inbound frames. This listener will only be set if the decoder's listener
   * is {@code null}.
   */
  ...
  /**
   * Returns the graceful shutdown timeout of the {@link Http2Connection} in milliseconds. Returns
   * -1 if the timeout is indefinite.
   */
  ...
  /**
   * Returns if {@link #build()} will to create a {@link Http2Connection} in server mode ({@code
   * true}) or client mode ({@code false}).
   */
   ...
   *
   * <p>By default this value will be ignored on the server for local endpoint. This is because the
   * RFC provides no way to explicitly communicate a limit to how many states can be in the reserved
   * state, and instead relies on the peer to send RST_STREAM frames when they will be rejected.
   */
  protected int maxReservedStreams() {
    return maxReservedStreams != null
        ? maxReservedStreams
        : Http2CodecUtil.DEFAULT_MAX_RESERVED_STREAMS;
  }
  ...
  protected B codec(final Http2ConnectionDecoder decoder, final Http2ConnectionEncoder encoder) {
    ...
    enforceConstraint(
        "codec", "encoderEnforceMaxConcurrentStreams", encoderEnforceMaxConcurrentStreams);
  ...
  /**
   * Returns if HTTP headers should be validated according to <a
   * href="https://tools.ietf.org/html/rfc7540#section-8.1.2.6">RFC 7540, 8.1.2.6</a>.
   */
  ...
  /**
   * Sets if HTTP headers should be validated according to <a
   * href="https://tools.ietf.org/html/rfc7540#section-8.1.2.6">RFC 7540, 8.1.2.6</a>.
   */
  ...
  /**
   * Returns if the encoder should queue frames if the maximum number of concurrent streams would
  ...
  /**
   * Sets if the encoder should queue frames if the maximum number of concurrent streams would
   * otherwise be exceeded.
   */
  ...
  /** Returns the {@link Http2HeadersEncoder.SensitivityDetector} to use. */
  protected Http2HeadersEncoder.SensitivityDetector headerSensitivityDetector() {
    return headerSensitivityDetector != null
        ? headerSensitivityDetector
        : DEFAULT_HEADER_SENSITIVITY_DETECTOR;
  }

  /** Sets the {@link Http2HeadersEncoder.SensitivityDetector} to use. */
  protected B headerSensitivityDetector(
    ...
    this.headerSensitivityDetector =
        checkNotNull(headerSensitivityDetector, "headerSensitivityDetector");
    return self();
  }
   ...
   *
   * @param ignoreMaxHeaderListSize {@code true} to ignore <a
   *     href="https://tools.ietf.org/html/rfc7540#section-6.5.2">SETTINGS_MAX_HEADER_LIST_SIZE</a>.
   * @return this.
   */
   ...
   *
   * @param initialHuffmanDecodeCapacity initial size of an intermediate buffer used during HPACK
   *     huffman decoding.
   * @return this.
   */
  protected B initialHuffmanDecodeCapacity(final int initialHuffmanDecodeCapacity) {
    ...
    this.initialHuffmanDecodeCapacity =
        checkPositive(initialHuffmanDecodeCapacity, "initialHuffmanDecodeCapacity");
    return self();
  }
   ...
   *
   * @param decoupleCloseAndGoAway {@code true} to make {@link Channel#close()} directly close the
   *     underlying transport, and not attempt graceful closure via GOAWAY.
   * @return {@code this}.
   */
  ...
  private T buildFromConnection(final Http2Connection connection) {
        ...
        new DefaultHttp2FrameReader(
            new DefaultHttp2HeadersDecoder(
                ...
                maxHeaderListSize == null
                    ? Http2CodecUtil.DEFAULT_HEADER_LIST_SIZE
                    : maxHeaderListSize,
    ...
    Http2FrameWriter writer =
        encoderIgnoreMaxHeaderListSize == null
            ...
            : new DefaultHttp2FrameWriter(
                headerSensitivityDetector(), encoderIgnoreMaxHeaderListSize);
    ...
    final boolean encoderEnforceMaxConcurrentStreams = encoderEnforceMaxConcurrentStreams();
    ...
    final DefaultHttp2ConnectionDecoder decoder =
        new DefaultHttp2ConnectionDecoder(
            connection, encoder, reader, promisedRequestVerifier(), isAutoAckSettingsFrame());
    return buildFromCodec(decoder, encoder);
  }
   ...
   *
   * <p>The return of this method will be subject to the following:
   *
   * <ul>
   *   <li>{@link #frameListener(Http2FrameListener)} will be set if not already set in the decoder
   *   <li>{@link #gracefulShutdownTimeoutMillis(long)} will always be set
   * </ul>
   */
  ...
  private static void enforceConstraint(
      final String methodName, final String rejectorName, final Object value) {
    if (value != null) {
      throw new IllegalStateException(
          methodName
              + "() cannot be called because "
              + rejectorName
