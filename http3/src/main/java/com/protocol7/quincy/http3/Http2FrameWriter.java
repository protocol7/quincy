...
public interface Http2FrameWriter extends Http2DataWriter, Closeable {
   ...
   * @return the future for the write. <a
   *     href="https://tools.ietf.org/html/rfc7540#section-10.5.1">Section 10.5.1</a> states the
   *     following:
   *     <pre>
   * The header block MUST be processed to ensure a consistent connection state, unless the connection is closed.
   * </pre>
   *     If this call has modified the HPACK header state you <strong>MUST</strong> throw a
   *     connection error.
   *     <p>If this call has <strong>NOT</strong> modified the HPACK header state you are free to
   *     throw a stream error.
   */
   ...
   * @param streamId the stream for which to send the frame.
   * @param headers the headers to be sent.
   * @param streamDependency the stream on which this stream should depend, or 0 if it should depend
   *     on the connection.
   ...
   *
   * @param ctx the context to use for writing.
   * @param ack indicates whether this is an ack of a PING frame previously received from the remote
   *     endpoint.
   * @param data the payload of the frame.
   * @param promise the promise for the write.
   * @return the future for the write.
   */
  ChannelFuture writePing(
