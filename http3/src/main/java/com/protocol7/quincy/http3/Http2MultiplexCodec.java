 ...
 * receive {@link Http2StreamFrame}s on the created channel. {@link ByteBuf}s cannot be processed by
 * the channel; all writes that reach the head of the pipeline must be an instance of {@link
 * Http2StreamFrame}. Writes that reach the head of the pipeline are processed directly by this
 * handler and cannot be intercepted.
 *
 ...
 * Channel#read()}, which follows the default behavior of channels in Netty. Applications are free
 * to close the channel in response to such events if they don't have use for any queued messages.
 * Any connection level events like {@link Http2SettingsFrame} and {@link Http2GoAwayFrame} will be
 * processed internally and also propagated down the pipeline for other handlers to act on.
 *
 ...
 * reference counted objects (e.g. {@link ByteBuf}s). The multiplex codec will call {@link
 ...
 *
 * A child channel becomes active as soon as it is registered to an {@link EventLoop}. Therefore, an
 * active channel does not map to an active HTTP/2 stream immediately. Only once a {@link
 * Http2HeadersFrame} has been successfully sent or received, does the channel map to an active
 * HTTP/2 stream. In case it is not possible to open a new HTTP/2 stream (i.e. due to the maximum
 * number of active streams being exceeded), the child channel receives an exception indicating the
 * cause and is closed immediately thereafter.
 *
 * <h3>Writability and Flow Control</h3>
 *
 * A child channel observes outbound/remote flow control via the channel's writability. A channel
 * only becomes writable when it maps to an active HTTP/2 stream and the stream's flow control
 * window is greater than zero. A child channel does not know about the connection-level flow
 * control window. {@link ChannelHandler}s are free to ignore the channel's writability, in which
 * case the excessive writes will be buffered by the parent channel. It's important to note that
 * only {@link Http2DataFrame}s are subject to HTTP/2 flow control.
 */
@UnstableApi
public class Http2MultiplexCodec extends Http2FrameCodec {

  private static final InternalLogger logger =
      InternalLoggerFactory.getInstance(DefaultHttp2StreamChannel.class);
  ...
  private static final ClosedChannelException CLOSED_CHANNEL_EXCEPTION =
      ThrowableUtil.unknownStackTrace(
          ...
          DefaultHttp2StreamChannel.Http2ChannelUnsafe.class,
          "write(...)");
  /**
  ...
  private static final class FlowControlledFrameSizeEstimator implements MessageSizeEstimator {
    ...
    static final MessageSizeEstimator.Handle HANDLE_INSTANCE =
        new MessageSizeEstimator.Handle() {
          @Override
  ...
  // Linked-List for DefaultHttp2StreamChannel instances that need to be processed by
  // channelReadComplete(...)
  private DefaultHttp2StreamChannel head;
  private DefaultHttp2StreamChannel tail;

  // Need to be volatile as accessed from within the DefaultHttp2StreamChannel in a multi-threaded
  // fashion.
  ...
  public void onHttpClientUpgrade() throws Http2Exception {
    ...
    if (upgradeStreamHandler == null) {
      throw connectionError(
          Http2Error.INTERNAL_ERROR, "Client is misconfigured for upgrade requests");
    }
    ...
    final Http2MultiplexCodecStream codecStream = newStream();
    codecStream.setStreamAndProperty(
        streamKey, connection().stream(Http2CodecUtil.HTTP_UPGRADE_STREAM_ID));
    onHttp2UpgradeStreamInitialized(ctx, codecStream);
  }

  private static void registerDone(final ChannelFuture future) {
  ...
  public final void handlerRemoved0(final ChannelHandlerContext ctx) throws Exception {
    ...
    while (ch != null) {
      final DefaultHttp2StreamChannel curr = ch;
      ch = curr.next;
  ...
  final void onHttp2Frame(final ChannelHandlerContext ctx, final Http2Frame frame) {
    if (frame instanceof Http2StreamFrame) {
      ...
      final Http2Settings settings = ((Http2SettingsFrame) frame).settings();
  ...
  private void onHttp2UpgradeStreamInitialized(
      final ChannelHandlerContext ctx, final Http2MultiplexCodecStream stream) {
    assert stream.state() == Http2Stream.State.HALF_CLOSED_LOCAL;
    final DefaultHttp2StreamChannel ch = new DefaultHttp2StreamChannel(stream, true);
  ...
  @Override
  final void onHttp2StreamStateChanged(
      final ChannelHandlerContext ctx, final Http2FrameStream stream) {
    final Http2MultiplexCodecStream s = (Http2MultiplexCodecStream) stream;
    ...
    switch (stream.state()) {
      ...
      case OPEN:
        ...
        final ChannelFuture future =
            ctx.channel().eventLoop().register(new DefaultHttp2StreamChannel(s, false));
        if (future.isDone()) {
          registerDone(future);
        } else {
          future.addListener(CHILD_CHANNEL_REGISTRATION_LISTENER);
        ...
        break;
      case CLOSED:
        final DefaultHttp2StreamChannel channel = s.channel;
        if (channel != null) {
  ...
  @Override
  final void onHttp2StreamWritabilityChanged(
      final ChannelHandlerContext ctx, final Http2FrameStream stream, final boolean writable) {
  ...
  @Override
  final void onHttp2FrameStreamException(
  ...
  private boolean isChildChannelInReadPendingQueue(final DefaultHttp2StreamChannel childChannel) {
    return childChannel.previous != null || childChannel.next != null || head == childChannel;
  }

  final void tryAddChildChannelToReadPendingQueue(final DefaultHttp2StreamChannel childChannel) {
    if (!isChildChannelInReadPendingQueue(childChannel)) {
      addChildChannelToReadPendingQueue(childChannel);
    }
  }

  final void addChildChannelToReadPendingQueue(final DefaultHttp2StreamChannel childChannel) {
    if (tail == null) {
      assert head == null;
  ...
  private void tryRemoveChildChannelFromReadPendingQueue(
      final DefaultHttp2StreamChannel childChannel) {
    if (isChildChannelInReadPendingQueue(childChannel)) {
  ...
  private void removeChildChannelFromReadPendingQueue(
      final DefaultHttp2StreamChannel childChannel) {
    final DefaultHttp2StreamChannel previous = childChannel.previous;
    if (childChannel.next != null) {
      childChannel.next.previous = previous;
    } else {
      tail = tail.previous; // If there is no next, this childChannel is the tail, so move the tail
      // back.
    }
    if (previous != null) {
      previous.next = childChannel.next;
    } else {
      head = head.next; // If there is no previous, this childChannel is the head, so move the tail
      // forward.
    }
    childChannel.next = childChannel.previous = null;
  }

  private void onHttp2GoAwayFrame(
      final ChannelHandlerContext ctx, final Http2GoAwayFrame goAwayFrame) {
    try {
      forEachActiveStream(
          new Http2FrameStreamVisitor() {
            ...
            public boolean visit(final Http2FrameStream stream) {
              ...
              final DefaultHttp2StreamChannel childChannel =
                  ((Http2MultiplexCodecStream) stream).channel;
              if (streamId > goAwayFrame.lastStreamId()
                  && connection().local().isValidStreamId(streamId)) {
  ...
  final void onChannelReadComplete(final ChannelHandlerContext ctx) {
    ...
    while (current != null) {
      final DefaultHttp2StreamChannel childChannel = current;
  ...
  private boolean initialWritability(final DefaultHttp2FrameStream stream) {
    // If the stream id is not valid yet we will just mark the channel as writable as we will be
    // notified
  ...
  private final class DefaultHttp2StreamChannel extends DefaultAttributeMap
     ...
     * Note that depending upon the {@link RecvByteBufAllocator} behavior a read may extend beyond
     * the {@link Http2ChannelUnsafe#beginRead()} method scope. The {@link
     * Http2ChannelUnsafe#beginRead()} loop may drain all pending data, and then if the parent
     * channel is reading this channel may still accept frames.
     */
    ...
    // Currently the child channel and parent channel are always on the same EventLoop thread. This
    // allows us to
    // extend the read loop of a child channel if the child channel drains its queued data during
    // read, and the
    // parent channel is still in its read loop. The next/previous links build a doubly linked list
    // that the parent
    // channel will iterate in its channelReadComplete to end the read cycle for each child channel
    // in the list.
    DefaultHttp2StreamChannel next;
    DefaultHttp2StreamChannel previous;

    DefaultHttp2StreamChannel(final DefaultHttp2FrameStream stream, final boolean outbound) {
      this.stream = stream;
    ...
    void writabilityChanged(final boolean writable) {
    ...
    void fireChildRead(final Http2Frame frame) {
      ...
      if (!isActive()) {
        ...
        // are limited
        // to the same EventLoop thread. There are a limited number of frame types that may come
        // after EOS is
    ...
    private final class Http2ChannelUnsafe implements Unsafe {
      ...
      public void close(final ChannelPromise promise) {
        ...
        // Only ever send a reset frame if the connection is still alive and if the stream may have
        // existed
        ...
        if (parent().isActive() && !readEOS && connection().streamMayHaveExisted(stream().id())) {
          final Http2StreamFrame resetFrame =
              new DefaultHttp2ResetFrame(Http2Error.CANCEL).stream(stream());
        ...
        if (inboundBuffer != null) {
          for (; ; ) {
            final Object msg = inboundBuffer.poll();
            if (msg == null) {
              break;
            }
      ...
      private void safeSetSuccess(final ChannelPromise promise) {
        if (!(promise instanceof VoidChannelPromise) && !promise.trySuccess()) {
          logger.warn(
              "Failed to mark a promise as success because it is done already: {}", promise);
        }
      }
      ...
      private void invokeLater(final Runnable task) {
        try {
          ...
          // later.
          // They do not trigger an inbound event immediately because an outbound operation might
          // have been
          // triggered by another inbound event handler method.  If fired immediately, the call
          // stack
          ...
          //       -> handlerA.channelInactive() - (2) another inbound handler method called while
          // in (1) yet
          //
      ...
      void doBeginRead() {
        ...
        if (inboundBuffer == null || (message = inboundBuffer.poll()) == null) {
          ...
          if (continueReading && parentReadInProgress && !readEOS) {
            // Currently the parent and child channel are on the same EventLoop thread. If the
            // parent is
            // currently reading it is possile that more frames will be delivered to this child
            // channel. In
      ...
      void notifyReadComplete(final Handle allocHandle) {
        ...
        // parent
        // channel is not currently reading we need to force a flush at the child channel, because
        // we cannot
      ...
      @SuppressWarnings("deprecation")
      void doRead0(final Http2Frame frame, final Handle allocHandle) {
      ...
      public void write(final Object msg, final ChannelPromise promise) {
        ...
        try {
          if (msg instanceof Http2StreamFrame) {
            final Http2StreamFrame frame =
                validateStreamFrame((Http2StreamFrame) msg).stream(stream());
            if (!firstFrameWritten && !Http2CodecUtil.isStreamIdValid(stream().id())) {
              if (!(frame instanceof Http2HeadersFrame)) {
                ReferenceCountUtil.release(frame);
                promise.setFailure(
                    new IllegalArgumentException(
                        "The first frame must be a headers frame. Was: " + frame.name()));
                return;
              }
              firstFrameWritten = true;
              final ChannelFuture future = write0(frame);
              if (future.isDone()) {
            ...
            promise.setFailure(
                new IllegalArgumentException(
                    "Message must be an "
                        + StringUtil.simpleClassName(Http2StreamFrame.class)
                        + ": "
                        + msgStr));
            return;
          }

          final ChannelFuture future = write0(msg);
          if (future.isDone()) {
      ...
      private void firstWriteComplete(final ChannelFuture future, final ChannelPromise promise) {
        ...
        if (cause == null) {
          // As we just finished our first write which made the stream-id valid we need to
          // re-evaluate
      ...
      private void writeComplete(final ChannelFuture future, final ChannelPromise promise) {
        ...
        if (cause == null) {
          ...
          final Throwable error = wrapStreamClosedError(cause);
      ...
      private ChannelFuture write0(final Object msg) {
    ...
    /**
     * {@link ChannelConfig} so that the high and low writebuffer watermarks can reflect the
     * outbound flow control window, without having to create a new {@link WriteBufferWaterMark}
    ...
    private final class Http2StreamChannelConfig extends DefaultChannelConfig {
      ...
      public WriteBufferWaterMark getWriteBufferWaterMark() {
        final int mark = getWriteBufferHighWaterMark();
