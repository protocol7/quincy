...
import static com.protocol7.quincy.http3.Http2CodecUtil.DEFAULT_WINDOW_SIZE;
...
public abstract class DefaultHttp2RemoteFlowControllerTest {
  ...
  @Mock private Http2RemoteFlowController.Listener listener;
  ...
  private void initConnectionAndController() throws Http2Exception {
    ...
    controller =
        new DefaultHttp2RemoteFlowController(connection, newDistributor(connection), listener);
  ...
  public void payloadSmallerThanWindowShouldBeWrittenImmediately() throws Http2Exception {
    final FakeFlowControlled data = new FakeFlowControlled(5);
  ...
  public void emptyPayloadShouldBeWrittenImmediately() throws Http2Exception {
    final FakeFlowControlled data = new FakeFlowControlled(0);
    sendData(STREAM_A, data);
    data.assertNotWritten();
    controller.writePendingBytes();
    data.assertFullyWritten();
  ...
  public void payloadsShouldMerge() throws Http2Exception {
    ...
    final FakeFlowControlled data1 = new FakeFlowControlled(5, true);
    final FakeFlowControlled data2 = new FakeFlowControlled(10, true);
  ...
  public void flowControllerCorrectlyAccountsForBytesWithMerge() throws Http2Exception {
    ...
    final FakeFlowControlled data1 = new FakeFlowControlled(5, 2, true);
    final FakeFlowControlled data2 = new FakeFlowControlled(5, 100, true);
    sendData(STREAM_A, data1);
    sendData(STREAM_A, data2);
    data1.assertNotWritten();
    data1.assertNotWritten();
  ...
  public void stalledStreamShouldQueuePayloads() throws Http2Exception {
    ...
    final FakeFlowControlled data = new FakeFlowControlled(15);
    final FakeFlowControlled moreData = new FakeFlowControlled(0);
    sendData(STREAM_A, data);
    controller.writePendingBytes();
    data.assertNotWritten();
  ...
  public void windowUpdateAndFlushShouldTriggerWrite() throws Http2Exception {
    ...
    final FakeFlowControlled data = new FakeFlowControlled(20);
    final FakeFlowControlled moreData = new FakeFlowControlled(10);
    sendData(STREAM_A, data);
    sendData(STREAM_A, moreData);
    controller.writePendingBytes();
  ...
  public void successiveSendsShouldNotInteract() throws Http2Exception {
    ...
    final FakeFlowControlled dataA = new FakeFlowControlled(10);
    ...
    final FakeFlowControlled dataB = new FakeFlowControlled(10);
  ...
  public void negativeWindowShouldNotThrowException() throws Http2Exception {
    ...
    final FakeFlowControlled data1 = new FakeFlowControlled(initWindow);
    final FakeFlowControlled data2 = new FakeFlowControlled(5);
  ...
  public void initialWindowUpdateShouldSendEmptyFrame() throws Http2Exception {
    ...
    final FakeFlowControlled data = new FakeFlowControlled(10, false);
    sendData(STREAM_A, data);
    controller.writePendingBytes();
    data.assertNotWritten();

    // Now send an empty frame on the same stream and verify that it's also buffered.
    final FakeFlowControlled data2 = new FakeFlowControlled(0, false);
    sendData(STREAM_A, data2);
    controller.writePendingBytes();
    data2.assertNotWritten();
  ...
  public void flowControlledWriteThrowsAnException() throws Exception {
    final Http2RemoteFlowController.FlowControlled flowControlled =
        mockedFlowControlledThatThrowsOnWrite();
    ...
    doAnswer(
        ...
        .when(flowControlled)
        .error(any(ChannelHandlerContext.class), any(Throwable.class));

    final int windowBefore = window(STREAM_A);
  ...
  public void flowControlledWriteCompleteThrowsAnException() throws Exception {
    ...
    doAnswer(
        ...
        .when(flowControlled)
        .write(any(ChannelHandlerContext.class), anyInt());
  ...
  public void nonWritableChannelDoesNotAttemptToWrite() throws Exception {
    ...
    assertWritabilityChanged(1, false);
    reset(listener);
    final FakeFlowControlled dataA = new FakeFlowControlled(1);
    final FakeFlowControlled dataB = new FakeFlowControlled(1);
    final Http2Stream stream = stream(STREAM_A);
  ...
  private void assertWritabilityChanged(final int amt, final boolean writable) {
  ...
  private void sendData(final int streamId, final FakeFlowControlled data) {
    final Http2Stream stream = stream(streamId);
    controller.addFlowControlled(stream, data);
  }

  private void exhaustStreamWindow(final int streamId) throws Http2Exception {
    incrementWindowSize(streamId, -window(streamId));
  }

  private int window(final int streamId) {
    return controller.windowSize(stream(streamId));
  }

  private void incrementWindowSize(final int streamId, final int delta) throws Http2Exception {
  ...
  private void setChannelWritability(final boolean isWritable) throws Http2Exception {
  ...
  private static final class FakeFlowControlled
    ...
    private FakeFlowControlled(final int size) {
      this(size, false);
    }

    private FakeFlowControlled(final int size, final boolean mergeable) {
      this(size, 0, mergeable);
    }

    private FakeFlowControlled(final int payloadSize, final int padding, final boolean mergeable) {
    ...
    @Override
    public void error(final ChannelHandlerContext ctx, final Throwable t) {
      this.t = t;
    }
    ...
    public boolean merge(
      ...
      if (mergeable && next instanceof FakeFlowControlled) {
        final FakeFlowControlled ffcNext = (FakeFlowControlled) next;
    ...
    public void assertPartiallyWritten(final int expectedWritten) {
      assertPartiallyWritten(expectedWritten, 0);
    }

    public void assertPartiallyWritten(final int expectedWritten, final int delta) {
    ...
    public void assertError(final Http2Error error) {
      assertNotNull(t);
      if (error != null) {
