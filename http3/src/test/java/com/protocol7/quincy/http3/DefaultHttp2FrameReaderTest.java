...
import static com.protocol7.quincy.http3.Http2CodecUtil.*;
import static com.protocol7.quincy.http3.Http2FrameTypes.*;
...
public class DefaultHttp2FrameReaderTest {
  ...
  public void readHeaderFrame() throws Http2Exception {
    ...
    try {
      final Http2Headers headers =
          new DefaultHttp2Headers().authority("foo").method("get").path("/").scheme("https");
      final Http2Flags flags = new Http2Flags().endOfHeaders(true).endOfStream(true);
  ...
  public void readHeaderFrameAndContinuationFrame() throws Http2Exception {
    ...
    try {
      final Http2Headers headers =
          new DefaultHttp2Headers().authority("foo").method("get").path("/").scheme("https");
      writeHeaderFrame(
          input, streamId, headers, new Http2Flags().endOfHeaders(false).endOfStream(true));
      writeContinuationFrame(
          input,
          streamId,
          new DefaultHttp2Headers().add("foo", "bar"),
  ...
  public void readUnknownFrame() throws Http2Exception {
    ...
    try {
      ...
      verify(listener).onUnknownFrame(ctx, (byte) 0xff, 0, new Http2Flags(), payload.slice(0, 1));
    } finally {
      payload.release();
      input.release();
    }
  ...
  public void failedWhenUnknownFrameInMiddleOfHeaderBlock() throws Http2Exception {
    ...
    try {
      final Http2Headers headers =
          new DefaultHttp2Headers().authority("foo").method("get").path("/").scheme("https");
      final Http2Flags flags = new Http2Flags().endOfHeaders(false).endOfStream(true);
      writeHeaderFrame(input, streamId, headers, flags);
  ...
  public void failedWhenContinuationFrameStreamIdMismatch() throws Http2Exception {
    ...
    try {
      final Http2Headers headers =
          new DefaultHttp2Headers().authority("foo").method("get").path("/").scheme("https");
      writeHeaderFrame(input, 1, headers, new Http2Flags().endOfHeaders(false).endOfStream(true));
      writeContinuationFrame(
          input,
          3,
          new DefaultHttp2Headers().add("foo", "bar"),
  ...
  public void readHeaderAndData() throws Http2Exception {
    final ByteBuf input = Unpooled.buffer();
    final ByteBuf dataPayload = Unpooled.buffer();
    try {
      final Http2Headers headers =
          new DefaultHttp2Headers().authority("foo").method("get").path("/").scheme("https");
  ...
  private void writeHeaderFrame(
      final ByteBuf output, final int streamId, final Http2Headers headers, final Http2Flags flags)
      throws Http2Exception {
    final ByteBuf headerBlock = Unpooled.buffer();
    try {
      hpackEncoder.encodeHeaders(
          streamId, headerBlock, headers, Http2HeadersEncoder.NEVER_SENSITIVE);
  ...
  private void writeHeaderFrameWithData(
    ...
    try {
      ...
      writeFrameHeader(
          output, dataPayload.readableBytes(), DATA, new Http2Flags().endOfStream(true), streamId);
  ...
  private static void writePriorityFrame(
      final ByteBuf output, final int streamId, final int streamDependency, final int weight) {
