...
public class DataCompressionHttp2Test {
  ...
  public void teardown() throws InterruptedException {
    ...
    final Future<?> serverGroup = sb.config().group().shutdownGracefully(0, 0, MILLISECONDS);
    final Future<?> serverChildGroup =
        sb.config().childGroup().shutdownGracefully(0, 0, MILLISECONDS);
    final Future<?> clientGroup = cb.config().group().shutdownGracefully(0, 0, MILLISECONDS);
    serverGroup.sync();
  ...
  public void gzipEncodingSingleEmptyMessage() throws Exception {
    ...
    try {
      ...
      awaitServer();
      assertEquals(text, serverOut.toString(CharsetUtil.UTF_8));
    } finally {
      data.release();
    }
  }
  ...
  public void gzipEncodingSingleMessage() throws Exception {
    final String text =
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbbbbbccccccccccccccccccccccc";
    final ByteBuf data = Unpooled.copiedBuffer(text.getBytes());
    bootstrapEnv(data.readableBytes());
    try {
      final Http2Headers headers =
  ...
  public void gzipEncodingMultipleMessages() throws Exception {
    ...
    try {
      ...
      awaitServer();
      assertEquals(text1 + text2, serverOut.toString(CharsetUtil.UTF_8));
    } finally {
      data1.release();
      data2.release();
    }
  ...
  public void deflateEncodingWriteLargeMessage() throws Exception {
    ...
    try {
      ...
      awaitServer();
      assertEquals(
          data.resetReaderIndex().toString(CharsetUtil.UTF_8),
          serverOut.toString(CharsetUtil.UTF_8));
    } finally {
      data.release();
    }
  }

  private void bootstrapEnv(final int serverOutSize) throws Exception {
    ...
    // Streams are created before the normal flow for this test, so these connection must be
    ...
    cb.handler(
        new ChannelInitializer<Channel>() {
          ...
          protected void initChannel(final Channel ch) throws Exception {
            ...
            clientConnection
                .remote()
                .flowController(new DefaultHttp2RemoteFlowController(clientConnection));
            clientConnection
                .local()
                .flowController(
            ...
            clientHandler =
                new Http2ConnectionHandlerBuilder()
                    .frameListener(
                        new DelegatingDecompressorFrameListener(clientConnection, clientListener))
    ...
    final ChannelFuture ccf = cb.connect(new InetSocketAddress(NetUtil.LOCALHOST, port));
