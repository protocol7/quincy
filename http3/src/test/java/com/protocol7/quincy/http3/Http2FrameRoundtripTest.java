...
import static com.protocol7.quincy.http3.Http2CodecUtil.MAX_PADDING;
import static com.protocol7.quincy.http3.Http2HeadersEncoder.NEVER_SENSITIVE;
import static com.protocol7.quincy.http3.Http2TestUtil.newTestDecoder;
import static com.protocol7.quincy.http3.Http2TestUtil.newTestEncoder;
import static com.protocol7.quincy.http3.Http2TestUtil.randomString;
...
public class Http2FrameRoundtripTest {
  ...
  public void setup() throws Exception {
    ...
    doAnswer(
            new Answer<ByteBuf>() {
              @Override
              public ByteBuf answer(final InvocationOnMock in) throws Throwable {
    ...
    writer =
        new DefaultHttp2FrameWriter(
            new DefaultHttp2HeadersEncoder(NEVER_SENSITIVE, newTestEncoder()));
  ...
  public void teardown() {
    try {
      ...
      for (final ByteBuf buf : needReleasing) {
        buf.release();
      }
  ...
  public void largeDataFrameShouldMatch() throws Exception {
    ...
    writer.writeData(
        ctx, STREAM_ID, originalData.slice(), originalPadding, endOfStream, ctx.newPromise());
    readFrames();

    // Verify that at least one frame was sent with eos=false and exactly one with eos=true.
    verify(listener, atLeastOnce())
        .onDataRead(eq(ctx), eq(STREAM_ID), any(ByteBuf.class), anyInt(), eq(false));
    ...
    final ArgumentCaptor<Integer> paddingCaptor = ArgumentCaptor.forClass(Integer.class);
    verify(listener, atLeastOnce())
        .onDataRead(
            eq(ctx), eq(STREAM_ID), dataCaptor.capture(), paddingCaptor.capture(), anyBoolean());

    // Make sure the data matches the original.
    for (final ByteBuf chunk : dataCaptor.getAllValues()) {
      final ByteBuf originalChunk = originalData.readSlice(chunk.readableBytes());
    ...
    for (final int framePadding : paddingCaptor.getAllValues()) {
  ...
  public void headersFrameWithPriorityShouldMatch() throws Exception {
    ...
    verify(listener)
        .onHeadersRead(
            eq(ctx), eq(STREAM_ID), eq(headers), eq(4), eq((short) 255), eq(true), eq(0), eq(true));
  }
  ...
  public void headersWithPaddingWithPriorityShouldMatch() throws Exception {
    ...
    verify(listener)
        .onHeadersRead(
            eq(ctx), eq(STREAM_ID), eq(headers), eq(2), eq((short) 3), eq(true), eq(1), eq(true));
  }
  ...
  public void continuedHeadersShouldMatch() throws Exception {
    ...
    verify(listener)
        .onHeadersRead(
            eq(ctx), eq(STREAM_ID), eq(headers), eq(2), eq((short) 3), eq(true), eq(0), eq(true));
  }
  ...
  public void continuedHeadersWithPaddingShouldMatch() throws Exception {
    final Http2Headers headers = largeHeaders();
    writer.writeHeaders(
        ctx, STREAM_ID, headers, 2, (short) 3, true, MAX_PADDING, true, ctx.newPromise());
    readFrames();
    verify(listener)
        .onHeadersRead(
            eq(ctx),
  ...
  private void readFrames() throws Http2Exception {
    ...
    final ByteBuf write = captureWrites();
    reader.readFrame(ctx, write, listener);
  }

  private static ByteBuf data(final int size) {
    final byte[] data = new byte[size];
    for (int ix = 0; ix < data.length; ) {
      final int length = min(MESSAGE.length, data.length - ix);
  ...
  private static ByteBuf buf(final byte[] bytes) {
    return Unpooled.wrappedBuffer(bytes);
  }

  private <T extends ByteBuf> T releaseLater(final T buf) {
  ...
  private ByteBuf captureWrites() {
    ...
    final CompositeByteBuf composite = releaseLater(Unpooled.compositeBuffer());
  ...
  private static Http2Headers headers() {
    return new DefaultHttp2Headers(false)
        ...
        .authority(AsciiString.of("example.org"))
        .path(AsciiString.of("/some/path/resource2"))
        .add(randomString(), randomString());
  }

  private static Http2Headers largeHeaders() {
    final DefaultHttp2Headers headers = new DefaultHttp2Headers(false);
    for (int i = 0; i < 100; ++i) {
      final String key = "this-is-a-test-header-key-" + i;
      final String value = "this-is-a-test-header-value-" + i;
