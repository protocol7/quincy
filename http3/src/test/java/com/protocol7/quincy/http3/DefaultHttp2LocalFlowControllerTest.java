...
import static com.protocol7.quincy.http3.DefaultHttp2LocalFlowController.DEFAULT_WINDOW_UPDATE_RATIO;
...
public class DefaultHttp2LocalFlowControllerTest {
  ...
  public void windowUpdateShouldSendOnceBytesReturned() throws Http2Exception {
    final int dataSize = (int) (DEFAULT_WINDOW_SIZE * DEFAULT_WINDOW_UPDATE_RATIO) + 1;
  ...
  public void halfWindowRemainingShouldUpdateAllWindows() throws Http2Exception {
    final int dataSize = (int) (DEFAULT_WINDOW_SIZE * DEFAULT_WINDOW_UPDATE_RATIO) + 1;
    final int initialWindowSize = DEFAULT_WINDOW_SIZE;
    final int windowDelta = getWindowDelta(initialWindowSize, initialWindowSize, dataSize);
  ...
  public void initialWindowUpdateShouldAllowMoreFrames() throws Http2Exception {
    ...
    final int newInitialWindowSize = 2 * initialWindowSize;
    ...
    final int delta = newInitialWindowSize - initialWindowSize;
  ...
  private void testRatio(
      final float ratio,
      final int newDefaultWindowSize,
      final int newStreamId,
      final boolean setStreamRatio)
      throws Http2Exception {
    final int delta = newDefaultWindowSize - DEFAULT_WINDOW_SIZE;
    controller.incrementWindowSize(stream(0), delta);
    final Http2Stream stream = connection.local().createStream(newStreamId, false);
    if (setStreamRatio) {
    ...
    reset(frameWriter);
    try {
      final int data1 = (int) (newDefaultWindowSize * ratio) + 1;
      final int data2 = (int) (DEFAULT_WINDOW_SIZE * DEFAULT_WINDOW_UPDATE_RATIO) >> 1;
  ...
  private static int getWindowDelta(
      final int initialSize, final int windowSize, final int dataSize) {
    final int newWindowSize = windowSize - dataSize;
    return initialSize - newWindowSize;
  }

  private void receiveFlowControlledFrame(
      final int streamId, final int dataSize, final int padding, final boolean endOfStream)
      throws Http2Exception {
    receiveFlowControlledFrame(stream(streamId), dataSize, padding, endOfStream);
  }

  private void receiveFlowControlledFrame(
      final Http2Stream stream, final int dataSize, final int padding, final boolean endOfStream)
      throws Http2Exception {
  ...
  private static ByteBuf dummyData(final int size) {
    final ByteBuf buffer = Unpooled.buffer(size);
    buffer.writerIndex(size);
    return buffer;
  }

  private boolean consumeBytes(final int streamId, final int numBytes) throws Http2Exception {
    return controller.consumeBytes(stream(streamId), numBytes);
  }

  private void verifyWindowUpdateSent(final int streamId, final int windowSizeIncrement) {
    verify(frameWriter)
        .writeWindowUpdate(eq(ctx), eq(streamId), eq(windowSizeIncrement), eq(promise));
  }

  private void verifyWindowUpdateNotSent(final int streamId) {
  ...
  private void initController(final boolean autoRefillConnectionWindow) throws Http2Exception {
    connection = new DefaultHttp2Connection(false);
    controller =
        new DefaultHttp2LocalFlowController(
                connection, DEFAULT_WINDOW_UPDATE_RATIO, autoRefillConnectionWindow)
