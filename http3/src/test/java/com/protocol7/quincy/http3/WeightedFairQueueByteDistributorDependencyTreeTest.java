...
import static com.protocol7.quincy.http3.Http2CodecUtil.MAX_WEIGHT;
import static com.protocol7.quincy.http3.Http2CodecUtil.MIN_WEIGHT;
...
public class WeightedFairQueueByteDistributorDependencyTreeTest
  ...
  private void setup(final int maxStateOnlySize) {
  ...
  public void closingStreamWithChildrenDoesNotCauseConcurrentModification() throws Http2Exception {
    // We create enough streams to wrap around the child array. We carefully craft the stream ids so
    // that they hash
    // codes overlap with respect to the child collection. If the implementation is not careful this
    // may lead to a
    ...
    for (int i = 0, streamId = 3;
        i < numStreams;
        ++i, streamId += WeightedFairQueueByteDistributor.INITIAL_CHILDREN_MAP_SIZE) {
    ...
    assertEquals(
        WeightedFairQueueByteDistributor.INITIAL_CHILDREN_MAP_SIZE, connection.numActiveStreams());
    streamA.close();
  ...
  public void remoteStreamCanDependUponIdleStream() throws Http2Exception {
    ...
    final Http2Stream streamA = connection.remote().createStream(2, false);
  ...
  public void prioritizeShouldUseDefaults() throws Exception {
    final Http2Stream stream = connection.local().createStream(1, false);
    assertTrue(
        distributor.isChild(
            stream.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
  ...
  public void stateOnlyPriorityShouldBePreservedWhenStreamsAreCreatedAndClosed()
    ...
    final short weight3 = MIN_WEIGHT + 1;
    final short weight5 = (short) (weight3 + 1);
    final short weight7 = (short) (weight5 + 1);
  ...
  private void verifyStateOnlyPriorityShouldBePreservedWhenStreamsAreCreated(
      final short weight3, final short weight5, final short weight7) {
    // Level 0
    assertEquals(1, distributor.numChildren(connection.connectionStream().id()));
  ...
  public void fireFoxQoSStreamsRemainAfterDataStreamsAreClosed() throws Http2Exception {
    ...
    final short htmlGetStreamWeight = 2;
    final Http2Stream htmlGetStream = connection.local().createStream(13, false);
    setPriority(htmlGetStream.id(), followersId, htmlGetStreamWeight, false);
    final Http2Stream favIconStream = connection.local().createStream(15, false);
    setPriority(
        favIconStream.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT, false);
    final Http2Stream cssStream = connection.local().createStream(17, false);
    setPriority(cssStream.id(), leadersId, DEFAULT_PRIORITY_WEIGHT, false);
    final Http2Stream jsStream = connection.local().createStream(19, false);
    setPriority(jsStream.id(), leadersId, DEFAULT_PRIORITY_WEIGHT, false);
    final Http2Stream imageStream = connection.local().createStream(21, false);
    ...
    assertTrue(
        distributor.isChild(unblockedId, connection.connectionStream().id(), unblockedWeight));
    assertEquals(0, distributor.numChildren(unblockedId));

    assertTrue(
        distributor.isChild(backgroundId, connection.connectionStream().id(), backgroundWeight));
    assertEquals(1, distributor.numChildren(backgroundId));

    assertTrue(
        distributor.isChild(
            favIconStream.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
    ...
    // Close all the data streams and ensure the "priority only streams" are retained in the
    // dependency tree.
    htmlGetStream.close();
    favIconStream.close();
    cssStream.close();
  ...
  public void lowestPrecedenceStateShouldBeDropped() throws Http2Exception {
    ...
    final short weight3 = MAX_WEIGHT;
    final short weight5 = (short) (weight3 - 1);
    final short weight7 = (short) (weight5 - 1);
    final short weight9 = (short) (weight7 - 1);
    setPriority(3, connection.connectionStream().id(), weight3, true);
    setPriority(5, connection.connectionStream().id(), weight5, true);
    ...
    // Attempt to create a new item in the dependency tree but the maximum amount of "state only"
    // streams is meet
    // so a stream will have to be dropped. Currently the new stream is the lowest "precedence" so
    // it is dropped.
    setPriority(9, 3, weight9, false);
    assertEquals(0, connection.numActiveStreams());
    verifyLowestPrecedenceStateShouldBeDropped1(weight3, weight5, weight7);

    // Set the priority for stream 9 such that its depth in the dependency tree is numerically lower
    // than stream 3,
    ...
    // Test that stream which has been activated is lower priority than other streams that have not
    // been activated.
    final Http2Stream streamA = connection.local().createStream(5, false);
    streamA.close();
    verifyLowestPrecedenceStateShouldBeDropped2(weight9, weight5, weight7);
    ...
    // All streams are at the same level, so stream ID should be used to drop the numeric lowest
    // valued stream.
    final short weight11 = (short) (weight9 - 1);
    setPriority(11, 0, weight11, false);
    verifyLowestPrecedenceStateShouldBeDropped5(weight7, weight9, weight11);
  }

  private void verifyLowestPrecedenceStateShouldBeDropped1(
      final short weight3, final short weight5, final short weight7) {
    // Level 0
  ...
  private void verifyLowestPrecedenceStateShouldBeDropped2(
      final short weight9, final short weight5, final short weight7) {
    // Level 0
    assertEquals(2, distributor.numChildren(connection.connectionStream().id()));
  ...
  private void verifyLowestPrecedenceStateShouldBeDropped3(
      final short weight3, final short weight7, final short weight9) {
    // Level 0
    assertEquals(2, distributor.numChildren(connection.connectionStream().id()));
  ...
  private void verifyLowestPrecedenceStateShouldBeDropped4(
      final short weight5, final short weight7, final short weight9) {
    // Level 0
    assertEquals(3, distributor.numChildren(connection.connectionStream().id()));
  ...
  private void verifyLowestPrecedenceStateShouldBeDropped5(
      final short weight7, final short weight9, final short weight11) {
    // Level 0
    assertEquals(3, distributor.numChildren(connection.connectionStream().id()));
  ...
  public void priorityOnlyStreamsArePreservedWhenReservedStreamsAreClosed() throws Http2Exception {
    ...
    final short weight3 = MIN_WEIGHT;
    setPriority(3, connection.connectionStream().id(), weight3, true);

    final Http2Stream streamA = connection.local().createStream(5, false);
    final Http2Stream streamB = connection.remote().reservePushStream(4, streamA);
    ...
    assertTrue(
        distributor.isChild(
            streamA.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
    assertEquals(0, distributor.numChildren(streamA.id()));

    assertTrue(
        distributor.isChild(
            streamB.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
  ...
  public void insertExclusiveShouldAddNewLevel() throws Exception {
    final Http2Stream streamA = connection.local().createStream(1, false);
    final Http2Stream streamB = connection.local().createStream(3, false);
    final Http2Stream streamC = connection.local().createStream(5, false);
    final Http2Stream streamD = connection.local().createStream(7, false);
  ...
  public void newExclusiveChildShouldUpdateOldParentCorrectly() throws Http2Exception {
    ...
    final Http2Stream streamC = connection.local().createStream(5, false);
    final Http2Stream streamD = connection.local().createStream(7, false);
    final Http2Stream streamE = connection.local().createStream(9, false);
    final Http2Stream streamF = connection.local().createStream(11, false);
    ...
    assertTrue(
        distributor.isChild(
            streamE.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
  ...
  public void weightChangeWithNoTreeChangeShouldBeRespected() throws Http2Exception {
    ...
    final short newWeight = (short) (DEFAULT_PRIORITY_WEIGHT + 1);
  ...
  public void sameNodeDependentShouldNotStackOverflowNorChangePrioritizableForTree()
    ...
    final boolean[] exclusives = {true, false};
    final short[] weights = {DEFAULT_PRIORITY_WEIGHT, 100, 200, DEFAULT_PRIORITY_WEIGHT};
    ...
    for (final short weight : weights) {
      for (final boolean exclusive : exclusives) {
  ...
  public void closeWithNoPrioritizableDependentsShouldRestructureTree() throws Exception {
    ...
    // Close internal nodes, leave 1 leaf node open, the only remaining stream is the one that is
    // not closed (E).
    streamA.close();
    streamB.close();
    streamC.close();
  ...
  public void priorityChangeWithNoPrioritizableDependentsShouldRestructureTree() throws Exception {
    ...
    assertTrue(
        distributor.isChild(
            streamF.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
  ...
  public void circularDependencyShouldRestructureTree() throws Exception {
    ...
    assertTrue(
        distributor.isChild(
            streamD.id(), connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
    assertEquals(2, distributor.numChildren(streamD.id()));
  ...
  public void unknownParentShouldBeCreatedUnderConnection() throws Exception {
    ...
    assertTrue(
        distributor.isChild(
            streamAId, connection.connectionStream().id(), DEFAULT_PRIORITY_WEIGHT));
