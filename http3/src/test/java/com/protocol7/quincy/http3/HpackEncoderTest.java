...
public class HpackEncoderTest {
  ...
  /**
   * The encoder should not impose an arbitrary limit on the header size if the server has not
   * specified any limit.
   *
  ...
  public void testWillEncode16MBHeaderByDefault() throws Http2Exception {
    final ByteBuf buf = Unpooled.buffer();
    final String bigHeaderName = "x-big-header";
    final int bigHeaderSize = 1024 * 1024 * 16;
    final String bigHeaderVal = new String(new char[bigHeaderSize]).replace('\0', 'X');
    final Http2Headers headersIn = new DefaultHttp2Headers().add("x-big-header", bigHeaderVal);
    final Http2Headers headersOut = new DefaultHttp2Headers();
  ...
  public void testSetMaxHeaderListSizeEnforcedAfterSet() throws Http2Exception {
    ...
    final Http2Headers headers =
        new DefaultHttp2Headers()
            .add("x-big-header", new String(new char[1024 * 16]).replace('\0', 'X'));
