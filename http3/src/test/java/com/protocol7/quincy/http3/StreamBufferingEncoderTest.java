...
import static com.protocol7.quincy.http3.Http2CodecUtil.SMALLEST_MAX_CONCURRENT_STREAMS;
...
public class StreamBufferingEncoderTest {
  ...
  public void setup() throws Exception {
    ...
    final Http2FrameSizePolicy frameSizePolicy = mock(Http2FrameSizePolicy.class);
    ...
    connection
        .local()
        .flowController(new DefaultHttp2LocalFlowController(connection).frameWriter(writer));

    final DefaultHttp2ConnectionEncoder defaultEncoder =
    ...
    doAnswer(
            new Answer<ChannelPromise>() {
              @Override
              public ChannelPromise answer(final InvocationOnMock invocation) throws Throwable {
                return newPromise();
              }
            })
        .when(ctx)
  ...
  public void multipleWritesToActiveStream() {
    ...
    final ArgumentCaptor<ByteBuf> bufCaptor = ArgumentCaptor.forClass(ByteBuf.class);
    verify(writer, times(1))
        .writeData(
            eq(ctx), eq(3), bufCaptor.capture(), eq(0), eq(false), any(ChannelPromise.class));
  ...
  public void sendingGoAwayShouldNotFailStreams() {
    ...
    final ChannelFuture f1 = encoderWriteHeaders(3, newPromise());
    assertEquals(0, encoder.numBufferedStreams());
    final ChannelFuture f2 = encoderWriteHeaders(5, newPromise());
    assertEquals(1, encoder.numBufferedStreams());
    final ChannelFuture f3 = encoderWriteHeaders(7, newPromise());
    assertEquals(2, encoder.numBufferedStreams());

    final ByteBuf empty = Unpooled.buffer(0);
  ...
  public void rstStreamClosesBufferedStream() {
    ...
    final ChannelPromise rstStreamPromise = newPromise();
  ...
  public void bufferUntilSettingsReceived() throws Http2Exception {
    final int initialLimit = SMALLEST_MAX_CONCURRENT_STREAMS;
    final int numStreams = initialLimit * 2;
  ...
  public void exhaustedStreamsDoNotBuffer() throws Http2Exception {
    ...
    final ChannelFuture f = encoderWriteHeaders(-1, newPromise());
  ...
  public void closedBufferedStreamReleasesByteBuf() {
    ...
    final ChannelFuture f1 = encoderWriteHeaders(3, newPromise());
    assertEquals(1, encoder.numBufferedStreams());
    final ChannelFuture f2 = encoder.writeData(ctx, 3, data, 0, false, newPromise());

    final ChannelPromise rstPromise = mock(ChannelPromise.class);
  ...
  public void headersAfterCloseShouldImmediatelyFail() {
    ...
    final ChannelFuture f = encoderWriteHeaders(3, newPromise());
    assertNotNull(f.cause());
  }

  private void setMaxConcurrentStreams(final int newValue) {
    try {
  ...
  private ChannelFuture encoderWriteHeaders(final int streamId, final ChannelPromise promise) {
  ...
  private void writeVerifyWriteHeaders(final VerificationMode mode, final int streamId) {
    verify(writer, mode)
        .writeHeaders(
            eq(ctx),
            eq(streamId),
  ...
  private Answer<ChannelFuture> successAnswer() {
    return new Answer<ChannelFuture>() {
      ...
      public ChannelFuture answer(final InvocationOnMock invocation) throws Throwable {
        for (final Object a : invocation.getArguments()) {
          ReferenceCountUtil.safeRelease(a);
        }

        final ChannelPromise future = newPromise();
        future.setSuccess();
        return future;
      }
    };
  ...
  private static ByteBuf data() {
    final ByteBuf buf = Unpooled.buffer(10);
