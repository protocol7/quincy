...
public class WeightedFairQueueByteDistributorTest
  ...
  private void setup(final int maxStateOnlySize) throws Http2Exception {
    connection = new DefaultHttp2Connection(false);
    distributor =
        maxStateOnlySize >= 0
            ? new WeightedFairQueueByteDistributor(connection, maxStateOnlySize)
            : new WeightedFairQueueByteDistributor(connection);
    distributor.allocationQuantum(ALLOCATION_QUANTUM);

    connection.local().createStream(STREAM_A, false);
    connection.local().createStream(STREAM_B, false);
    final Http2Stream streamC = connection.local().createStream(STREAM_C, false);
    final Http2Stream streamD = connection.local().createStream(STREAM_D, false);
  ...
  /**
   * In this test, we block B such that it has no frames. We distribute enough bytes for all streams
   * and stream B should be preserved in the priority queue structure until it has no "active"
   * children, but it should not be doubly added to stream 0.
   *
   * <pre>
   *         0
   *         |
  ...
  public void writeWithNonActiveStreamShouldNotDobuleAddToPriorityQueue() throws Http2Exception {
    ...
    final Http2Stream streamA = stream(STREAM_A);
    final Http2Stream streamB = stream(STREAM_B);
    final Http2Stream streamC = stream(STREAM_C);
    final Http2Stream streamD = stream(STREAM_D);
  ...
  public void connectionErrorForWriterException() throws Http2Exception {
    ...
    final Exception fakeException = new RuntimeException("Fake exception");
  ...
  /**
   * In this test, we verify that each stream is allocated a minimum chunk size. When bytes run out,
   * the remaining streams will be next in line for the next iteration.
   */
  @Test
  public void minChunkShouldBeAllocatedPerStream() throws Http2Exception {
    ...
    final int written = 3 * ALLOCATION_QUANTUM;
  ...
  /**
   * In this test, we verify that the highest priority frame which has 0 bytes to send, but an empty
   * frame is able to send that empty frame.
   *
   * <pre>
   *         0
   *        / \
  ...
  /**
   * In this test, we block A which allows bytes to be written by C and D. Here's a view of the tree
   * (stream A is blocked).
   *
   * <pre>
   *         0
  ...
  /**
   * In this test, we block A and also give it an empty data frame to send. All bytes should be
   * delegated to by C and D. Here's a view of the tree (stream A is blocked).
   *
   * <pre>
   *           0
   *         /   \
  ...
  /**
   * In this test, we allow A to send, but expect the flow controller will only write to the stream
   * 1 time. This is because we give the stream a chance to write its empty frame 1 time, and the
   * stream will not be written to again until a update stream is called.
   *
   * <pre>
   *         0
   *        / \
  ...
  private void blockedStreamShouldSpreadDataToChildren(final boolean streamAShouldWriteZero)
      throws Http2Exception {
  ...
  /**
   * In this test, we block B which allows all bytes to be written by A. A should not share the data
  ...
  /**
   * In this test, we block B which allows all bytes to be written by A. Once A is complete, it will
   * spill over the remaining of its portion to its children.
   *
   * <pre>
   *         0
   *        / \
  ...
  /**
   * Test that the maximum allowed amount the flow controller allows to be sent is always fully
   * allocated if the streams have at least this much data to send. See
   * https://github.com/netty/netty/issues/4266.
   *
   * <pre>
   *            0
   *          / | \
  ...
  public void writeShouldPreferHighestWeight() throws Http2Exception {
    ...
    // Set allocation quantum to 1 so it is easier to see the ratio of total bytes written between
    // each stream.
  ...
  /**
   * In this test, we root all streams at the connection, block streams C and D, and then verify
   * that data is prioritized toward stream B which has a higher weight than stream A.
   *
   * <p>We also verify that the amount that is written is not uniform, and not always the allocation
   * quantum.
   *
   * <pre>
   *            0
  ...
  /**
   * In this test, we root all streams at the connection, and then verify that data is split equally
   * among the stream, since they all have the same weight.
   *
   * <pre>
   *           0
   *        / / \ \
  ...
  /**
   * In this test, we call distribute with 0 bytes and verify that all streams with 0 bytes are
   * written.
   *
   * <pre>
   *         0
  ...
  /**
   * In this test, we call distribute with 100 bytes which is the total amount eligible to be
   * written, and also have streams with 0 bytes to write. All of these streams should be written
   * with a single call to distribute.
   *
   * <pre>
   *         0
   *        / \
  ...
  public void bytesDistributedWithAdditionShouldBeCorrect() throws Http2Exception {
    final Http2Stream streamE = connection.local().createStream(STREAM_E, false);
  ...
  private boolean write(final int numBytes) throws Http2Exception {
    return distributor.distribute(numBytes, writer);
  }

  private void verifyWrite(final int streamId, final int numBytes) {
    verify(writer).write(same(stream(streamId)), eq(numBytes));
  }

  private void verifyWrite(final VerificationMode mode, final int streamId, final int numBytes) {
    verify(writer, mode).write(same(stream(streamId)), eq(numBytes));
  }

  private void verifyAnyWrite(final int streamId, final int times) {
    verify(writer, times(times)).write(same(stream(streamId)), anyInt());
  }

  private void verifyNeverWrite(final int streamId) {
    verifyNeverWrite(stream(streamId));
  }

  private void verifyNeverWrite(final Http2Stream stream) {
    verify(writer, never()).write(same(stream), anyInt());
  }

  private int captureWrites(final int streamId) {
    return captureWrites(stream(streamId));
  }

  private int captureWrites(final Http2Stream stream) {
    final ArgumentCaptor<Integer> captor = ArgumentCaptor.forClass(Integer.class);
    verify(writer, atLeastOnce()).write(same(stream), captor.capture());
    int total = 0;
    for (final Integer x : captor.getAllValues()) {
      total += x;
    }
    return total;
  }
