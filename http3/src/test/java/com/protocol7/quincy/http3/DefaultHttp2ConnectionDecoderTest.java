...
public class DefaultHttp2ConnectionDecoderTest {
  ...
  @Mock private Http2Stream pushStream;

  @Mock private Http2FrameListener listener;

  @Mock private Http2FrameReader reader;
  ...
  public void setup() throws Exception {
    ...
    doAnswer(
            new Answer<Boolean>() {
              @Override
              public Boolean answer(final InvocationOnMock in) throws Throwable {
    ...
    doAnswer(
            new Answer<Http2Stream>() {
              ...
              public Http2Stream answer(final InvocationOnMock in) throws Throwable {
                final boolean isInformational = in.getArgument(0);
                if (isInformational) {
                  return stream;
                }
                for (; ; ) {
                  final int current = headersReceivedState.get();
                  int next = current;
  ...
  public void dataReadAfterGoAwaySentShouldApplyFlowControl() throws Exception {
    ...
    final int processedBytes = data.readableBytes() + padding;
    ...
    try {
      ...
      verify(listener, never())
          .onDataRead(eq(ctx), anyInt(), any(ByteBuf.class), anyInt(), anyBoolean());
    } finally {
      data.release();
    }
  }
  ...
  public void dataReadAfterGoAwaySentOnUnknownStreamShouldIgnore() throws Exception {
    ...
    try {
      decode().onDataRead(ctx, STREAM_ID, data, 10, true);
      verify(localFlow)
          .receiveFlowControlledFrame((Http2Stream) isNull(), eq(data), eq(10), eq(true));
      verify(listener, never())
          .onDataRead(eq(ctx), anyInt(), any(ByteBuf.class), anyInt(), anyBoolean());
    } finally {
      data.release();
  ...
  public void errorDuringDeliveryShouldReturnCorrectNumberOfBytes() throws Exception {
    ...
    doAnswer(
        ...
        .when(localFlow)
        .unconsumedBytes(eq(stream));
    doAnswer(
            new Answer<Void>() {
              @Override
              public Void answer(final InvocationOnMock in) throws Throwable {
                final int delta = (Integer) in.getArguments()[1];
                final int newValue = unprocessed.addAndGet(-delta);
                if (newValue < 0) {
    ...
    doAnswer(
        ...
        .when(listener)
        .onDataRead(eq(ctx), eq(STREAM_ID), any(ByteBuf.class), eq(10), eq(true));
    try {
      decode().onDataRead(ctx, STREAM_ID, data, padding, true);
  ...
  public void headersReadForStreamThatAlreadySentResetShouldBeIgnored() throws Exception {
    ...
    // Verify that the event was absorbed and not propagated to the observer.
    verify(listener, never())
        .onHeadersRead(eq(ctx), anyInt(), any(Http2Headers.class), anyInt(), anyBoolean());
    verify(remote, never()).createStream(anyInt(), anyBoolean());
    verify(stream, never()).open(anyBoolean());
  }
  ...
  private void infoHeadersAndTrailersAllowed(final boolean eos, final int infoHeaderCount)
      throws Exception {
  ...
  public void headersDependencyNotCreatedShouldCreateAndSucceed() throws Exception {
    ...
    verify(listener)
        .onHeadersRead(
            ...
            eq(STREAM_ID),
            eq(EmptyHttp2Headers.INSTANCE),
            eq(STREAM_DEPENDENCY_ID),
            eq(weight),
            eq(true),
            eq(0),
            eq(true));
    verify(remoteFlow)
        .updateDependencyTree(eq(STREAM_ID), eq(STREAM_DEPENDENCY_ID), eq(weight), eq(true));
  ...
  public void pushPromiseReadAfterGoAwaySentShouldBeIgnored() throws Exception {
    ...
    verify(listener, never())
        .onPushPromiseRead(eq(ctx), anyInt(), anyInt(), any(Http2Headers.class), anyInt());
  }
  ...
  public void pushPromiseReadShouldSucceed() throws Exception {
    ...
    verify(listener)
        .onPushPromiseRead(
            eq(ctx), eq(STREAM_ID), eq(PUSH_STREAM_ID), eq(EmptyHttp2Headers.INSTANCE), eq(0));
  }
  ...
  public void priorityReadShouldNotCreateNewStream() throws Exception {
    ...
    verify(remoteFlow)
        .updateDependencyTree(eq(STREAM_ID), eq(STREAM_DEPENDENCY_ID), eq((short) 255), eq(true));
    verify(listener)
        .onPriorityRead(
            eq(ctx), eq(STREAM_ID), eq(STREAM_DEPENDENCY_ID), eq((short) 255), eq(true));
  ...
  public void rstStreamReadAfterGoAwayShouldSucceed() throws Exception {
    ...
    decode().onRstStreamRead(ctx, STREAM_ID, Http2Error.PROTOCOL_ERROR.code());
  ...
  public void rstStreamReadShouldCloseStream() throws Exception {
    ...
    verify(listener)
        .onRstStreamRead(
            eq(ctx), eq(STREAM_ID), ArgumentMatchers.eq(Http2Error.PROTOCOL_ERROR.code()));
  }
  ...
  private Http2FrameListener decode() throws Exception {
    final ArgumentCaptor<Http2FrameListener> internalListener =
