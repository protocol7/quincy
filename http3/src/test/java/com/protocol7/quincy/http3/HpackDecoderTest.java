...
import static com.protocol7.quincy.http3.HpackDecoder.decodeULE128;
...
public class HpackDecoderTest {
  ...
  private void decode(final String encoded) throws Http2Exception {
    final byte[] b = StringUtil.decodeHexDump(encoded);
    final ByteBuf in = Unpooled.wrappedBuffer(b);
    try {
  ...
  public void testDecodeULE128IntMax() throws Http2Exception {
    final byte[] input = {(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0x07};
  ...
  public void testDecodeULE128IntOverflow2() throws Http2Exception {
    final byte[] input = {(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0x08};
    final ByteBuf in = Unpooled.wrappedBuffer(input);
    final int readerIndex = in.readerIndex();
    try {
      decodeULE128(in, 0);
  ...
  public void testSetTableSizeWithMaxUnsigned32BitValueSucceeds() throws Http2Exception {
    final byte[] input = {
      (byte) 0x3F, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0x0E
    };
    final ByteBuf in = Unpooled.wrappedBuffer(input);
    try {
  ...
  public void testSetTableSizeOverLimitFails() throws Http2Exception {
    ...
    try {
      hpackDecoder.setMaxHeaderTableSize(
          4026531870L - 1); // based on the input above ... 1 less than is above.
      hpackDecoder.decode(0, in, mockHeaders, true);
    } finally {
      in.release();
    }
  ...
  public void testLiteralHuffmanEncodedWithEmptyNameAndValue() throws Http2Exception {
    final byte[] input = {0, (byte) 0x80, 0};
    final ByteBuf in = Unpooled.wrappedBuffer(input);
    try {
      hpackDecoder.decode(0, in, mockHeaders, true);
  ...
  public void testLiteralHuffmanEncodedWithPaddingGreaterThan7Throws() throws Http2Exception {
    final byte[] input = {0, (byte) 0x81, -1};
    final ByteBuf in = Unpooled.wrappedBuffer(input);
    try {
      hpackDecoder.decode(0, in, mockHeaders, true);
    } finally {
  ...
  public void testLiteralHuffmanEncodedWithDecodingEOSThrows() throws Http2Exception {
    final byte[] input = {0, (byte) 0x84, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF};
    final ByteBuf in = Unpooled.wrappedBuffer(input);
    try {
      hpackDecoder.decode(0, in, mockHeaders, true);
    } finally {
  ...
  public void testLiteralHuffmanEncodedWithPaddingNotCorrespondingToMSBThrows()
    ...
    final byte[] input = {0, (byte) 0x81, 0};
    final ByteBuf in = Unpooled.wrappedBuffer(input);
    try {
      hpackDecoder.decode(0, in, mockHeaders, true);
    } finally {
  ...
  public void testIncompleteIndex() throws Http2Exception {
    final byte[] compressed = StringUtil.decodeHexDump("FFF0");
  ...
  public void testMaxValidDynamicTableSize() throws Http2Exception {
    hpackDecoder.setMaxHeaderTableSize(MAX_VALUE);
    final String baseValue = "3FE1FFFFFF0";
    for (int i = 0; i < 7; ++i) {
      decode(baseValue + i);
    }
  }
  ...
  public void testDecodeLargerThanMaxHeaderListSizeUpdatesDynamicTable() throws Http2Exception {
    ...
    try {
      ...
      final HpackEncoder hpackEncoder = new HpackEncoder(true);

      // encode headers that are slightly larger than maxHeaderListSize
      final Http2Headers toEncode = new DefaultHttp2Headers();
      ...
      final Http2Headers decoded2 = new DefaultHttp2Headers();
      hpackDecoder.decode(1, in, decoded2, true);

      final Http2Headers golden = new DefaultHttp2Headers();
      golden.add("test_3", "3");
  ...
  public void testAccountForHeaderOverhead() throws Exception {
    ...
    try {
      final String headerName = "12345";
      final String headerValue = "56789";
      final long headerSize = headerName.length() + headerValue.length();
  ...
  public void pseudoHeaderAfterRegularHeader() throws Exception {
    ...
    try {
      ...
      final Http2Headers toEncode = new InOrderHttp2Headers();
      toEncode.add("test", "1");
      toEncode.add(":method", "GET");
      hpackEncoder.encodeHeaders(1, in, toEncode, NEVER_SENSITIVE);
  ...
  public void failedValidationDoesntCorruptHpack() throws Exception {
    ...
    final ByteBuf in2 = Unpooled.buffer(200);
    try {
      final HpackEncoder hpackEncoder = new HpackEncoder(true);
      ...
      } catch (final Http2Exception.StreamException expected) {
