...
import static com.protocol7.quincy.http3.Http2CodecUtil.connectionPrefaceBuf;
...
import com.protocol7.quincy.http3.Http2CodecUtil.SimpleChannelPromiseAggregator;
...
public class Http2ConnectionHandlerTest {
  ...
  @Mock private Http2Connection connection;

  @Mock private Http2RemoteFlowController remoteFlow;

  @Mock private Http2LocalFlowController localFlow;

  @Mock private Http2Connection.Endpoint<Http2RemoteFlowController> remote;

  @Mock private Http2RemoteFlowController remoteFlowController;

  @Mock private Http2Connection.Endpoint<Http2LocalFlowController> local;

  @Mock private Http2LocalFlowController localFlowController;
  ...
  @Mock private Http2Stream stream;

  @Mock private Http2ConnectionDecoder decoder;

  @Mock private Http2ConnectionEncoder encoder;
  ...
  public void setup() throws Exception {
    ...
    final DefaultChannelConfig config = new DefaultChannelConfig(channel);
    when(channel.config()).thenReturn(config);

    final Throwable fakeException = new RuntimeException("Fake exception");
    ...
    doAnswer(
            new Answer<Http2Stream>() {
              @Override
              public Http2Stream answer(final InvocationOnMock in) throws Throwable {
                final Http2StreamVisitor visitor = in.getArgument(0);
        ...
        .when(connection)
        .forEachActiveStream(any(Http2StreamVisitor.class));
    ...
    doAnswer(
            new Answer() {
              ...
              public Object answer(final InvocationOnMock in) throws Throwable {
                final Object msg = in.getArgument(0);
  ...
  public void serverReceivingClientPrefaceStringFollowedByNonSettingsShouldHandleException()
    ...
    final ByteBuf buf = Unpooled.buffer().writeBytes(connectionPrefaceBuf()).writeZero(10);
    handler.channelRead(ctx, buf);
    final ArgumentCaptor<ByteBuf> captor = ArgumentCaptor.forClass(ByteBuf.class);
    verify(frameWriter, atLeastOnce())
        .writeGoAway(
            eq(ctx),
            eq(0),
            ArgumentMatchers.eq(Http2Error.PROTOCOL_ERROR.code()),
  ...
  public void serverReceivingValidClientPrefaceStringShouldContinueReadingFrames()
    ...
    when(connection.isServer()).thenReturn(true);
    handler = newHandler();
    final ByteBuf prefacePlusSome =
        addSettingsHeader(Unpooled.buffer().writeBytes(connectionPrefaceBuf()));
    handler.channelRead(ctx, prefacePlusSome);
    verify(decoder, atLeastOnce())
        .decodeFrame(
            any(ChannelHandlerContext.class),
            any(ByteBuf.class),
  ...
  public void verifyChannelHandlerCanBeReusedInPipeline() throws Exception {
    when(connection.isServer()).thenReturn(true);
    handler = newHandler();
    // Only read the connection preface...after preface is read internal state of
    // Http2ConnectionHandler
    // is expected to change relative to the pipeline.
    final ByteBuf preface = connectionPrefaceBuf();
    ...
    handler.channelRead(ctx, prefacePlusSome);
    verify(decoder, atLeastOnce())
        .decodeFrame(eq(ctx), any(ByteBuf.class), ArgumentMatchers.<List<Object>>any());
  }
  ...
  public void connectionErrorShouldStartShutdown() throws Exception {
    handler = newHandler();
    final Http2Exception e = new Http2Exception(Http2Error.PROTOCOL_ERROR);
  ...
  public void serverShouldSend431OnHeaderSizeErrorWhenDecodingInitialHeaders() throws Exception {
    final int padding = 0;
    handler = newHandler();
    final Http2Exception e =
        new Http2Exception.HeaderListSizeException(
            STREAM_ID,
            Http2Error.PROTOCOL_ERROR,
            "Header size exceeded max allowed size 8196",
            true);
    ...
    verify(encoder)
        .writeHeaders(eq(ctx), eq(STREAM_ID), captor.capture(), eq(padding), eq(true), eq(promise));
  ...
  public void serverShouldNeverSend431HeaderSizeErrorWhenEncoding() throws Exception {
    ...
    verify(encoder, never())
        .writeHeaders(
            eq(ctx), eq(STREAM_ID), any(Http2Headers.class), eq(padding), eq(true), eq(promise));
    verify(frameWriter).writeRstStream(ctx, STREAM_ID, Http2Error.PROTOCOL_ERROR.code(), promise);
  }
  ...
  public void prefaceUserEventProcessed() throws Exception {
    ...
    handler =
        new Http2ConnectionHandler(decoder, encoder, new Http2Settings()) {
          @Override
  ...
  public void serverShouldCreateStreamIfNeededBeforeSending431() throws Exception {
    ...
    when(remote.lastStreamCreated()).thenReturn(STREAM_ID);
    when(frameWriter.writeRstStream(
            eq(ctx), eq(STREAM_ID), eq(Http2Error.PROTOCOL_ERROR.code()), eq(promise)))
        .thenReturn(future);
    handler.exceptionCaught(ctx, e);
  ...
  public void writeRstOnNonExistantStreamShouldSucceed() throws Exception {
    handler = newHandler();
    when(frameWriter.writeRstStream(
            eq(ctx), eq(NON_EXISTANT_STREAM_ID),
            ArgumentMatchers.eq(Http2Error.STREAM_CLOSED.code()), eq(promise)))
        .thenReturn(future);
    handler.resetStream(ctx, NON_EXISTANT_STREAM_ID, Http2Error.STREAM_CLOSED.code(), promise);
    verify(frameWriter)
        .writeRstStream(
            eq(ctx),
            eq(NON_EXISTANT_STREAM_ID),
            ArgumentMatchers.eq(Http2Error.STREAM_CLOSED.code()),
            eq(promise));
  }
  ...
  public void writeRstOnClosedStreamShouldSucceed() throws Exception {
    ...
    handler.resetStream(ctx, STREAM_ID, Http2Error.STREAM_CLOSED.code(), promise);
    verify(frameWriter)
        .writeRstStream(eq(ctx), eq(STREAM_ID), anyLong(), any(ChannelPromise.class));
  }
  ...
  public void canSendGoAwayFrame() throws Exception {
    ...
    final long errorCode = Http2Error.INTERNAL_ERROR.code();
    when(future.isDone()).thenReturn(true);
    when(future.isSuccess()).thenReturn(true);
    doAnswer(
            new Answer<Void>() {
  ...
  public void canSendGoAwayFramesWithDecreasingLastStreamIds() throws Exception {
    ...
    verify(frameWriter)
        .writeGoAway(eq(ctx), eq(STREAM_ID + 2), eq(errorCode), eq(data), eq(promise));
  ...
  public void cannotSendGoAwayFrameWithIncreasingLastStreamIds() throws Exception {
    ...
    doAnswer(
        ...
        .when(connection)
        .goAwaySent(anyInt(), anyLong(), any(ByteBuf.class));
  ...
  public void canSendGoAwayUsingVoidPromise() throws Exception {
    ...
    doAnswer(
            new Answer<ChannelFuture>() {
              @Override
              public ChannelFuture answer(final InvocationOnMock invocation) throws Throwable {
                final ChannelPromise promise = invocation.getArgument(4);
                assertFalse(promise.isVoid());
                // This is what DefaultHttp2FrameWriter does... I hate mocking :-(.
                final SimpleChannelPromiseAggregator aggregatedPromise =
                    new SimpleChannelPromiseAggregator(
                        promise, channel, ImmediateEventExecutor.INSTANCE);
        ...
        .writeGoAway(
            ...
            any(ByteBuf.class),
            any(ChannelPromise.class));
    handler.goAway(ctx, STREAM_ID, errorCode, data, Http2TestUtil.newVoidPromise(channel));
  ...
  public void gracefulShutdownTimeoutTest() throws Exception {
    ...
    verify(executor, atLeastOnce())
        .schedule(any(Runnable.class), eq(expectedMillis), eq(TimeUnit.MILLISECONDS));
  }
  ...
  private void writeRstStreamUsingVoidPromise(final int streamId) throws Exception {
    handler = newHandler();
    final Throwable cause = new RuntimeException("fake exception");
    when(stream.id()).thenReturn(STREAM_ID);
        ...
        .then(
            new Answer<ChannelFuture>() {
              ...
              public ChannelFuture answer(final InvocationOnMock invocationOnMock)
                  throws Throwable {
                final ChannelPromise promise = invocationOnMock.getArgument(3);
                assertFalse(promise.isVoid());
                return promise.setFailure(cause);
              }
            });
    handler.resetStream(
        ctx, streamId, Http2Error.STREAM_CLOSED.code(), Http2TestUtil.newVoidPromise(channel));
  ...
  private static ByteBuf addSettingsHeader(final ByteBuf buf) {
