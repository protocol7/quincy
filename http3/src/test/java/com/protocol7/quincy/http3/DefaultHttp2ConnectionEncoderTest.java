...
import static com.protocol7.quincy.http3.Http2TestUtil.newVoidPromise;
...
import com.protocol7.quincy.http3.Http2RemoteFlowController.FlowControlled;
...
public class DefaultHttp2ConnectionEncoderTest {
  ...
  @Mock private Http2RemoteFlowController remoteFlow;
  ...
  @Mock private Http2FrameWriter.Configuration writerConfig;

  @Mock private Http2FrameSizePolicy frameSizePolicy;

  @Mock private Http2LifecycleManager lifecycleManager;
  ...
  public void setup() throws Exception {
    ...
    final ChannelMetadata metadata = new ChannelMetadata(false, 16);
    when(channel.isActive()).thenReturn(true);
    when(channel.pipeline()).thenReturn(pipeline);
    when(channel.metadata()).thenReturn(metadata);
    when(channel.unsafe()).thenReturn(unsafe);
    final ChannelConfig config = new DefaultChannelConfig(channel);
    when(channel.config()).thenReturn(config);
    doAnswer(
            new Answer<ChannelFuture>() {
              @Override
              public ChannelFuture answer(final InvocationOnMock in) {
                return newPromise().setFailure((Throwable) in.getArgument(0));
              }
            })
        .when(channel)
        .newFailedFuture(any(Throwable.class));
    ...
    doAnswer(
            new Answer<ChannelFuture>() {
              @Override
              public ChannelFuture answer(final InvocationOnMock in) throws Throwable {
                return ((ChannelPromise) in.getArguments()[2]).setSuccess();
              }
            })
        .when(writer)
        .writeSettings(eq(ctx), any(Http2Settings.class), any(ChannelPromise.class));
    doAnswer(
            new Answer<ChannelFuture>() {
              @Override
              public ChannelFuture answer(final InvocationOnMock in) throws Throwable {
        ...
        .when(writer)
        .writeGoAway(eq(ctx), anyInt(), anyInt(), any(ByteBuf.class), any(ChannelPromise.class));
    writtenData = new ArrayList<String>();
    writtenPadding = new ArrayList<Integer>();
    when(writer.writeData(
            eq(ctx),
            anyInt(),
            any(ByteBuf.class),
            anyInt(),
        ...
        .then(
            new Answer<ChannelFuture>() {
              ...
              public ChannelFuture answer(final InvocationOnMock in) throws Throwable {
                ...
                final ChannelPromise promise = (ChannelPromise) in.getArguments()[5];
                if (streamClosed) {
                ...
                final ByteBuf data = (ByteBuf) in.getArguments()[2];
        ...
        .then(
            new Answer<ChannelFuture>() {
              ...
              public ChannelFuture answer(final InvocationOnMock invocationOnMock)
                  throws Throwable {
                final ChannelPromise promise = (ChannelPromise) invocationOnMock.getArguments()[8];
                if (streamClosed) {
                  fail("Stream already closed");
                } else {
    ...
    doAnswer(
            new Answer<ChannelPromise>() {
              @Override
              public ChannelPromise answer(final InvocationOnMock in) throws Throwable {
                return newPromise();
              }
            })
        .when(ctx)
  ...
  private void dataWriteShouldSignalThatFrameWasConsumedOnError0(final boolean endOfStream)
      throws Exception {
    createStream(STREAM_ID, false);
    final ByteBuf data = dummyData();
    final ChannelPromise p = newPromise();
    encoder.writeData(ctx, STREAM_ID, data, 0, endOfStream, p);

    final FlowControlled controlled = payloadCaptor.getValue();
  ...
  public void dataFramesShouldMerge() throws Exception {
    ...
    final ChannelPromise promise1 = newPromise();
    encoder.writeData(ctx, STREAM_ID, data, 0, true, promise1);
    final ChannelPromise promise2 = newPromise();
    encoder.writeData(ctx, STREAM_ID, data, 0, true, promise2);

    // Now merge the two payloads.
    final List<FlowControlled> capturedWrites = payloadCaptor.getAllValues();
    final FlowControlled mergedPayload = capturedWrites.get(0);
  ...
  public void dataFramesShouldMergeUseVoidPromise() throws Exception {
    ...
    final ChannelPromise promise1 = newVoidPromise(channel);
    encoder.writeData(ctx, STREAM_ID, data, 0, true, promise1);
    final ChannelPromise promise2 = newVoidPromise(channel);
    encoder.writeData(ctx, STREAM_ID, data, 0, true, promise2);
  ...
  public void emptyFrameShouldSplitPadding() throws Exception {
    final ByteBuf data = Unpooled.buffer(0);
  ...
  public void writeHeadersUsingVoidPromise() throws Exception {
    ...
    encoder.writeHeaders(
        ctx, STREAM_ID, EmptyHttp2Headers.INSTANCE, 0, true, newVoidPromise(channel));
  ...
  private void assertSplitPaddingOnEmptyBuffer(final ByteBuf data) throws Exception {
    createStream(STREAM_ID, false);
  ...
  public void headersWriteForUnknownStreamShouldCreateStream() throws Exception {
    ...
    verify(writer)
        .writeHeaders(
            eq(ctx),
            eq(streamId),
            eq(EmptyHttp2Headers.INSTANCE),
            eq(0),
            eq(DEFAULT_PRIORITY_WEIGHT),
            eq(false),
            eq(0),
            eq(false),
            eq(promise));
  ...
  public void headersWriteShouldOpenStreamForPush() throws Exception {
    writeAllFlowControlledFrames();
    final Http2Stream parent = createStream(STREAM_ID, false);
  ...
  private void tooManyHeadersThrows(final boolean eos) {
    writeAllFlowControlledFrames();
    final int streamId = 6;
    final ChannelPromise promise = newPromise();
    encoder.writeHeaders(ctx, streamId, EmptyHttp2Headers.INSTANCE, 0, false, promise);
    ...
    final ChannelPromise promise3 = newPromise();
    final ChannelFuture future =
  ...
  private void infoHeadersAndTrailers(final boolean eos, final int infoHeaderCount) {
    writeAllFlowControlledFrames();
    final int streamId = 6;
    final Http2Headers infoHeaders = informationalHeaders();
    ...
    verify(writer, times(infoHeaderCount))
        .writeHeaders(
            eq(ctx),
            eq(streamId),
            eq(infoHeaders),
  ...
  private void tooManyHeadersWithDataThrows(final boolean eos) {
    writeAllFlowControlledFrames();
    final int streamId = 6;
    final ChannelPromise promise = newPromise();
    encoder.writeHeaders(ctx, streamId, EmptyHttp2Headers.INSTANCE, 0, false, promise);
  ...
  private void infoHeadersAndTrailersWithData(final boolean eos, final int infoHeaderCount) {
    writeAllFlowControlledFrames();
    final int streamId = 6;
    final Http2Headers infoHeaders = informationalHeaders();
    for (int i = 0; i < infoHeaderCount; ++i) {
  ...
  public void pushPromiseWriteAfterGoAwayReceivedShouldFail() throws Exception {
    ...
    final ChannelFuture future =
        encoder.writePushPromise(
            ctx, STREAM_ID, PUSH_STREAM_ID, EmptyHttp2Headers.INSTANCE, 0, newPromise());
    assertTrue(future.isDone());
    assertFalse(future.isSuccess());
  }
  ...
  public void pushPromiseWriteShouldReserveStream() throws Exception {
    ...
    final ChannelPromise promise = newPromise();
    encoder.writePushPromise(
        ctx, STREAM_ID, PUSH_STREAM_ID, EmptyHttp2Headers.INSTANCE, 0, promise);
  ...
  public void priorityWriteOnPreviouslyExistingParentStreamShouldSucceed() throws Exception {
    ...
    verify(writer)
        .writePriority(
            eq(ctx), eq(STREAM_ID), eq(parentStreamId), eq(weight), eq(true), eq(promise));
  }
  ...
  public void rstStreamShouldCloseStream() throws Exception {
    ...
    stream(STREAM_ID);
    final ChannelPromise promise = newPromise();
    encoder.writeRstStream(ctx, STREAM_ID, Http2Error.PROTOCOL_ERROR.code(), promise);
  ...
  public void dataWriteShouldCreateHalfClosedStream() throws Exception {
    ...
    final Http2Stream stream = createStream(STREAM_ID, false);
    final ByteBuf data = dummyData();
    final ChannelPromise promise = newPromise();
  ...
  public void headersWriteShouldHalfClosePushStream() throws Exception {
    writeAllFlowControlledFrames();
    final Http2Stream parent = createStream(STREAM_ID, false);
    final Http2Stream stream = reservePushStream(PUSH_STREAM_ID, parent);
    final ChannelPromise promise = newPromise();
  ...
  public void headersWriteShouldHalfCloseAfterOnErrorForPreCreatedStream() throws Exception {
    ...
    when(writer.writeHeaders(
        ...
        .thenAnswer(
            new Answer<ChannelFuture>() {
              @Override
              public ChannelFuture answer(final InvocationOnMock invocation) {
    ...
    final InOrder inOrder = inOrder(lifecycleManager);
  ...
  public void encoderDelegatesGoAwayToLifeCycleManager() {
    ...
    verify(lifecycleManager)
        .goAway(
            eq(ctx),
            eq(STREAM_ID),
            eq(Http2Error.INTERNAL_ERROR.code()),
            eq((ByteBuf) null),
            eq(promise));
  ...
  public void dataWriteToClosedStreamShouldFail() throws Exception {
    createStream(STREAM_ID, false).close();
    final ByteBuf data = mock(ByteBuf.class);
    final ChannelPromise promise = newPromise();
    ...
    doAnswer(
            new Answer<Void>() {
              ...
              public Void answer(final InvocationOnMock invocationOnMock) throws Throwable {
                final FlowControlled flowControlled =
                    (FlowControlled) invocationOnMock.getArguments()[1];
        ...
        .when(remoteFlow)
        .addFlowControlled(any(Http2Stream.class), payloadCaptor.capture());
  }

  private Http2Stream createStream(final int streamId, final boolean halfClosed)
      throws Http2Exception {
    return connection.local().createStream(streamId, halfClosed);
  }

  private Http2Stream reservePushStream(final int pushStreamId, final Http2Stream parent)
      throws Http2Exception {
    return connection.local().reservePushStream(pushStreamId, parent);
  }

  private Http2Stream stream(final int streamId) {
    return connection.stream(streamId);
  }

  private void goAwayReceived(final int lastStreamId) throws Http2Exception {
    connection.goAwayReceived(lastStreamId, 0, EMPTY_BUFFER);
  }

  private void goAwaySent(final int lastStreamId) throws Http2Exception {
