...
public class Http2MultiplexCodecBuilderTest {
  ...
  public void setUp() throws InterruptedException {
    ...
    final LocalAddress serverAddress = new LocalAddress(getClass().getName());
  ...
  private Http2StreamChannel newOutboundStream(final ChannelHandler handler) {
    return new Http2StreamChannelBootstrap(clientChannel)
        .handler(handler)
        .open()
        .syncUninterruptibly()
        .getNow();
  ...
  public void multipleOutboundStreams() throws Exception {
    final Http2StreamChannel childChannel1 = newOutboundStream(new TestChannelInitializer());
    assertTrue(childChannel1.isActive());
    assertFalse(isStreamIdValid(childChannel1.stream().id()));
    final Http2StreamChannel childChannel2 = newOutboundStream(new TestChannelInitializer());
    ...
    // Test that streams can be made active (headers sent) in different order than the corresponding
    // channels
    // have been created.
    ...
    final Http2HeadersFrame headersFrame2 = serverLastInboundHandler.blockingReadInbound();
    assertNotNull(headersFrame2);
    assertEquals(3, headersFrame2.stream().id());

    final Http2HeadersFrame headersFrame1 = serverLastInboundHandler.blockingReadInbound();
  ...
  public void createOutboundStream() throws Exception {
    final Channel childChannel = newOutboundStream(new TestChannelInitializer());
    ...
    final Http2HeadersFrame headersFrame = serverLastInboundHandler.blockingReadInbound();
    assertNotNull(headersFrame);
    assertEquals(3, headersFrame.stream().id());
    assertEquals(headers, headersFrame.headers());

    final Http2DataFrame dataFrame = serverLastInboundHandler.blockingReadInbound();
    ...
    final Http2ResetFrame rstFrame = serverLastInboundHandler.blockingReadInbound();
