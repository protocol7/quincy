...
public class Http2MultiplexCodecTest {
    ...
    private Http2StreamChannel newInboundStream(final int streamId, final boolean endStream, final ChannelHandler childHandler) {
        return newInboundStream(streamId, endStream, null, childHandler);
    }

    private Http2StreamChannel newInboundStream(final int streamId, final boolean endStream,
                                                final AtomicInteger maxReads, final ChannelHandler childHandler) {
    ...
    public void headerAndDataFramesShouldBeDelivered() {
        ...
        final Http2HeadersFrame headersFrame = new DefaultHttp2HeadersFrame(request).stream(channel.stream());
        final Http2DataFrame dataFrame1 = new DefaultHttp2DataFrame(bb("hello")).stream(channel.stream());
        final Http2DataFrame dataFrame2 = new DefaultHttp2DataFrame(bb("world")).stream(channel.stream());
    ...
    public void unhandledHttp2FramesShouldBePropagated() {
        ...
        final DefaultHttp2GoAwayFrame goAwayFrame = new DefaultHttp2GoAwayFrame(1,
    ...
    public void streamExceptionTriggersChildChannelExceptionAndClose() throws Exception {
        ...
        final StreamException cause = new StreamException(channel.stream().id(), Http2Error.PROTOCOL_ERROR, "baaam!");
    ...
    public void creatingWritingReadingAndClosingOutboundStreamShouldWork() {
        ...
        final Http2Headers headers = new DefaultHttp2Headers().scheme("https").method("GET").path("/foo.txt");
    ...
    public void endOfStreamDoesNotDiscardData() {
        ...
        final Consumer<ChannelHandlerContext> ctxConsumer = new Consumer<ChannelHandlerContext>() {
            @Override
        ...
        final Http2DataFrame dataFrame1 = new DefaultHttp2DataFrame(bb("1")).stream(childChannel.stream());
        final Http2DataFrame dataFrame2 = new DefaultHttp2DataFrame(bb("2")).stream(childChannel.stream());
        final Http2DataFrame dataFrame3 = new DefaultHttp2DataFrame(bb("3")).stream(childChannel.stream());
        final Http2DataFrame dataFrame4 = new DefaultHttp2DataFrame(bb("4")).stream(childChannel.stream());

        assertEquals(new DefaultHttp2HeadersFrame(request).stream(childChannel.stream()), inboundHandler.readInbound());

        final ChannelHandler readCompleteSupressHandler = new ChannelInboundHandlerAdapter() {
            @Override
            public void channelReadComplete(final ChannelHandlerContext ctx) {
    ...
    private static void verifyFramesMultiplexedToCorrectChannel(final Http2StreamChannel streamChannel,
