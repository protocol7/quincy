...
public class ReadOnlyHttp2HeadersTest {
  ...
  public void pseudoHeaderNotAllowedAfterNonPseudoHeaders() {
    ReadOnlyHttp2Headers.trailers(
        true,
        new AsciiString(":name"),
        new AsciiString("foo"),
        new AsciiString("othername"),
        new AsciiString("goo"),
        new AsciiString(":pseudo"),
        new AsciiString("val"));
  }
  ...
  public void testSize() {
    final Http2Headers headers = newTrailers();
  ...
  public void testGet() {
    ...
    assertTrue(
        AsciiString.contentEqualsIgnoreCase(
            "/foo", headers.get(Http2Headers.PseudoHeaderName.PATH.value())));
  ...
  private static void testValueIteratorSingleValue(
      final Http2Headers headers, final CharSequence name, final CharSequence value) {
    final Iterator<CharSequence> itr = headers.valueIterator(name);
    assertTrue(itr.hasNext());
    assertTrue(AsciiString.contentEqualsIgnoreCase(value, itr.next()));
    assertFalse(itr.hasNext());
  }

  private static void testIteratorReadOnly(final Http2Headers headers) {
    final Iterator<Map.Entry<CharSequence, CharSequence>> itr = headers.iterator();
    assertTrue(itr.hasNext());
    itr.remove();
  }

  private static void testIteratorEntryReadOnly(final Http2Headers headers) {
    final Iterator<Map.Entry<CharSequence, CharSequence>> itr = headers.iterator();
    assertTrue(itr.hasNext());
  ...
  private static ReadOnlyHttp2Headers newClientHeaders() {
    return ReadOnlyHttp2Headers.clientHeaders(
        false,
        new AsciiString("meth"),
        new AsciiString("/foo"),
        new AsciiString("schemer"),
        new AsciiString("respect_my_authority"),
        otherHeaders());
  }
