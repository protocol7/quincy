...
public class TestChannelInitializer extends ChannelInitializer<Channel> {
  ...
  /**
   * Designed to read a single byte at a time to control the number of reads done at a fine
   * granularity.
   */
  static final class TestNumReadsRecvByteBufAllocator implements RecvByteBufAllocator {
    private final AtomicInteger numReads;

    private TestNumReadsRecvByteBufAllocator(final AtomicInteger numReads) {
    ...
    public ExtendedHandle newHandle() {
      return new ExtendedHandle() {
        ...
        @Override
        public int guess() {
          return 1; // only ever allocate buffers of size 1 to ensure the number of reads is
          // controlled.
        }
