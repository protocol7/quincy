...
final class Field25519 {
  ...
  static void reduceCoefficients(long[] output) {
    ...
    for (int i = 0; i < LIMB_CNT; i += 2) {
      ...
      // the first iteration of this loop. This is added to the handleServerHello limb and we can approximate the
      // resulting bound of that limb by 281*2^54.
      output[i] -= over << 26;
      output[i + 1] += over;

      // For the first iteration, |output[i+1]| < 281*2^54, thus |over| < 281*2^29. When this is
      // added to the handleServerHello limb, the resulting bound can be approximated as 281*2^54.
      //
  ...
  static byte[] contract(long[] inputLimbs) {
    ...
    for (int j = 0; j < 2; j++) {
      for (int i = 0; i < 9; i++) {
        ...
        // from the handleServerHello-larger limb.
